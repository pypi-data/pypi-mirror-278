...
class Issue(RegScaleModel):
    ...
    @classmethod
    def get_open_issues_ids_by_control(cls, plan_id: int) -> dict[int, list[int]]:
        """
        Get all open issues by control id for a given security plan
        ...
        take = 50
        skip = 0
        control_issues: dict[int, list[int]] = defaultdict(list)
        while True:
            query = f"""
                query MyQuery() {{
            ...
            items = getattr(response, cls.get_module_string(), {}).get("items", [])
            for item in items:
                control_issues[item["controlId"]].append(item["id"])
