...
class STIGMapper:
    ...
    def map_stigs_to_assets(
        ...
        for rule in self.rules:
            ...
            if not component_id:
                continue
            matching_assets = self.find_matching_assets(name=value, asset_list=asset_list, comparator=comparator)
    ...
    @staticmethod
    def find_matching_assets(name: str, asset_list: List["Asset"], comparator: str) -> List["Asset"]:
        """
        Find matching assets based on the comparator method.
        :param str name: The name to compare
        :param List[Asset] asset_list: A list of assets
        ...
        if comparator_func:
            for asset in asset_list:
                if comparator_func(asset.name, name):
