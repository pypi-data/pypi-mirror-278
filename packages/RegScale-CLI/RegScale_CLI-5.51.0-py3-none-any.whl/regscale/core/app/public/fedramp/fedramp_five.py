...
def handle_parts(
    ...
    for index, part in enumerate(parts):
        logger.info(f"Part: {part.get('name')}")
        if part.get("value") == "":
            continue
        logger.info(f"Control: {control.controlId} Part: {part.get('name')}")
        part_name = part.get("name", "")
        logger.info(f"Part Name: {part_name}")
        part_letter = get_part_letter(part_name)
        logger.info(f"Part Letter: {part_letter}")
        logger.info(f"Control Id: {control.controlId}")
        ...
        logger.info(f"Matching Objectives: {matching_objectives}")
        handle_matching_objectives(
            ...
            parent_id=parent_id,
            status=status,
            multiple_control_objectives=multiple_control_objectives,
...
def handle_matching_objectives(
    ...
    parent_id: int,
    status: Optional[str],
    multiple_control_objectives: bool,
    ...
    :param int parent_id: The parent ID.
    :param Optional[str] status: The status of the implementation.
    :param bool multiple_control_objectives: Whether there are multiple control objectives.
    ...
    for objective in matching_objectives:
        ...
        existing_options = ImplementationOption.get_all_by_parent(parent_id=control.id, plan_id=parent_id)
        option = get_or_create_option(
            part_name=f"{control.controlId}:{part_name}",
            part_value=f"{part_name}: {part.get('value')}",
            control=control,
        ...
        if not option:
            logger.warning(f"Option not found for part: {part_name}")
            continue
        has_existing_obj = check_for_existing_objective(control_implementation, objective, option, status)
        if has_existing_obj:
            continue
        duplicate = True if option.id in option_ids_used else False
        handle_implementation_objectives(
            ...
            duplicate,
            origination,
            multiple_control_objectives,
        )
        option_ids_used.append(option.id)
...
def check_for_existing_objective(
    ...
    for existing_obj in existing_objectives:
        if existing_obj.objectiveId == objective.id and existing_obj.optionId == option.id:
            existing_obj.status = status
            existing_obj.statement = option.description
...
def handle_implementation_objectives(
    ...
    duplicate: bool,
    origination: str = None,
    multiple_control_objectives: bool = False,
):
    """
    ...
    :param ImplementationOption option:
    :param str status:
    ...
    imp_obj = ImplementationObjective(
        ...
        objectiveId=objective.id,
        optionId=option.id,
        status=status if not multiple_control_objectives else ImplementationStatus.NOT_IMPLEMENTED.value,
        statement=option.description,
        notes="#replicated-data-part" if duplicate else "",
        responsibility=map_responsibility(responsibility=origination),
    )
