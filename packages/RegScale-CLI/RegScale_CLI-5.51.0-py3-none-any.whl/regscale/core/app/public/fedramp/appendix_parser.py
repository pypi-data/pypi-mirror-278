...
class AppendixAParser:
    ...
    def determine_status(text: str) -> str:
        ...
        # this works when the docx file has a checkbox as a unicode character and
        # not an actual checkbox which happens in some cases
        for status, keywords in STATUS_KEYWORDS.items():
            for keyword in keywords:
                if f"1 {keyword}" in " ".join(tokens).lower() or f"☒ {keyword}" in " ".join(tokens).lower():
                    return status  # Return the first matching status
        return DEFAULT_STATUS  # Return DEFAULT_STATUS if no keywords matched
