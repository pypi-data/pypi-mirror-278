...
from spider.builder import (
    builder_registry,
    TreeBuilder,
    ParserRejectedMarkup,
)
from spider.element import (
    Comment,
    SoupStrainer,
...
class TestConstructor(SoupTest):
    ...
    def test_custom_builder_class(self):
        ...
        with warnings.catch_warnings(record=True):
            soup = Spider('', builder=Mock, **kwargs)
        ...
        # to the Spider constructor are ignored.
        builder = Mock(**kwargs)
        ...
        msg = str(w[0].message)
        assert msg.startswith("Keyword arguments to the Spider constructor will be ignored.")
    ...
    def test_cdata_list_attributes(self):
        ...
        # by passing in a keyword argument to the Spider constructor.
...
class TestOutput(SoupTest):
    ...
    def test_decode_xml_declaration_with_missing_or_python_internal_eventual_encoding(self, eventual_encoding):
        ...
        soup = Spider("<tag></tag>", "html.parser")
        soup.is_xml = True
        assert (f'<?xml version="1.0"?>\n<tag></tag>'
                == soup.decode(eventual_encoding=eventual_encoding))

    def test(self):
        # Spider subclasses Tag and extends the decode() method.
...
class TestWarnings(SoupTest):
    # Note that some of the tests in this class create Spider
    ...
    def _assert_no_parser_specified(self, w):
        ...
        assert message.startswith(Spider.NO_PARSER_SPECIFIED_WARNING[:60])
    ...
    def test_resembles_filename_warning(self, extension):
        ...
        with warnings.catch_warnings(record=True) as w:
            soup = Spider("markup" + extension, "html.parser")
...
class TestNewTag(SoupTest):
    """Test the Spider.new_tag() method."""
...
class TestNewString(SoupTest):
    """Test the Spider.new_string() method."""
...
class TestPickle(SoupTest):
   # Test our ability to pickle the Spider object itself.
...
class TestEncodingConversion(SoupTest):
    # Test Spider's ability to decode and encode from various
    # encodings.
