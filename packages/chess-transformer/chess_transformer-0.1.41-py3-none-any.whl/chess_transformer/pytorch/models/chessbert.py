...
class ChessBert(nn.Module):
  ...
  def append_tokens(self, num_tokens: int):
    """Append `num_tokens` new tokens in the model's vocabulary"""
    config = self.config
    new_vocab_size = config.vocab_size + num_tokens
    new_word_embed = nn.Embedding(new_vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
    old_embed = self.bert.embeddings.word_embeddings.weight
    with torch.no_grad():
      new_word_embed.weight[:config.vocab_size] = old_embed
      new_word_embed.weight[config.vocab_size:] = torch.mean(old_embed, dim=0)
