...
class GaussianAtom(object):
    ...
    def plot(self):
        """
        Represents the atom in a mayavi canvas
        """
        from mayavi import mlab
        code = colors_codes()[self.color]
        mlab.points3d(
            *self.position,
            scale_factor=1,
            resolution=20,
...
class GaussianGeometry(object):
    ...
    def plot(self, cut:float=1.8):
        """
        Plots the molecule in the current mayavi canvas
        """
        from mayavi import mlab
        # Plot the atoms
        for atom in self:
    ...
    @property
    def moleculegro(self):
        """
        The MoleculeGro version of the geometry.
        """
        from gaddlemaps.components import AtomGro
        atoms = []
        index = 1
