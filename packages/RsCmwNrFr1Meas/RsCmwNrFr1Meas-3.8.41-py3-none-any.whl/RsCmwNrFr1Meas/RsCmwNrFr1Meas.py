...
class RsCmwNrFr1Meas:
	"""762 total commands, 5 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW100,100001,3.8.41.0002'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		"""Initializes new RsCmwNrFr1Meas session. \n
		Parameter options tokens examples:
			...
			- ``LogToGlobalTarget = True`` - Sets the logging target to the class-property previously set with RsCmwNrFr1Meas.set_global_logging_target() Default: ``False``
		...
		self._core = Core(resource_name, id_query, reset, RsCmwNrFr1Meas._driver_options, options, direct_session)
		self._core.driver_version = '3.8.41.0002'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwNrFr1Meas':
		"""Creates a new RsCmwNrFr1Meas object with the entered 'session' reused. \n
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwNrFr1Meas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwNrFr1Meas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.41.0002'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwNrFr1Meas version failed. Current version: '3.8.41.0002', minimum required version: '{min_version}'")
	...
	def close(self) -> None:
		"""Closes the active RsCmwNrFr1Meas session."""
		self._core.io.close()
	...
	def _sync_to_custom_properties(self, cloned: 'RsCmwNrFr1Meas') -> None:
	...
	@property
	def nrSubMeas(self):
		"""nrSubMeas commands group. 1 Sub-classes, 0 commands."""
	...
	def clone(self) -> 'RsCmwNrFr1Meas':
		"""Creates a deep copy of the RsCmwNrFr1Meas object. Also copies:
		...
		cloned = RsCmwNrFr1Meas.from_existing_session(self.get_session_handle(), self._options)
