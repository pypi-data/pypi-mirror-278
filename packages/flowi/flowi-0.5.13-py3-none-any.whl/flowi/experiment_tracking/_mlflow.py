...
class MLflow(Base):
    def __init__(self, flow_name_fqdn: str, version: str):
        ...
        self._session_run_id = self.start_experiment(session=True)
    ...
    def start_experiment(self, session: bool = False) -> str:
        if session:
            run = self._client.create_run(experiment_id=self._experiment_id, tags={"version": self._version})
        else:
            run = self._client.create_run(experiment_id=self._experiment_id, tags={
                "version": self._version,
                "mlflow.parentRunId": self._session_run_id
            })
