...
class EfficientFormer(nn.Module):
    ...
    Args:
        ...
        norm_layer ( nn.Module, optional): The norm layer type. Default: nn.BatchNorm2d
        act_layer ( nn.Module, optional): The activate function type. Default: nn.GELU
...
class Attention4D( nn.Module):
    def __init__(self,
        ...
        self.register_buffer(
            'attention_bias_idxs', torch.ones(
                [49, 49], dtype=torch.int64))
        ...
        self.register_buffer('attention_bias_idxs_seg',
                             torch.from_numpy(np.array(idxs)).reshape([N, N]))
...
class Attention4DDownsample( nn.Module):
    def __init__(
        ...
        self.register_buffer(
            ...
            torch.from_numpy(
                np.array(idxs,dtype=np.int64)).reshape([N_, N]))
...
class AttnFFN( nn.Module):
    def __init__(self,
        ...
        if use_layer_scale:
            param_value = layer_scale_init_value * torch.ones(
...
class FFN( nn.Module):
    def __init__(self,
        ...
        self.use_layer_scale = use_layer_scale
        if use_layer_scale:
            param_value_2 = layer_scale_init_value * torch.ones(
                [dim]).unsqueeze(-1).unsqueeze(-1)
            self.layer_scale_2 = self.create_parameter(
