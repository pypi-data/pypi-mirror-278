...
class InstancePrinter(NVPrettyPrint):
    ...
    def print_list(self, instance_list, columns=None):
        ...
        if self.config.format_type == "json":
            ...
            for instance in instance_list:
                out = InstanceOutput(instance, self.config)
                output.append([getattr(out, col, "") for col in cols])
        self.print_data(output, True)

    def print_info(self, instance, status_history=False):

        if self.config.format_type == "json":
            self.print_data(instance)
        else:
            output = InstanceOutput(instance)
            ...
            for ib in output.infinibandInterfaces:
                ibo = InfinibandInterfaceOutput(ib)
    ...
    def print_list_interface(self, it_list):
...
class InfinibandInterfaceOutput:
    def __init__(self, interface):
        self.interface = interface
