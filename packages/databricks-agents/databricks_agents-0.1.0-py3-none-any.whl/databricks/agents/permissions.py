...
class UserPermissionAclRequest:
    ...
    def to_experiment_permissions_object(self):
        return WorkspaceObjectAccessControlRequest.from_dict(
            self._build_acl_permission_dict()
        )

    def to_serving_endpoint_permissions_object(self):
        return ServingEndpointAccessControlRequest.from_dict(
            self._build_acl_permission_dict()
        )
...
def _remove_users_from_permissions_list(permissions, users):
    ...
    modified_acls = list(
        filter(lambda acl: _extract_permission_identifier(acl) not in user_set, acls)
    )
...
def _extract_permission_identifier(permission_acl) -> str:
    return (
        permission_acl.user_name
        or permission_acl.group_name
        or permission_acl.service_principal_name
    )
...
def _update_permissions_on_endpoint(
    ...
    acl_requests = [
        UserPermissionAclRequest(
            user, permission_level
        ).to_serving_endpoint_permissions_object()
        for user in users
    ]
    w = WorkspaceClient()
    _call_workspace_api(
...
def _update_permissions_on_experiment(
    ...
    acl_requests = [
        UserPermissionAclRequest(
            user, permission_level
        ).to_experiment_permissions_object()
        for user in users
    ]
...
def set_permissions(
    ...
    :param users: List of account user emails or groups.
    :param permission_level: Permissions level assigned to the list of users. Supported permission levels are
                             ...
                             `CAN_MANAGE`: users can update existing RAG chain deployments and deploy agents.
    """
    users_set = set(users)
