...
class Youtubi : 
	...
	def download(self,video_item, folder_path  , format_title = "{index}--{playlist}---{idx}--{title}") : 
		try : 
			...
			mp4_streams.download(output_path=folder_path , filename = format_title.format(**video_item))
		except Exception as err : 
			print(f"Error {err}")
			print(f"Error {err}" , file = open(f'{format_title.format(**video_item).split()}.txt',"w"))
