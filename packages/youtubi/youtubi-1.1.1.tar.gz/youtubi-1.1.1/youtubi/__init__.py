...
class Youtubi : 
	...
	def get_channels(self,file = None , text = None ) : 
		...
		for index , itm in enumerate(playlist.findAll('ytd-playlist-panel-video-renderer' , attrs={'id' : 'playlist-items'})) : 
			...
			self.download(video_item)
		return result 
	
	def download(self,video_item, format_title = "{playlist} --- {index} -- {idx} * {title}") : 
		yt = YouTube(video_item.get('url')) 
		mp4_streams = yt.streams.first()
		mp4_streams.download(output_path="./" , filename = format_title.format(**video_item))
