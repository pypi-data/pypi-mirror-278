...
class RedisMixin(object):
    ...
    def setup_redis(self, crawler=None):
        ...
        # 使用了mq, 区分和生产队列名称
        if settings.getbool("MQ_USED", defaults.MQ_USED):
    ...
    def get_queue_size(self, redis_key):
        try:
            ...
            r = requests.get(defaults.GET_QUEUE_SIZE.format(queueName=msg_name), timeout=5)
    ...
    def spider_opened_latest_pop(self):
        ...
        if self.server.hexists(self.latest_queue, inner_ip):
            ...
            for data in eval(bytes_to_str(latest_datas)):
                if self.settings.getbool("MQ_USED", defaults.MQ_USED):
                    if self.rq_name:
