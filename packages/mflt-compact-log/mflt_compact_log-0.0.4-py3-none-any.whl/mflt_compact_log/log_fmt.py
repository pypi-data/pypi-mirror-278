...
def main(elf, log_id, json_output):
    ...
    if json_output:
        ...
        log_ids = list(mappings.keys())
        log_ids.sort()
        print(f"Found {len(log_ids)} log formats")

        if log_id is not None:
            log_ids = [n for n in log_ids if n == int(log_id)]

        for k in log_ids:
            v = mappings[k]
            pt.add_row([k, v.filename, v.line, v.n_args, v.fmt])
