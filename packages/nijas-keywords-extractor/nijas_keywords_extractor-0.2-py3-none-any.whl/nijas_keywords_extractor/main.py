...
#Function 2:
def gen_data_fields_from_any(X, keywords, stop_words):
    df = X.dropna()
    ...
    return field_lists
def gen_data_fields(X,keywords,stop_words):
    df=X.dropna()
...
#Function 3
def update_DF(fields,list_fields):
    import pandas as pd
    da=pd.DataFrame()
    i=0
    for col in fields:
...
def rename_keywords(df,col,dfmapa,col_map):
    import numpy as np
    dfmapa[col_map[0]]=dfmapa[col_map[0]].str.lower()
    d={}
    for k,x in zip(dfmapa[col_map[0]].values,dfmapa[col_map[1]].values):
        d[k]=x
...
def remove_special_characters(df,columns):
    ...
    for col in columns:
        ...
        df[col] = df[col].apply(remove_special_character)
