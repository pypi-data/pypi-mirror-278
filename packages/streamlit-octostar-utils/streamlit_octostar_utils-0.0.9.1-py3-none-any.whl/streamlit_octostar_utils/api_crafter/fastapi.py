...
class RoutesQueue(object):
    ...
    async def add_task(self, route_name, task):
        future = asyncio.Future()
        if not await self.does_route_exist(route_name):
            raise ValueError(f"Route {route_name} does not exist.")
        async with self.lock:
            self.queues[self.get_priority(route_name)][route_name].append((task, future))
        return await future
    ...
    async def worker(self, i):
        while not self.kill_event.is_set():
            task, future = await self.get_next_task()
            if task is None:
                await asyncio.sleep(0.5)
                continue
            await self.semaphore.acquire()
            task_future = asyncio.get_event_loop().run_in_executor(self.executor, task)
            try:
