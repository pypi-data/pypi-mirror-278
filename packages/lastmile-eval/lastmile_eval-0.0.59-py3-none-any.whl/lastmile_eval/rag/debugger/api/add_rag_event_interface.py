...
class AddRagEventInterface(abc.ABC):
    ...
    def add_multi_embedding_event(
        ...
        This sometimes fails with "413 Request Entity Too Large" error and fails
        to save the span. If this happens, please either reduce the size of
        embeddings object or make multiple calls to `add_multi_embedding_event`
        within separate sub-spans.

        Example:
        # Before
        tracer.add_multi_embedding_event(embeddings, span)

        # After
        for embedding in embeddings:
            with tracer.start_as_current_span(
                "sub-embedding",
                context=span.get_span_context() #Connect this span to parent span
            ) as sub_span:
                tracer.add_embedding_event(embedding, sub_span)
        ...
        embedding_vectors: list[list[float]] = [
            list(embedding.vector) for embedding in embeddings
        ]
        self.add_rag_event_for_span(
            ...
            # embedding_vectors are in list[list[float]] so we need to dump
            # to str
            output=json.dumps(embedding_vectors),
            event_data=metadata,
            should_also_save_in_span=should_also_save_in_span,
            ingestion_trace_id=ingestion_trace_id,
            rag_flow_type=rag_flow_type,
