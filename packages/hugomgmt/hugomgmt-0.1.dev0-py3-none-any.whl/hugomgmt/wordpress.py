...
@sqlite_option
@click.argument("id", type=int)
def wp_convcomment1(sqlite3_conn, wp: WP, id):
    """WP: convert single comment to isso"""
    isso = IssoComment(sqlite3_conn)
    permalink = wp.get_option("permalink_structure")
    _log.debug("permalink: %s", permalink)
    post = wp.convert_post(wp.get_post(id))
    title = post.get("post_title")
...
def wp_convpost_all(wp: WP, outdir, template):
    ...
    for p in wp.posts():
        ...
        dt = post["post_date"]
        outd = os.path.join(outdir, dt.strftime("%Y-%m"),
                            dt.strftime("%Y-%m-%d-")+str(post["ID"])+".markdown")
        os.makedirs(os.path.dirname(outd), exist_ok=True)
        with open(outd, "w") as ofp:
            ofp.write(template.render(post))
    for p in wp.pages():
        page = wp.convert_page(p)
        dt = page["post_date"]
        outd = os.path.join(outdir, "pages", page["post_name"]+".markdown")
        os.makedirs(os.path.dirname(outd), exist_ok=True)
        with open(outd, "w") as ofp:
            ofp.write(template.render(page))
...
def wp_init_hugo(wp: WP, output):
    ...
    toml.dump(hugodata, open(confpath, 'w'))
