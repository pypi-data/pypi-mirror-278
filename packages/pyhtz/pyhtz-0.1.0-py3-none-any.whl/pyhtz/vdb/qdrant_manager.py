...
class QdrantManager(QdrantClient):
    ...
    def __init__(
        ...
        self.client = QdrantClient(host, timeout=timeout)
    ...
    def check_health(self, client_path):
        response = requests.get(client_path)
    ...
    def get_existing_vdb_ids(self) -> List[str]:
        limit = max(self.client.count(collection_name=self.collection_name).count, 1)
    ...
    def upsert(self, collection_name: str, id: str, vector: list, payload: dict = None):
        ...
        response = self.client.upsert(collection_name=collection_name, points=[point])
