...
def interactive_ripple(ntx, coords, conn, depth_array, sizes):
    ...
    # TODO : CHANGE THAT TO A LOOP TO AVOID HAVING ONLY ONE OBJECT
    net_nodes = []
    ...
    # Sliders
    ax_lvl = fig.add_axes([0.25, 0.20, 0.65, 0.03])
    slider_lvl = Slider(
        ax=ax_lvl,
        label="Level  ",
        valmin=-6.,
    ...
    ax_conn = fig.add_axes([0.25, 0.17, 0.65, 0.03])
    slider_conn = Slider(
        ax=ax_conn,
        label="Connexion  ",
        valmin=-6.,
    ...
    ax_ovl = fig.add_axes([0.25, 0.14, 0.65, 0.03])
    slider_ovl = Slider(
        ax=ax_ovl,
        label="Overlay  ",
        valmin=-6.,
    ...
    ax_rep = fig.add_axes([0.25, 0.11, 0.65, 0.03])
    slider_rep = Slider(
        ax=ax_rep,
        label="Repulsion  ",
        valmin=-6.,
    ...
    ax_nit = fig.add_axes([0.25, 0.08, 0.65, 0.03])
    slider_nit = Slider(
        ax=ax_nit,
        label="Iterations  ",
        valmin=1.,
    ...
    ax_gvt = fig.add_axes([0.25, 0.05, 0.65, 0.03])
    slider_gvt = Slider(
        ax=ax_gvt,
        label="Frontier  ",
        valmin=-6.,
