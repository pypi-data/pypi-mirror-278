...
def getFileInfo(fileName,reName=False, createMd5=True, part=0, partSize=50):
    '''
        返回一个文件信息对象 2023.10
        ...
        * 参数 partSize = 50 (M为单位) = 52428800 = 50 * 1024 * 1024 #文件块大小默认是 50M  2024.1
    ...
    def caleFilePartInfo(filePart,fileSize,part_size):
            '''
                filePart 必须大于0  # 分块发送,则从 1开始 (2024.1)
                part_size 分包的大小
            '''      
            import math
            partSizeTemp = part_size * 1024 * 1024
            ...
            if filePart>filePartCount:
                print('caleFilePartParam()错误: 指定文件块索引大于文件的分块数!')
                return None,None

            # 计算出需要发送的文件块字节数 (最后一块和整个文件小于50M的,sendFilePartSize可能都不足 50M)
            if filePart == filePartCount:
                filePartSize = fileSize - (partSizeTemp * (filePart-1))
            else:
                filePartSize = partSizeTemp

            return filePartSize,startPos
    ...
    if os.path.exists(fileName):
        ...
        if part>0: # 如果指定分包的索引,则添加以下三个'part','partFileSize','partFilePos'信息到fileInfo对象中 2024.1
            partFileSize,partStartPos = caleFilePartInfo(part,fileInfo['size'],partSize)
            fileInfo['part'] = part
