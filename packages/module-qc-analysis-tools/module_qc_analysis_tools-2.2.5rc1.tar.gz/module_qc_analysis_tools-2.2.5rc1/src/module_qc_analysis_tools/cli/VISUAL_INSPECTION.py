...
def main(
    ...
    for filename in sorted(allinputs):
        ...
        for j, inputDF in zip(jsonData, inputDFs):
            ...
            is_bare = module_name.startswith("20UPGB")
            ...
            # Empty observation field TODO see if it is possible to have useful observations
            ## temporary fix: observation field currently in the PDB for module but not for bare module
            if not is_bare:
                qc["OBSERVATION"] = "-"
