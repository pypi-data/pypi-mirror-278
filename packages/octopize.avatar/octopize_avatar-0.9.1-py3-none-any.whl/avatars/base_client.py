    ...
    def send_request(self) -> Response:
        ...
        while True:
            try:
                ...
                error_messages = {
                    "EOF occurred in violation of protocol": f"Got EOF error on {self.data.url}.",
                    "Name or service not known": f"Got DNS resolution error on {self.data.base_url}.",  # noqa: E501
                }
                ...
                for error, message in error_messages.items():
                    ...
                    # Is only raised if no error message matched
                    raise e
