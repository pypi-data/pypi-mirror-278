...
from gautomator.decorators import deprecated
...
class PathUtil:
    ...
    @staticmethod
    def __convert_to_window_format(path) -> str:
        return path.replace('/', '\\') if platform.system() not in ('Darwin', 'Linux') else path
        
    @deprecated
    @staticmethod
    def join_prj_root_path(path: str) -> str:
        return os.path.join(PathUtil.get_prj_root_path(), PathUtil.__convert_to_window_format(path))
    ...
    def get_full_prj_path() -> str:
        ...
        Returns:
            str: path
        """
        return os.path.dirname(Path(__file__).parent.parent.parent.parent.parent.parent.parent)
    
    @staticmethod
    def join_full_prj_root_path(path: str) -> str:
        return os.path.join(PathUtil.get_full_prj_path(), PathUtil.__convert_to_window_format(path))
