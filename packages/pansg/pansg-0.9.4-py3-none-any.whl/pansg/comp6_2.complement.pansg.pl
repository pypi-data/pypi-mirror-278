###################################################################
# File Name: comp6_2.complement.pansg.pl
...
chomp $head;##read head title of Format.pansg_add.dup.txt
my @sp=split/\t/,$head;
for (my $cc=4;$cc<=$#sp;$cc++){
	my $name=(split/_/,$sp[$cc])[0];
...
sub extract_max_element {  
    ...
    foreach my $element (@$arr) {  
        my $value = $len_hash->{$element} // 0; #The default value is 0 in case the element is not in the hash
...
sub syngene {
	...
	}else{##sub gene is miniprot pos B73-chr1:283353374..283356030
