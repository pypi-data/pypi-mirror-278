...
class Bd:
    ...
    def mouse_drag(self,x,y,x2,y2,delay=0.7):
        ...
        # 縮放坐標
        x = int(self.x / self.display_zoom)
        y = int(self.y / self.display_zoom)
        x2 = int(self.x2 / self.display_zoom)
        y2 = int(self.y2 / self.display_zoom)
        ...
        # 模擬滑鼠移動到 (x2, y2) 座標
        win32api.PostMessage(self.hwnd, win32con.WM_MOUSEMOVE, win32con.MK_LBUTTON, y2 << 16 | x2)
        
        time.sleep(delay / 2)
        
        # 在 (x2, y2) 座標釋放滑鼠按鈕
        win32api.PostMessage(self.hwnd, win32con.WM_LBUTTONUP, 0, y2 << 16 | x2)
