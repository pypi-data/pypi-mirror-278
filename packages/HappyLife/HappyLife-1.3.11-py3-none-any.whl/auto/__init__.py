...
class Bd:
    ...
    def mouse_drag(x1, y1, x2, y2, duration=0.7):
        """
        在 (x1, y1) 坐標按下鼠標左鍵，移動到 (x2, y2) 坐標，再放開鼠標左鍵。
        
        :param x1: 起始坐標 x
        :param y1: 起始坐標 y
        :param x2: 目標坐標 x
        :param y2: 目標坐標 y
        :param duration: 移動持續時間，默認 0.1 秒
        """
        ...
        # 將起始坐標和目標坐標轉換為0到65535的範圍
        x1_normalized, y1_normalized = int(x1 * 65535 / screen_width), int(y1 * 65535 / screen_height)
        x2_normalized, y2_normalized = int(x2 * 65535 / screen_width), int(y2 * 65535 / screen_height)
        
        # 按下鼠標左鍵
        ctypes.windll.user32.mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE | MOUSEEVENTF_LEFTDOWN, x1_normalized, y1_normalized, 0, 0)
        
        # 等待一段時間，模擬拖動效果
        time.sleep(duration)
        
        # 移動鼠標到目標位置
        ctypes.windll.user32.mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, x2_normalized, y2_normalized, 0, 0)
        
        # 放開鼠標左鍵
        ctypes.windll.user32.mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE | MOUSEEVENTF_LEFTUP, x2_normalized, y2_normalized, 0, 0)
...
class Fd:
    ...
    def mouse_drag(self,x,y,x2,y2,delay=0.1):
        '''
