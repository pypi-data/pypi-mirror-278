...
def are_credentials_valid(credentials):
    sts = boto3.client('sts', aws_access_key_id=credentials.access_key, 
...
def get_bucket_name():
    ...
    for bucket in buckets:
        if bucket['Name'].startswith('tensorkube-build-bucket-'):
            return bucket['Name']
    else:
        bucket_name = f"tensorkube-build-bucket-{uuid.uuid4()}"
        return bucket_name
