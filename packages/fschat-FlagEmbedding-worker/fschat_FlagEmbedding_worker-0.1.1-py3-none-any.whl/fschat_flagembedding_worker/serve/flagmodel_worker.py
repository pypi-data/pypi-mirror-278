"""
An Embedding Only Model Worker.
"""
...
class FlagModelWorker(BaseModelWorker):
    def __init__(
        self,
        controller_addr: str,
        worker_addr: str,
        ...
        **kwargs,
        ...
        logger.info(
            f"Loading the FlagModel {self.model_names} on worker {worker_id} ..."
        )
    ...
    # 当用户把本模型当成对话模型使用时直接生成异常
    def generate_stream_gate(self, params):
        ret = {
            "text": "Text-generation is NOT supported by Embedding Only Model.",
    ...
    # 处理 encoding_format=base64 时的情况
    # openai默认要求使用 base64 编码
    def __encode_base64(self, embeddings):
        result = []
        for e in embeddings:
            e = e.astype(numpy.float32)  # openai using dtype=float32
            result.append(base64.b64encode(e.tobytes()).decode("utf-8"))
        return result
    ...
    def get_embeddings(self, params):
        ...
        try:
            if self.debug:
                logger.info(
                    "Calling get_embeddings with params=%s",
                    params,
                )
            inputs = params["input"]
            ret = {
            ...
            normalized_embeddings = self.model.encode(
                inputs,
            ...
            if self.debug:
                logger.info("FlagModel encode returns: %s", normalized_embeddings)
...
def create_model_worker():
    ...
    parser.add_argument(
        ...
        help="Query instruction for retrieval.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
