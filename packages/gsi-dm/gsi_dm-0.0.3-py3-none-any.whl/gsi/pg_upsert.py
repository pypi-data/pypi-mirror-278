...
description_short = "Update and insert (upsert) data from staging tables to base tables."
...
class PostgresDB:
    ...
    def __repr__(self: PostgresDB) -> str:
        return f"{self.__class__.__name__}(host={self.host}, database={self.database}, user={self.user})"
...
class CompareUI:
    def __init__(
        ...
        if m is not None:
            ...
            xpos = (swd / 2) - (wwd / 2)
            ypos = (sht / 2) - (what / 2)
            self.win.geometry("%dx%d+%d+%d" % (wwd, what, xpos, ypos))
        # Limit resizing
    ...
    if db.execute(sql).rowcount > 0:
        errors.append(
            "Invalid table(s) specified: {}".format(
                db.dataframe(SQL("select schema_table from ups_invl_table"))["schema_table"][0],
            ),
        )
...
def validate_control(base_schema: str, stg_schema: str, control_table: str):
    ...
    if db.execute(sql).rowcount > 0:
        error_handler(
            [
                "Invalid table(s) specified: {}".format(
                    db.dataframe("select schema_table from ups_ctrl_invl_table")["schema_table"][0],
                ),
            ],
        )
...
def qa_all(base_schema: str, stg_schema: str, control_table: str):
    ...
    interactive = db.dataframe("select interactive from ups_toprocess;")["interactive"][0]
    # Null checks
    logger.info("")
    qa_check = "Non-NULL"
...
def null_qa_one(
    ...
    while True:
        ...
        if not null_df.is_empty():
            logger.warning(
                f"    Column {df['column_name'][0]} has {null_df['nrows'][0]} null values",
            )
            db.execute(
                SQL(
                    """
...
def qa_all_pkloop(
    ...
    while True:
        ...
        db.execute(
            SQL(
                """update ups_proctables set processed = True where table_name = {table_name};""",
            ).format(table_name=Literal(df["table_name"][0])),
        )
    ...
    if upsert_method in ("upsert", "insert"):
        ...
        if new_rowcount > 0:
            if interactive:
                btn, return_value = TableUI(
                    "New Data",
                    f"Do you want to add these new data to the {base_schema}.{table} table?",
                    [
                        ("Continue", 0, "<Return>"),
                        ("Skip", 1, "<Escape>"),
                        ("Cancel", 2, "<Escape>"),
...
def elapsed_time(start_time: datetime):
...
def upsert(
    ...
    **kwargs,
    ...
    logger.debug(f"Upsert completed in {elapsed_time(timer)}")
