...
"""
This module provides authentication functions for FOS services.
"""
...
def _get_canonical_headers(headers, headers_to_sign=None):
    ...
    if headers_to_sign is None or not headers_to_sign:
...
def sign(credentials,
    ...
    _logger.debug('Sign params: %s %s %s %s %d %d %s' % (http_method,
                                                         path,
                                                         headers,
                                                         params,
                                                         timestamp,
    ...
    # if headers_to_sign is None, then sign the host only by default
    if not headers_to_sign:
        headers_to_sign = {b"host"}
    headers_to_sign = set(
        [k.strip().lower() for k in headers_to_sign if not k.strip().lower().startswith(b"x-fos-")]
    )
    ...
    _logger.debug('sign_key=[%s] sign_string=[%d bytes][ %s ]' % (sign_key, len(string_to_sign), string_to_sign))
