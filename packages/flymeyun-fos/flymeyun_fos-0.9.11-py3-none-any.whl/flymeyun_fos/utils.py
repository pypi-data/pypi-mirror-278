...
"""
This module provide some tools for FOS client.
"""
...
def parse_host_port(endpoint, default_protocol):
    ...
    :type: flymeyun_fos.protocol.HTTP or flymeyun_fos.protocol.HTTPS
    ...
    if parse_result.scheme == compat.convert_to_bytes(flymeyun_fos.protocol.HTTP.name):
        protocol = flymeyun_fos.protocol.HTTP
        port = flymeyun_fos.protocol.HTTP.default_port
    elif parse_result.scheme == compat.convert_to_bytes(flymeyun_fos.protocol.HTTPS.name):
        protocol = flymeyun_fos.protocol.HTTPS
        port = flymeyun_fos.protocol.HTTPS.default_port
    else:
...
def is_custom_host(host, bucket_name):
    ...
    # split http head
    return host.lower().startswith(
        compat.convert_to_bytes(bucket_name.lower())
    ) and len(host_split) == 4 and is_fos_suffixed_host(host)


def is_fos_suffixed_host(host):
    """
    :param host: fos endpoint
    :return: fos endpoint or not
    """
    if host is None:
        return False
    if host.endswith(b'/'):
    ...
    return check_host.lower().endswith(DEFAULT_FOS_DOMAIN_SUFFIX)
