...
"""
This module provides general http handler functions for processing http responses
from flymeyun_fos services.
"""
...
def parse_copy_object_response(http_response, response):
    ...
    if headers_list.get(TRANSFER_ENCODING, b'not exist') == b'chunked':
        ...
        if body:
            ...
            if b'code' in d:
                http_response.close()
                raise FosServerError(d[b'message'], code=d[b'code'], request_id=d[b'requestId'])
            else:
                response.__dict__.update(
                    json.loads(body, object_hook=utils.dict_to_python_object).__dict__)
                http_response.close()
        else:
            e = FosServerError(http_response.reason, request_id=response.metadata.fos_request_id)
            http_response.close()
            raise e
        return True
    else:
