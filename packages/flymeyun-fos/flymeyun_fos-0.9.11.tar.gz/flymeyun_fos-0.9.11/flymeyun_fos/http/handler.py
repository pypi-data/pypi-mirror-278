...
def parse_error(http_response, response):
    ...
    :raise flymeyun_fos.exception.FosClientError: if http status code is NOT 2xx
    """
    if http_response.status // 100 == http.client.OK // 100:
        return False
    if http_response.status // 100 == http.client.CONTINUE // 100:
        raise FosClientError(b'Can not handle 1xx http status code')
    bse = None
    body = http_response.read()
    if body:
        d = json.loads(compat.convert_to_string(body))
        bse = FosServerError(d['message'], code=d['code'], request_id=d['requestId'])
    if bse is None:
        bse = FosServerError(http_response.reason, request_id=response.metadata.fos_request_id)
