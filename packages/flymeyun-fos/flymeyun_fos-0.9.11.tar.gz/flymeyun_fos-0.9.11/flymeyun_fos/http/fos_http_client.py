...
"""
This module provide http request function for FOS services.
"""
...
from flymeyun_fos.fos_response import FosResponse
...
def _get_connection(protocol, host, port, connection_timeout_in_millis, proxy_host=None, proxy_port=None):
    """
    :param protocol
    :type protocol: flymeyun_fos.protocol.Protocol
    :param endpoint
    :type endpoint: str
    ...
    if protocol.name == flymeyun_fos.protocol.HTTP.name:
    ...
    elif protocol.name == flymeyun_fos.protocol.HTTPS.name:
        if proxy_host and proxy_port:
...
def check_headers(headers):
    ...
    for k, v in iteritems(headers):
        if isinstance(v, (bytes, str)) and \
            ...
            raise FosClientError(r'There should not be any "\n" in header[%s]:%s' % (k, v))
...
def send_request(config,
    ...
    """
    Send request to FOS services.

    :param config
    :type config: flymeyun_fos.FosClientConfiguration
    ...
    :param request:
    :type request: flymeyun_fos.internal.InternalRequest

    :return:
    :rtype: flymeyun_fos.FosResponse
    """
    _logger.debug(b'%s request start: %s %s, %s, %s', http_method, path, headers, params, body)
    headers = headers or {}

    user_agent = 'flymeyun-fos-sdk/%s/%s/%s' % (compat.convert_to_string(flymeyun_fos.SDK_VERSION),
                                                sys.version,
                                                sys.platform)
    ...
    if http_headers.FOS_DATE not in headers:
    ...
    if isinstance(body, str):
        body = body.encode(flymeyun_fos.DEFAULT_ENCODING)
    if not body:
    ...
    while True:
        ...
        try:
            ...
            if should_get_new_date is True:
                headers[http_headers.FOS_DATE] = utils.get_canonical_time()
            ...
            http_response = _send_http_request(conn,
                                               http_method,
                                               uri,
                                               headers,
                                               body,
            ...
            if isinstance(e, FosServerError):
            ...
            if config.retry_policy.should_retry(e, retries_attempted):
                ...
                raise FosHttpClientError('Unable to execute HTTP request. Retried %d times. '
                                         'All trace backs:\n%s' % (retries_attempted,
                                                                   '\n'.join(errors)),
                                         e,
                                         status_code,
