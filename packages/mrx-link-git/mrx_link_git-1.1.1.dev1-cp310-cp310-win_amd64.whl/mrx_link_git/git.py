...
class Git:
    ...
    async def get_remote_commit_ids(self, branch, path):
        """
        Execute git log origin/{branch name} --format='%H' command & return the commid IDs.
        """
        cmd = ["git", "log", "origin/" + branch, "--format='%H'"]
        code, output, error = await execute(
            cmd,
            cwd=path,
        )
