...
class PathExpander:
    ...
    def _prepare_path_list(self, source) -> [Path]:
        result = []
        if not isinstance(source, Iterable):
            result.append(Path(str(source)))
        elif self.settings.expand_glob:
            for x in source:
                result += [Path(str(a)) for a in glob.glob(str(x))]
        else:
            result += [Path(str(a)) for a in source]
        return sorted(result, key=lambda a: a.absolute())
    ...
    def __init__(self, sources: Any,
        ...
        if expand_glob is not None:
            self.settings.expand_glob = expand_glob
        ...
        self._sources = self._prepare_path_list(sources)
