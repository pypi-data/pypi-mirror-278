...
class sqlResult_to_Excel:
    ...
    def add_sheet(self,target_wb='',need_add_workbook='',need_sheet_name='',new_sheet_name='new sheet'):
        ...
        try:
            ...
            tmp_path = need_add_workbook.split('.')[0]+'_tmp.xlsx'
            self._solve_out_of_index_error(path=need_add_workbook,dis=tmp_path)
    ...
    # 加载 Excel 文件
        try:
            ...
            workbook = self._del_the_specific_sheet_stream(workbook,del_str='Eva')
            self._delete_file(tmp_path)
