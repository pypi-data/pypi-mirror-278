...
{ time "$TEST_SUITE_DIR/testu" "${BINARY_FILE}" "${COUNT_BITS}" big > "${RESULT_DIR}/test_bigcrush.log"; } 2> "$RESULT_DIR/time_bigcrush"
echo -e "\nBigCrush" >> "$RESULT_DIR/time"
cat "$RESULT_DIR/time_bigcrush" >> "$RESULT_DIR/time"
rm "$RESULT_DIR/time_bigcrush"

echo "TestU01 BigCrush test completed."
