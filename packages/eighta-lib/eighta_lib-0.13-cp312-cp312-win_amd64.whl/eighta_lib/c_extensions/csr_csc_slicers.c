...
void print_int64_array(const char* name, int64_t* array, int size) {
    printf("%s: [", name);
        ...
        if (data_start_idx < data_end_idx) {
            ...
            for (hsize_t j = 0; j < row_size; j++) {
                if (npy_index_dtype == NPY_INT32) {
                    if (((int*)indices_slice)[j] <= end_col_index && col_map[((int*)indices_slice)[j]] >= 0) {
                        valid_count++;
                    }
                } else if (npy_index_dtype == NPY_INT64) {
                    if (((int64_t*)indices_slice)[j] <= end_col_index && col_map[((int64_t*)indices_slice)[j]] >= 0) {
                        valid_count++;
                    }
                }
            }
...
CSCMatrix* read_process_csc_matrix(const char* file_path, const char* group_name, int64_t* row_indices, int n_rows, int64_t* col_indices, int n_cols) {
    ...
    // Use the first and last elements to determine the range
    int64_t start_row_index = col_indices[0];
    int64_t end_row_index = col_indices[n_cols - 1];
    ...
    // Map row indices
    for (int i = 0; i < n_cols; i++) {
        row_map[col_indices[i] - start_row_index] = i;
    }
    ...
    for (int i = 0; i < n_cols; i++) {
        ...
        if (data_start_idx < data_end_idx) {
            ...
            for (hsize_t j = 0; j < col_size; j++) {
                if (npy_index_dtype == NPY_INT32) {
                    if (((int*)indices_slice)[j] <= end_row_index && row_map[((int*)indices_slice)[j]] >= 0) {
                        valid_count++;
                    }
                } else if (npy_index_dtype == NPY_INT64) {
                    if (((int64_t*)indices_slice)[j] <= end_row_index && row_map[((int64_t*)indices_slice)[j]] >= 0) {
                        valid_count++;
                    }
                }
            }
