...
def write_process_csr_matrix(source_group, dest_file_path, group_path, row_indices, col_indices, batch_size):
    with write_lock:
...
def write_process_csc_matrix(source_group, dest_file_path, group_path, row_indices, col_indices, batch_size):
    with write_lock:
        with h5py.File(dest_file_path, 'a') as f_dest:
            dest_group = f_dest.require_group(group_path)
            data_group = dest_group.create_dataset(
...
def write_process_matrix(source_group, dest_file_path, group_path, row_indices, col_indices, batch_size, is_csr):
    if is_csr:
        with write_lock:
            with h5py.File(dest_file_path, 'a') as f_dest:
                dest_group = f_dest.require_group(group_path)
        ...
        slicers_write.write_process_csr_matrix(source_group.file.filename, dest_file_path, group_path, row_indices, col_indices, batch_size)
    else:
        with write_lock:
            with h5py.File(dest_file_path, 'a') as f_dest:
                dest_group = f_dest.require_group(group_path)
        ...
        slicers_write.write_process_csc_matrix(source_group.file.filename, dest_file_path, group_path, row_indices, col_indices, batch_size)
    with write_lock:
        with h5py.File(dest_file_path, 'a') as f_dest:
            dest_group = f_dest.require_group(group_path)
            copy_attrs(source_group, dest_group, shape=(len(row_indices), len(col_indices)))
    # if is_csr:
    #     write_process_csr_matrix(source_group, dest_file_path, group_path, row_indices, col_indices, batch_size)
    # else:
    #     write_process_csc_matrix(source_group, dest_file_path, group_path, row_indices, col_indices, batch_size)


def write_process_categorical_group(source_group, dest_file_path, group_path, row_indices, col_indices):
    categories = source_group['categories'][:]
...
def write_process_raw_group(source_group, dest_file_path, group_path, row_indices, batch_size):
    if 'X' in source_group.name:
        parent_encoding_type = source_group.attrs.get('encoding-type', None)
        is_csr = parent_encoding_type != "csc_matrix"
...
def write_process_obsp_group(source_group, dest_file_path, group_path, row_indices, batch_size):
    for key in source_group.keys():
        item = source_group[key]
        if isinstance(item, h5py.Group):
            new_group_path = os.path.join(group_path, key)
            with write_lock:
                with h5py.File(dest_file_path, 'a') as f_dest:
                    dest_group = f_dest.require_group(new_group_path)
                    copy_attrs(item, dest_group, shape=(len(row_indices), len(row_indices)))
            parent_encoding_type = item.attrs.get('encoding-type', None)
            is_csr = parent_encoding_type != "csc_matrix"
            write_process_matrix(item, dest_file_path, new_group_path, row_indices, row_indices, batch_size, is_csr)
        elif isinstance(item, h5py.Dataset):
            data = item[row_indices, :][:, row_indices]
            with write_lock:
                with h5py.File(dest_file_path, 'a') as f_dest:
...
def write_process_varp_group(source_group, dest_file_path, group_path, col_indices, batch_size):
    for key in source_group.keys():
        item = source_group[key]
        if isinstance(item, h5py.Group):
            new_group_path = os.path.join(group_path, key)
            with write_lock:
                with h5py.File(dest_file_path, 'a') as f_dest:
                    dest_group = f_dest.require_group(new_group_path)
                    copy_attrs(item, dest_group, shape=(len(col_indices), len(col_indices)))
            parent_encoding_type = item.attrs.get('encoding-type', None)
            is_csr = parent_encoding_type != "csc_matrix"
            write_process_matrix(item, dest_file_path, new_group_path, col_indices, col_indices, batch_size, is_csr)
        elif isinstance(item, h5py.Dataset):
            data = item[col_indices, :][:, col_indices]
            with write_lock:
                with h5py.File(dest_file_path, 'a') as f_dest:
...
def write_process_group(source_group, dest_file_path, group_path, row_indices, col_indices, batch_size = 1000):
    parent_encoding_type = source_group.attrs.get('encoding-type', None)
    if parent_encoding_type == 'csr_matrix':
        write_process_matrix(
            source_group,
        ...
        write_process_categorical_group(source_group, dest_file_path, group_path, row_indices, col_indices)
    elif 'obsp' in source_group.name:
        write_process_obsp_group(source_group, dest_file_path, group_path, row_indices, batch_size)
    elif 'varp' in source_group.name:
        write_process_varp_group(source_group, dest_file_path, group_path, col_indices, batch_size)
    else:
        for key in source_group.keys():
            item = source_group[key]
            ...
            if isinstance(item, h5py.Dataset):
                ...
                if source_group.name == '/raw':
                    write_process_raw_group(item, dest_file_path, new_group_path, row_indices, batch_size)
                else:
                    write_process_group(item, dest_file_path, new_group_path, row_indices, col_indices, batch_size)
                ...
                if key == 'uns':
                    ...
                    with write_lock:
                        with h5py.File(dest_file_path, 'a') as f_dest:
                            ...
                            copy_attrs(item, dest_group)
                    write_process_group(item, dest_file_path, key, row_indices, col_indices, batch_size)
            elif isinstance(item, h5py.Dataset):
