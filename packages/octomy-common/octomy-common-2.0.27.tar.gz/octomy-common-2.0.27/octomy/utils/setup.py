...
def get_package_data(extensions:list, modules:list, package_dir:str):
	data_filters = list()
...
def megasetup(source_data:dict):
	...
	python_version = "3.9"
	data_extensions = ['sql']
	modules = group_base_name and [group_base_name] or []
	...
	if has_cli:
		...
		executable = f"{executable_name} = {base_name}:main"
		if executable_name:
			scripts = [f"{executable_name}.py"]
	...
	defaults_data = {
		...
		, "package_data": get_package_data(data_extensions, modules, package_dir)
