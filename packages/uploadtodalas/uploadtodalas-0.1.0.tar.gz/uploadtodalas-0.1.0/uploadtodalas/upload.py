...
class UploadToDalas():
    ...
    async def test_site(self):
        ...
        """    
        try:
            logger.info(f"Тестируем доступность эндпоинта {self.upload_url}")
            async with aiohttp.ClientSession() as session:
                    
                    async with session.get(self.upload_url+"/docs") as response:
                        ...
                        logger.info(f"Поситите {self.upload_url+'/docs'}, для прочтения документации.")
                        return True
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                while True:
                    ...
                    url = f"{self.upload_url}/upload/{seller_id}/{shedul_in_hours}"
