...
class Settings:
    ...
    def use_processor(self, processor):
        name = _name(processor)
        if self.mode == "evict":
            for reason, processor_names in self.evict.items():
                if name in processor_names:
                    return False, f"{name!r} is evicted due to {reason}"
            return True, f"{name!r} is kept because no rule evicted it"
        elif self.mode == "keep":
            for reason, processor_names in self.keep.items():
                if name in processor_names:
                    return True, f"{name!r} is kept due to {reason}"
            return False, f"{name!r} is evicted because no rule keeps it"
...
def log(
    ...
    new_processors += [p for p in orig_processors if settings.use_processor(p)[0]]
...
def pytest_report_collectionfinish(config: pytest.Config) -> list[str]:
    ...
    if verbosity > 0:
        lines = [f"Pytest-structlog plugin is operating in {settings.mode!r} mode."]
        if verbosity > 1:
            proclist = sorted(set().union(*getattr(settings, settings.mode).values()))
            lines += [f"- {settings.mode} {name!r}" for name in proclist]
        for processor in structlog.get_config().get("processors", []):
            keep, reason = settings.use_processor(processor)
            line = "Structlog processor " + reason
            if verbosity > 1:
                line += f" ({processor!r})"
            lines.append(line + ".")
        return lines
