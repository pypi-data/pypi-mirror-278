...
def _fetch_metadata(file_path: str, gl: gitlab.Gitlab, project_id: int, branch: str) -> dict:
    return gl.http_head(f'/projects/{project_id}/repository/files/{EncodedId(file_path)}', query_parameters={'ref': branch})
...
class GitlabAccess(BaseSync):
    ...
    def _get_files(self, with_metadata=True):
        ...
        if with_metadata:
            ...
            all_files = [FileInformation(path=x['path'],
                                        size=self._get_file_size(x),
                                        last_modified=self._get_file_lastmod(x),
                                        hashes={'sha256': self._get_file_sha256(x)}) for x in all_files]
        else:
    ...
    def _get_files_local(self):
        ...
        for cur_path in self.gitlab_project.paths:
            files_generator = self.project.repository_tree(path=cur_path,
                                                           recursive=True,
                                                           iterator=True, ref=self.gitlab_project.branch)
            file_list = [x for x in files_generator if x['type'] == 'blob' and any(fnmatch(x['path'], glob) for glob in self.gitlab_project.globs)]
    ...
    def _fetch_metadata(self, file_paths: list[str]) -> None:
        d_fetch = lambda f: _fetch_metadata(f['path'], self.gl, self.project.id, self.gitlab_project.branch)
