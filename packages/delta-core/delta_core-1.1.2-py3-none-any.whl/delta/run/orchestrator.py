...
from delta.run.job import Image, Job, PathMapper
from delta.run.job.builder.python_builder import PythonRunner
from delta.run.job import JobService
...
class Edge:
    ...
    to_id: int
    to_port: str
    value: DataParameterModel | PrimitiveParameterModel = None
    ...
    def check_inputs(self):
        ...
        for k, v in self._manifest.inputs.items():
            ...
            if k in user_inputs:
                ...
                elif v.type != "Data" and v.type != retrieve_primitive_type(
                    user_inputs[k].value
                ):
                    self.run_ctx.return_code = 110
                    self.run_ctx.message = (
