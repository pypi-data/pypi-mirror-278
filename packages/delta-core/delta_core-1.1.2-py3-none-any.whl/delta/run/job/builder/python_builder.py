...
class PythonRunner(JobBuilder):
    ...
    def __build_run_command(
        ...
        if apt_requirements:
            segment = " ".join(
                map(lambda x: f"{x.name}={x.version}", apt_requirements)
            )
            run.append("apt-get update")
            run.append(f"apt-get install --no-install-recommends -y {segment}")
            run.append(f"apt-get clean")
            run.append(f"rm -rf /var/apt/lists/*")

        if pip_requirement_files:
            segment = "pip install --no-cache-dir"
            for requirement_file in pip_requirement_files:
                segment = f"{segment} -r /root/{requirement_file}"
            run.append(segment)

        if pip_requirements:
            segment = " ".join(
                map(lambda x: f"{x.name}=={x.version}", pip_requirements)
            )
    ...
    def generate_dockerfile(self) -> str:
        ...
        with io.StringIO() as buf:
            buf.write(f"FROM python:{params['pythonVersion']}-slim\n")
            ...
            buf.write(f"COPY . /delta\n")
            buf.write(f"WORKDIR /delta\n")
            return buf.getvalue()
    ...
    def build_command(self, **kwargs) -> str:
        ...
        for cmd_segment in cmd_pattern.split(" "):
            ...
            if m:
                ...
                evaluated_cmd.append(placeholder.evaluate())
            else:
                evaluated_cmd.append(cmd_segment)
        return " ".join(evaluated_cmd)
