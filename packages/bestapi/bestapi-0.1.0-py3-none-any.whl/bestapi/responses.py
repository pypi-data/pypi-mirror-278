...
from bestapi.concurrency import iterate_in_threadpool
from bestapi.types import Receive, Scope, Send
...
class RedirectResponse(Response):
    def __init__(self, url: str, status_code: int = 307, headers: typing.Mapping[str, str] | None = None) -> None:
...
class FileResponse(Response):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        if self.stat_result is None:
            try:
                stat_result = await asyncio.to_thread(os.stat, self.path)
                self.set_stat_headers(stat_result)
