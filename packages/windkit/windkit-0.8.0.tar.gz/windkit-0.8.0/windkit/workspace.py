...
class Workspace:
    ...
    def get_owc(self, ID, crs=None):
        ...
        if self.mast_coords != None:
            ...
            return add_crs(bwc, self.crs)

        bwc = add_crs(bwc, crs)
        ds = update_var_attrs(bwc, _BWC_ATTRS)
        return update_history(ds)

    def get_gwc(
        self, ID, crs=None, west_east=None, south_north=None, height=None, header=None
    ):
        """
        ...
        if west_east is not None and south_north is not None:
            ...
            if header is None:
                header = self.mast_coords["header"]
        ...
        gwc = add_crs(gwc, crs)
        gwc = update_var_attrs(gwc, _WEIB_ATTRS)
        return update_history(gwc)

    def get_vector_map(self, ID, map_type="elevation", crs=None):
        """
        ...
        """
        if map_type not in ["elevation", "roughness"]:
            raise ValueError("""map_type must be either 'elevation' or 'roughness'""")
        if ID not in self.catalogue:
            raise ValueError("ID does not exist in object catalogue")
        if self.catalogue[ID]["ObjectDescription"] != "Vector map":
            raise ValueError("ID is not linked to vector map")
        if crs == None and self._map_files[ID][map_type]["crs"] == None:
            raise ValueError(
        ...
        vector_map = read_vector_map(path_tmp.joinpath(filename), crs, map_type)
        tempfolder.cleanup()
    ...
    def get_object(self, ID, map_type="elevation", crs=None):
        ...
        if ID not in self.catalogue:
            raise ValueError("ID does not exist in object catalogue")
        if self.catalogue[ID]["ObjectDescription"] == "Vector map":
            return self.get_vector_map(ID, map_type=map_type, crs=crs)
        if self.catalogue[ID]["ObjectDescription"] == "Generalised wind climate":
