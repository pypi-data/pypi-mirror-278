...
BASEPATH_LAND_COVER_TABLES = Path(__file__).parent / "landcovertables"

LAND_COVER_TABLES = {
    "CGLS-LC100": {
        "default": BASEPATH_LAND_COVER_TABLES / "CGLS-LC100.json",
    },
    "CORINE": {
        "default": BASEPATH_LAND_COVER_TABLES / "CORINE.json",
    },
    "GlobCover": {
        "default": BASEPATH_LAND_COVER_TABLES / "GlobCover.json",
    },
    "MODIS": {
        "default": BASEPATH_LAND_COVER_TABLES / "MODIS.json",
    },
    "WorldCover": {
        "default": BASEPATH_LAND_COVER_TABLES / "WorldCover.json",
    },
    "ESA_CCI": {
        "default": BASEPATH_LAND_COVER_TABLES / "ESA_CCI.json",
    },
}
...
class LandCoverTable(dict):
    ...
    def get_table(cls, dataset, table="default"):
        ...
        table : str
            Name of landcover table to get.
        ...
        """
        if dataset not in LAND_COVER_TABLES:
            raise ValueError(
                "Dataset {} not available. Available datasets are {}".format(
                    dataset, list(LAND_COVER_TABLES.keys())
                )
            )
        if table not in LAND_COVER_TABLES[dataset]:
            raise ValueError(
                "Table {} not available for dataset {}. Available tables are {}".format(
                    table, dataset, list(LAND_COVER_TABLES[dataset].keys())
                )
            )
        return cls.read_json(LAND_COVER_TABLES[dataset][table])
...
def get_land_cover_table(dataset, table="default"):
    """Get landcover table from dataset and table name.
    ...
    Examples
    --------
    >>> from windkit import get_land_cover_table
    >>> lct = get_land_cover_table("CGLS-LC100")
    >>> lct
    id = LandCoverID
    z0 = Roughness length (m)
    d = Displacement height (m)
