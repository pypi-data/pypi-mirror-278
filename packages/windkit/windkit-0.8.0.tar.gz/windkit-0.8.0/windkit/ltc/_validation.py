    ...
    def _validate(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "self" in inspect.getcallargs(func, *args):
                args_idx = [1, 2][:num_args]
            else:
                args_idx = [0, 1][:num_args]
            for i in args_idx:
...
def validate_dataset_ltc(ds_ts):
    ...
    Raises WindClimateValidationError if any error is found, otherwise
    does nothing
    """
    errors = []
...
def assert_timesteps_equal(ds_ref, ds_tgt):
    ...
    """
    if (ds_ref.sizes["time"] != ds_tgt.sizes["time"]) or np.any(
        ds_ref.time.values != ds_tgt.time.values
    ):
        raise WindClimateValidationError(
            "datasets do not have the same time coordinates."
        )
