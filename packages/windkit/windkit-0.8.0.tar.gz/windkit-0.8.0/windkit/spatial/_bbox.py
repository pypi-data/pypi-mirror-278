...
class BBox:
    ...
    def reproject(self, to_crs, use_bounds=False):
        ...
        if use_bounds:
            ...
            warnings.warn(
                "'use_bounds' currently defaults to False in BBox.reproject, in the future this will change to True"
            )
    ...
    def buffer(self, distance, cap_style=3, join_style=2):
        ...
        cap_style: shapely.BufferCapStyle or {'round', 'square', 'flat'}
            Shape of buffered line endings, it is passed to `shapely.buffer`.
            Defaults to (3) 'square'

        join_style: shapely.BufferJoinStyle or {'round', 'mitre', 'bevel'}
            Shape of bufered line midpoints, it is passed to  `shapely.buffer`.
            Defaults  to (2) 'mitre'
