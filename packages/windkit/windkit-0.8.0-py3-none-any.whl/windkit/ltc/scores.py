...
def rmse(ds_tgt, ds_pred):
    ...
    """
    y_tgt = ds_tgt.wind_speed.values.flatten()
...
# @ltc_validate
@ltc_validate(num_args=2)
def wasserstein_distance(ds_tgt, ds_pred):
    """
...
# @ltc_validate
@ltc_validate(num_args=2)
def calc_scores(ds_tgt, ds_pred, name="name", period="full"):
    """
    ...
    vals = np.array([f(ds_tgt, ds_pred) for f in scoring])
    names = [name] * len(vals)
