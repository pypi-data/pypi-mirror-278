...
from .spatial import BBox, add_crs, clip, get_crs, reproject, warp
...
def _get_era5_from_planetary_computer(datetime, bbox=None, translate_to_windkit=True):
    ...
    dependencies = [zarr, pystac_client, planetary_computer, fsspec, adlfs]
    for dependency_name, dependency in zip(dependencies_names, dependencies):
        if dependency is None:
            raise ImportError(
                f"To use the planetary computer source, you must install the '{dependency_name}' package."
            )
