...
from .constants import OPERATION_MODE_HUMAN, OPERATION_TYPE_ANNOTATION, OPERATION_MODE_AUTO, \
    META_UPDATE_REQUEST_BATCH_SIZE, AnnotationUploadType, AnnotationShapeType
...
class Annotation:
    ...
    def upload_annotation_json(
        ...
        label_references = self._client.datalake_interface.find_datalake_label_references(
            label_attribute_values_dict)
        annotation_logger.debug(
            'retrieving datalake label references finished')
        ...
        for image in annotation_data[IMAGES]:
            ...
            for annotation in image[ANNOTATIONS]:
                ...
                elif annotation_geometry == AnnotationShapeType.POLYGON_ANNOTATION.value:
                    self.format_polygon_annotation(
                ...
                try:
                            ...
                            for attr, val in annotation[METADATA].items():
                                try:
                                    ...
                                    annotation_object[LABEL][ATTRIBUTE_VALUES][attr_ref].append(
                                        {
                                            VALUE: val_ref,
                                            CONFIDENCE: 1 if operation_mode == OPERATION_MODE_HUMAN else annotation[
                                                CONFIDENCE]
                                        }
                                    )
                            ...
                            for attr, val_array in annotation[ATTRIBUTES].items():
                                try:
                                    ...
                                    for val_obj in val_array:
                                        val_ref = label_references[annotation[LABEL]
                                                                   ][TEXTS][attr][TEXTS][val_obj[VALUE]]
                    ...
                    print(
                        f"An Error Occurred at label class reference finding for object_key: {object_key}", te)
                    continue
    ...
    def remove_collection_annotations(self, collection_id, model_run_id):
        session_uuid = str(datetime.datetime.now().timestamp())
        meta_update_response = self._client.datalake_interface.remove_modelrun_collection_annotation(
