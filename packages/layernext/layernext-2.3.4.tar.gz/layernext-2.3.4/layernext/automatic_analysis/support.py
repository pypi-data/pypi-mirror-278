...
def validate_input_prompt(input_data):
    ...
    for item in input_data:
        if not isinstance(item, dict):
            raise ValueError("Each item in the list in input_prompt should be a dictionary.")
        if "label" not in item:
            raise ValueError("Missing 'label' key in one of the dictionaries in input_promt.")
        if "description" not in item:
            raise ValueError("Missing 'description' key in one of the dictionaries in input_prompt.")
...
def validate_input_annotation_type(input_string):
    ...
    if input_string not in ["bbox", "segment"]:
        raise ValueError("Input for annotation_type should be either 'bbox' or 'segment'.")
