...
class HadoopInstaller(ArchiveDownloadAndExtractInstaller):
	...
	def _replace_aws_sdk(D,target):
		A=os.path.join(D._get_install_dir(target),'share/hadoop/tools/lib');B=glob.glob(os.path.join(A,'aws-java-sdk-*.jar'))
		for E in B:rm_rf(E)
		if B:
			C=os.path.join(A,AWS_SDK_BUNDLE_JAR)
			if not file_exists_not_empty(C):download(AWS_SDK_BUNDLE_JAR_URL,C)
