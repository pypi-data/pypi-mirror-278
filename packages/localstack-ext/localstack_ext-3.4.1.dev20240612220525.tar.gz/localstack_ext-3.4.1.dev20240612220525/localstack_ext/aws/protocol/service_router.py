...
def patch_service_router():
	...
	def B(fn,host,**B):
		...
		if'.appsync-api.'in A:return ServiceModelIdentifier(C)
		return fn(A,**B)
	...
	def D(fn,request,**H):
		...
		if G(K):return ServiceModelIdentifier(E)
		if re.match('/graphql/[a-zA-Z0-9-]+',B):return ServiceModelIdentifier(C)
		L=D.get('authorization','')
		if L.startswith('Bearer '):return ServiceModelIdentifier(E)
		if'/2018-06-01/runtime'in B:return ServiceModelIdentifier('lambda')
