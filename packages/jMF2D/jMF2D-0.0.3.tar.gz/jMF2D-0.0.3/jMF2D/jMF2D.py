...
def runDeconvolution(X, Y, L2, alpha=1, beta=1, gamma=1, loss=1e-3, max_iter=500,
    ...
    while converge!=0 and iter<max_iter:
        ...
        # Update C
        Ck = C
        Crow = np.sqrt(np.sum(np.abs(np.conj(C.conj().T)) * C.conj().T))
        D1 = np.diag(1/2*Crow)
        ...
        # Update E
        Ek = E
        Erow = np.sqrt(np.sum(np.abs(np.conj(E.conj().T)) * E.conj().T))
        D2 = np.diag(1 / 2 * Erow)
        ...
        T2 = T2 + delta2 * (F-E)
        # cal loss
        tol_loss = max(tb,tf) / max(np.linalg.norm(B,ord='fro'), np.linalg.norm(F,ord='fro'))
        if tol_loss < loss:
            converge=1
        else:
            iter = iter + 1
