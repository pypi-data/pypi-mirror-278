...
class BasetenApi:
    ...
    def deploy_chain(self, name: str, chainlet_data: List[ChainletData]):
        chainlet_data_strings = [f"{{name: \"{chainlet.name}\", oracle_version_id: \"{chainlet.oracle_version_id}\", is_entrypoint: {'true' if chainlet.is_entrypoint else 'false'}}}" for chainlet in chainlet_data]
