...
from .serialize import TemporaryOutputPath, AtomicOutputPath, PickleOutput, StringOutput
from .types import cfg_as_strings
...
def function_cache_path(f, version, *args, **kwargs):
    ...
    if not version and self and hasattr(self, "_version"):
        version = getattr(self, "_version")
    params["_version"] = version

    hash_params = {
        k: v.cache_str() if hasattr(v, "cache_str") else v for k, v in params.items() if k not in _function_cache_path_skip
    }
    ...
    logger.debug("%s%s", self.__module__ + "." + self.__class__.__name__ + "." if self else "", cache_hash)
    ...
    def inner(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_params, cache_string, module_hash, func_hash, self = function_cache_path(
                func, all_args.get("_version"), *args, **kwargs
            )
            ...
            if output_path is None and self and hasattr(self, "output_path"):
                output_path = self.output_path

            search_path = all_args.get("_search_path")
            if search_path is None and self and hasattr(self, "search_path"):
                search_path = self.search_path

            read_only = all_args.get("_read_only")
            if read_only is None and self and hasattr(self, "read_only"):
