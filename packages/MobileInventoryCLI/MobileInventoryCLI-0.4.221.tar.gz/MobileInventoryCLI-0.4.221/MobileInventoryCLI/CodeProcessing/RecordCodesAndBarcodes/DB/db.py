    ...
    def gross(self,rate,unit="$"):
        try:
            ...
            if isinstance(rate,float):
                ...
                if float(ur.convert(total_duration.total_seconds(),"seconds","hours")) > 40:
                    ot=(ur.convert(total_duration.total_seconds(),"seconds","hours")-40)
                    otr=(rate+(rate*1.5))
                    otr_total=(ot*otr)
                    dur=otr_total+(40*rate)
                    print(f"{Fore.light_yellow}{Style.bold}OverTime was accrued: {Fore.light_red}${round(otr,2)}/Hr{Fore.light_yellow} for {Fore.light_magenta}{round(ot,2)} {Fore.light_green}Hrs. = ${otr_total} (OT Gross) + ${round(rate*40,2)} (40 Hr * ${rate}) = ${round(dur,2)}{Style.reset}")
                else:
