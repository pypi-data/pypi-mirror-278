    ...
    def NewEntrySchematic(self):
        while True:
            ...
            with Session(self.engine) as session:
                ...
                if not check:
                    newEntry=self.mkNew(code=code,data={'Name':'','Code':code,'CaseCount':1,'Facings':1})
                    if newEntry == None:
...
""")
    ...
    def __init__(self,engine,parent):
        ...
        self.options["new entry from schematic"]={
                    'cmds':["#"+str(count),f"nefs","new entry from schematic","new_entry_from_schematic"],
                    'desc':f'add a new entry from schematic directly, checking for new item by barcode; the Entry added will have InList=True and ListQty=1, so use {Fore.orange_red_1}ls-lq{Style.reset}{Fore.light_yellow} to view items added{Style.reset}',
