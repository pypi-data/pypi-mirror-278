...
def mkint(text,TYPE):
    try:
        if text.lower() in ['sun_we','sat_we','mon_we','sun_ws']:
            return text.lower()
...
def DatePkr():
    extra='''
sunday week start - sun_we
monday week start - mon_we
sunday week end - sun_we
saturday week end - sat_we
    '''
    def mkYesterday():
    ...
    while True:
        try:
            pass
            year=Prompt.__init2__(None,func=mkint,ptext=f"Year[Default:{date.today().year}]",helpText=f"year to be used in date returned; use y|yesterday for yesterday's date;t|today for today;{extra}",data='year')
            if year in [None,]:
                return None
            elif year in ['y',]:
               return mkYesterday()
    ...
    while True:
        try:
            pass
            month=Prompt.__init2__(None,func=mkint,ptext=f"Month[0..12|Default:{date.today().month}]",helpText=f"month to be used in date returned; use y|yesterday for yesterday's date;t|today for today;{extra}",data='month')
            if month in [None,]:
                return None
            elif month in ['y',]:
               return mkYesterday()
    ...
    while True:
        try:
            pass
            day=Prompt.__init2__(None,func=mkint,ptext=f"Day[{'..'.join((str(1),str(calendar.monthrange(date.today().year,date.today().month)[-1])))}|Default:{date.today().day}]",helpText=f"day to be used in date returned; use y|yesterday for yesterday's date ; t|today for today;{extra}",data='day')
            if day in [None,]:
                return None
            elif day in ['y',]:
               return mkYesterday()
