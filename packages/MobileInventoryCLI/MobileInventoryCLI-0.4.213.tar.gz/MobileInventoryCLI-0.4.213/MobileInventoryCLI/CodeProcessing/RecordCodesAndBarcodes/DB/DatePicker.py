...
def mkint(text,TYPE):
    try:
        if text.lower() in ['y','yesterday',] and TYPE in ['day','month','year']:
            ...
            print([i+1 for i in range(*drange)])
            if TYPE == 'day':
...
def DatePkr():
    ...
    while True:
        try:
            pass
            day=Prompt.__init2__(None,func=mkint,ptext=f"Day[{'..'.join((str(0),str(calendar.monthrange(date.today().year,date.today().month)[-1])))}|Default:{date.today().day}]",helpText="day to be used in date returned; use y|yesterday for yesterday's date ; t|today for today",data='day')
            if day in [None,]:
                return None
            elif day in ['y',]:
               return mkYesterday()
