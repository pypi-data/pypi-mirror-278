...
class Pipeline(ApiDto):
    ...
    def _load_related_entities(self, obj):
        ...
        if "twin" in obj:
            self.twin = Twin()
            self.twin.from_json(obj["twin"])
        if "twinRegistrations" in obj and len(obj['twinRegistrations']) == 1:
            self.registration = TwinRegistration()
            self.registration.from_json(obj["twinRegistrations"][0])
        if "template" in obj["pipeline"]:
            self.template = Template()
            self.template.from_json(obj["pipeline"]["template"])
