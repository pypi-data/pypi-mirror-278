...
class LaundrifyAPI:
	...
	# async def get_hourly_energy(self, _machine: str, startDate: str, endDate: str):
	# 	"""Read hourly energy consumption for a machine"""
	# 	# if endDate is not set, set it to current date
	# 	if endDate is None:
	# 		endDate = date.today().strftime("%Y-%m-%d")
	# 	res = await self.request('get', f'/api/machines/{_machine}/usage', params={"from": startDate, "to": endDate})
