...
def main():
    ...
    get_file(r'D:\MyGames\远行星号 V0.97 RC11\mods')
    for f in files:
        data = pandas.read_csv(f) 
        line_size = int(data.shape[0])
        for i in range(0, line_size):
            name = str(data.iloc[i]["name"])
            mod = str(data.iloc[i]["tech/manufacturer"])
            size = str(data.iloc[i]["designation"])
            hp = str(data.iloc[i]["hitpoints"])
            zj = str(data.iloc[i]["armor rating"])
            fr = str(data.iloc[i]["max flux"])
            fs = str(data.iloc[i]["flux dissipation"])
            fighter = str(data.iloc[i]["fighter bays"]).replace('nan', '0')
            speed = str(data.iloc[i]["max speed"])
            rec = str(data.iloc[i]["supplies/rec"]).replace('nan', '0')
            tags = '"' + str(data.iloc[i]["tags"]) + '"'
            ship = mod + ',' + name + ',' + size + ',' + hp + ',' + zj + ',' + fr + ',' + fs + ',' + fighter + ',' + speed + ',' + rec + ',' + tags
            
            if rec != '0.0' and rec != '0' and name != 'nan' and not name.startswith('#'):
                # print(ship)
                echo(ship, r'D:\ships.csv', 'a')
