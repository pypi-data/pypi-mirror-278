...
from jf_ingest.jf_jira.downloaders import (
    ...
    pull_jira_issues_by_jira_ids, get_buffered_date_string_for_jql,
)
...
def _download_issues_and_metadata_wrapper(
        mocker, earliest_issue_dt: datetime, jf_issue_metadata: list[IssueMetadata]
):
...
def test_download_issues_no_updates():
    ...
    with _mock_for_full_issue_test(
        ...
        jira_issues, issue_metadata_from_jira = _download_issues_and_metadata_wrapper(
            mocker, earliest_issue_dt=earliest_issue_dt, jf_issue_metadata=jf_issue_metadata
        )
...
def test_get_buffered_date_string_for_jql():
    pull_from = datetime(2020, 1, 1, tzinfo=timezone.utc)
    # test that with no issues we use the pull_from
    buffered_date_str = get_buffered_date_string_for_jql([], pull_from)
    assert buffered_date_str == pull_from.strftime("%Y-%m-%d")

    # test that with issues we use the last updated date (with 2 day buffer)
    one_issue = IssueMetadata(id="1", key="PROJ-1", updated=pull_from + timedelta(days=1))
    two_issue = IssueMetadata(id="2", key="PROJ-2", updated=pull_from + timedelta(days=2))
    buffered_date_str = get_buffered_date_string_for_jql([one_issue, two_issue], pull_from)
    assert buffered_date_str == (two_issue.updated - timedelta(days=2)).strftime("%Y-%m-%d")

    # test that an issue with minimum datetime works correctly
    one_issue = IssueMetadata(id="1", key="PROJ-1", updated=datetime.min)
    buffered_date_str = get_buffered_date_string_for_jql([one_issue], pull_from)
    assert buffered_date_str == '0001-01-01'
