...
def get_buffered_date_string_for_jql(
    ...
    year_str = last_run_buffered_string.split("-")[0]
    if len(year_str) < 4:
        year_str = f"{year_str:0>4}"
        last_run_buffered_string = f"{year_str}-{last_run_buffered_string.split('-')[1]}-{last_run_buffered_string.split('-')[2]}"

    return last_run_buffered_string
...
def fetch_id_to_key_for_updated(
    ...
    for proj_id, issues in project_id_to_issue.items():
        last_run_date_with_buffer = get_buffered_date_string_for_jql(issues, pull_from)
