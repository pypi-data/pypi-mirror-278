...
def create_ds(h5_file : 'h5.File', var_info : 'get_var | set_var_dynamic | set_var_static', cache : 'h5_dynamic_cache | H5_static_cache', name=None) -> 'h5.Dataset':
    ...
    if isinstance(var_info, get_var):
        ...
        if size_result_shape*64 < 64e3:
            ...
            chunk_of_result = list(result_shape)
            for i, result_shape_item in enumerate(result_shape):
                if n_chunks < result_shape_item:
                    chunk_of_result[i] = 1
                else:
                    chunk_of_result[i] = np.floor(result_shape_item/n_chunks)
            chunk_size[var_info.ndim:] = chunk_of_result
    ...
    ds =  h5_file.create_dataset(name, shape, maxshape = tuple(max_shape),
                                ...
                                compression="gzip",compression_opts=9, fillvalue=np.NaN)
    ds.write_direct(cache.cache)
