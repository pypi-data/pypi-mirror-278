...
def update_and_run(package_name, package_version):
    try:
        ...
        logic_source = os.path.join(installed_package_path, "CloudConnectorServiceLogic.py")
        main_source = os.path.join(installed_package_path, "CloudConnectorServiceMain.py")
        ...
        if not os.path.isfile(logic_source) or not os.path.isfile(main_source):
            print(f"CloudConnectorServiceLogic.py or CloudConnectorServiceMain.py not found in {installed_package_path}.")
            return
        ...
        logic.ci_print(f"Copied CloudConnectorServiceLogic.py, CloudConnectorServiceMain.py, myservice.py, and setup.py to {current_script_dir}")
...
def init():
    ...
    set_service_restart_delay("CI_CloudConnectorService", 10000)
    set_service_startup_type("CI_CloudConnectorService", "auto")
...
if __name__ == '__main__':
    ...
    args(sys.argv)
    #args([0, 'Start'])
    main_file = "CloudConnectorServiceLogic.py"
