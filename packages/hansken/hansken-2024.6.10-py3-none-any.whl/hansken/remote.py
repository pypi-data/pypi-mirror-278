...
class Connection:
    ...
    def wait_for_task(self, task_id, poll_interval_sec=1, not_found_means_completed=False, progress_callback=None):
        ...
        :param not_found_means_completed: whether to consider task not found a valid (completed) state.
            This is relevant for project deletion tasks after which the task itself will be gone as well.
        :param progress_callback: method to be called every time the status is polled, method should take
            two arguments (poll_count: int, progress: double[0..1]), if not provided the callback will be skipped.
        ...
        while True:
            ...
            if not_found_means_completed and response.status_code == codes.not_found:
                log.info('task finished with status: completed')
                return 'completed'

            task_json = _expect_ok(response).json()
            task = task_json['task']
            ...
            poll_count = poll_count + poll_interval_sec
