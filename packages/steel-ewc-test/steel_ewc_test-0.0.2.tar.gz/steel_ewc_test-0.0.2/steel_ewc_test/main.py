...
from util.ewc_utils import EWC
...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        # 分别给训练集和测试集创建路径
        self.task_TRain_names = os.listdir(f"steel_ewc_test/data/{self.dataset}/TRain")
        self.task_TEst_names = os.listdir(f"steel_ewc_test/data/{self.dataset}/TEst")
        ...
        for task_name in self.task_TRain_names:
            dir_path = f"steel_ewc_test/data/{self.dataset}/TRain/{task_name}"
        ...
        for task_name in self.task_TEst_names:
            dir_path = f"steel_ewc_test/data/{self.dataset}/TEst/{task_name}"
            steel_folder = os.listdir(dir_path)
            print("steel_folder:", steel_folder)
            test_dataset_task = []
    ...
    def run(self):
        ...
        for task_idx in range(num_tasks):
            if task_idx == 0:
                dir_path = f"steel_ewc_test/data/{self.dataset}/TRain/Task1(拉速1.2)"
            elif task_idx == 1:
                dir_path = f"steel_ewc_test/data/{self.dataset}/TRain/Task2(拉速1)"
            else:
                dir_path = f"steel_ewc_test/data/{self.dataset}/TRain/Task3(拉速1.4)"
    ...
    def plot_anomalies(self, prediction, observation, task_idx, steel_idx):
        ...
        plt.savefig(f'./results/task1/compare_label.png')
        plt.show()
        plt.close()
        # 绘制回归散点图
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.savefig(f'./results/task1/scatter.png')
        plt.show()
        plt.close()
        # 计算并绘制方差分布图
        ...
        plt.savefig(f'./results/task1/resid.png')
        plt.show()
        plt.close()
        # 评价指标
        # 计算均方根误差 (RMSE)
...
def model_exp(model_name=None):
    ...
    parser.add_argument('-dataset', help='steel', type=str, default='steel')
