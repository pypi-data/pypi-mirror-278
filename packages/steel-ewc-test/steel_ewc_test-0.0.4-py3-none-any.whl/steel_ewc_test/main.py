...
class Main():
    ...
    def plot_anomalies(self, prediction, observation, task_idx, steel_idx):
        ...
        plt.savefig(f'./steel_ewc_test/results/task1/compare_label.png')
        plt.show()
        plt.close()
        # 绘制回归散点图
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.savefig(f'./steel_ewc_test/results/task1/scatter.png')
        plt.show()
        plt.close()
        # 计算并绘制方差分布图
        ...
        plt.savefig(f'./steel_ewc_test/results/task1/resid.png')
        plt.show()
        plt.close()
        # 评价指标
        # 计算均方根误差 (RMSE)
