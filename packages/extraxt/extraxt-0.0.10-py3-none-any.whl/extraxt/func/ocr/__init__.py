...
class OCR:
    ...
    def doc(self, stream):
        stream.seek(0)
        content = stream.read()
        logging.info(f"Opening file with `fitz`...")
        doc = fitz.open(stream=content, filetype="pdf")
        return doc

    def text(self, doc):
        logging.info(f"Reading text content from file...")
    ...
    def parse(self, lines):
        logging.info(f"Parsing and sanitising text content...")
        data = {}
        key = None
        capture_age = False
        ...
        for line in lines:
            ...
            if "date of birth:" in lower_line:
                logging.info(
                    f"* Sensitive data found: [Date of birth]. This data will be parsed as [Age]..."
                )
                key = "age"
                data[key] = ""
                capture_age = True
