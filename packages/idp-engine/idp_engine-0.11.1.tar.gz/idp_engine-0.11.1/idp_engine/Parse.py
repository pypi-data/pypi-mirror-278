...
from .utils import (RESERVED_SYMBOLS, OrderedSet, NEWL, BOOL, INT, REAL, DATE,
                    CONCEPT, GOAL_SYMBOL, EXPAND, RELEVANT, ABS, COUNTER,
                    IDPZ3Error, MAX_QUANTIFIER_EXPANSION, Semantics as S,
                    flatten, split_prefix)
...
class IDP(ASTNode):
    ...
    def __init__(self, **kwargs):
        ...
        self.prefixes = {name: pref.uri for (name, pref) in
                         self.dedup_nodes(kwargs, 'prefixes').items()}
        for name, voc in self.vocabularies.items():
            for k, v in self.prefixes.items():
                self.check(k not in voc.declared_prefixes or
            ...
            if isinstance(decl, SymbolDeclaration):
                for symbol in decl.symbols:
                    ...
                    if new.partiality == 'partial' and decl.domains == decl.sorts:
                        ...
                        name = f"dom_{symbol}"
                        dom = SymbolDeclaration(parent=parent,
                                                annotations=None,
                                                sorts=new.domains,
                                                sort_=BOOL_SETNAME,
                                                name=name)
    ...
    def extensionE(self,
        ...
        def filter(args):
            if type(args[0]) != Variable and type(args[0].decl) == Constructor:  # try to simplify it
                #TODO add tests
                self.check(self.parent.name == args[0].decl.codomain.name,
                        f"Incorrect type of {args[0]} for {self.parent.name}")
                self.check(len(args[0].sub_exprs) == len(args[0].decl.domains),
                        f"Incorrect arity")
                return AND([t.decl.domains[0].has_element(e, extensions)
                            for e,t in zip(args[0].sub_exprs, args[0].decl.domains)])
            out = [AppliedSymbol.construct(constructor.tester, args)
                    for constructor in self.constructors]
