...
def _propagate_using_models(self, solver, valqs, tag: S):
...
Theory._propagate_using_models = _propagate_using_models
    ...
    if not removed_choices and not added_choices:
        ...
        for a in todo.values():
            if (a.code in self.assignments
               and self.assignments[a.code].status not in [S.UNKNOWN]):
                to_remove.append(a)
