...
from .Expression import (INT_SETNAME, AIfExpr, IF,
...
def interpret(self: TypeDeclaration, problem: Theory):
    ...
    if self.name in [BOOL, CONCEPT]:
        ...
        if enum.constructors:
            ...
            enum.lookup = {e.code: TRUE for e in enum.sorted_tuples}
        ...
        if self.super_set and enum.tuples:  # check that the enumeration is a subset of the supertype
            super_set = self.super_sets[0].decl.interpretation.enumeration.tuples
            if super_set:
                for t in enum.tuples:
                    self.check(t in super_set,
...
def interpret(self: SymbolDeclaration, problem: Theory):
    ...
    def filter(args):
        ...
        """
        self.check(self.codomain == BOOL_SETNAME, "Internal error")
        out = AppliedSymbol.make(self.symbol_expr, args, type_check=False)
        return out
        ...
        for inst in self.instances.values():
            ...
            if self.codomain != BOOL_SETNAME:
                ...
                if len(filters) == len(args):  # domain: p1*pn
                    cond = AND([f([t]) if f is not None else TRUE
                ...
                constraint = IMPLIES([cond, range_condition])
                ...
                if len(self.super_sets) == 1:  # for p <: q
                    ...
                    q = AppliedSymbol.make(symb, args, type_check=False)
...
def interpret(self: SymbolInterpretation, problem: Theory):
    ...
    if not self.name in [GOAL_SYMBOL, EXPAND, COUNTER]:
        ...
        if self.default is not None:
            ...
            if len(decl.domains) == decl.arity:
                ...
                quantees = [Quantee.make(list(VARIABLE(f"${sort.decl.name}!{str(i)}$", sort)
            ...
            if (len(decl.domains) == decl.arity
            and constraint1.sub_exprs[0] == FALSE):  # no filter added
                ...
                if domain != enumeration:
                    ...
                    self.check(False, f"Enumeration of {self.name} should cover its domain ({errors})")
