...
def test_create_data():
    ...
    # 主数据
    tenant = Tenant(name='测试租户', domain='test.com', enabled=True)
    session.add(tenant)
    session.commit()
    # 关联主数据
    attribute = Attribute(code='test', object='test', value_type='string', name='测试属性', default_value='test')
    attribute.tenant_id = tenant.id
    session.add(attribute)
    session.commit()
    # 创建子数据
    attribute_select_item1 = AttributeSelectItem(tenant_id=tenant.id, text='测试数据', value='test')
    attribute_select_item2 = AttributeSelectItem(tenant_id=tenant.id, text='测试数据2', value='test2')
    attribute.attribute_select_items = [attribute_select_item1, attribute_select_item2]
    session.commit()
    # 指向自身的数据
    parent_organization = Organization(tenant_id=tenant.id, number='test_parent', name='测试父级组织')
    child_organization = Organization(tenant_id=tenant.id, number='test_child', name='测试子级组织')
    child_organization.parent = parent_organization
    session.add(parent_organization)
    session.add(child_organization)
    session.commit()
    # 关联表关联的数据
    user = User(tenant_id=tenant.id, number='testuser', password='123456', name='测试用户')
...
def test_get_data():
    ...
    assert attribute.tenant.id == tenant.id
    # 指向自身的数据
    ...
    # 关联表关联的数据
    assert parent_organization.users[0].number == 'test_user'
