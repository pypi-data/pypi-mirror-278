...
from ...utilities.s2 import get_citation_counts

logger = logging.getLogger(__name__)

HTML_CUTOFF_DATE = date(2023, 12, 1)

# from importlib.resources import files
from .arxiv_categories import CATEGORIES
# CATEGORIES = json.loads(files('arxiv_agent.tools.search').joinpath('arxiv_categories.json').read_text())
...
class ArxivAPIWrapper(BaseModel):
    ...
    def run(self, query: str) -> str:
        ...
        citation_counts = get_citation_counts([result.entry_id.split('/')[-1] for result in results])

        docs = []
        for i, result in enumerate(results):
            arxiv_id = result.entry_id.split('/')[-1]

            # additional fields not from arxiv
            ...
            doc = (
                ...
                f"last update: {result.updated.date()}\n"
                f"primary category: {result.primary_category}\n"
                f"categories: {', '.join(result.categories)}\n"
                f"citation count: {count}\n"
            )
            docs.append( doc )
