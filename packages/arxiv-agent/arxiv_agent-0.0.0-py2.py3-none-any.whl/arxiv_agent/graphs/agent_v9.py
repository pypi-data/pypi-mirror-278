...
def predict9(
    ...
    actor_instructions = f"What should you do next to answer the question?"
    actor_instructions_x = f"DO NOT try the following tool again because you already tried them: "
    ...
    while 'final_answer' not in current_tools:
        ...
        if verbose: blue(f"Iter #{iter}. execute_tools") 
        ...
        instructions = actor_instructions + actor_instructions_x + str(List_of_actions)

        initial = student_w_Notes.invoke(
            {"messages": history, "question": question, "Notes": str(List_of_Notes[:-1]), "instructions": instructions},
            config={"configurable": {"session_id": session_id}}
