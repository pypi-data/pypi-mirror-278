...
from keras.layers import Conv2D, Normalization
...
class CachedDataGenerator(Sequence):
    def __init__(self, df, batch_size=32, mode=None, n_jobs=-1, cache_folder=None):
        self.df_orig = df
        self.df = None
        self.indices = None
        ...
        self.update_df(self.df_orig)
        self.on_epoch_end()

        self._count_cache = False
        self._cache_folders = dict()
    ...
    def cached(self, func):
        """
        Helper decorator: can use inside get_sample()
        Wrapper around x_cached_call()
        """

        plan = x_cached_call(func, cache_folder=self._cache_folder, get_plan='cache_folder')
        self._cache_folders[func.__name__] = plan.cache_folder

        def wrapper(*args, **kwargs):
            if self._count_cache:
                plan = x_cached_call(func, *args, get_plan=True, cache_folder=self._cache_folder, **kwargs)
                cache_miss = int(not plan.from_cache)
            ...
            return x_cached_call(func, *args, cache_folder=self._cache_folder, **kwargs)
        return wrapper
    ...
    def clear_cache(self, cache_name):
        ...
        if cache_name not in self._cache_folders:
            print(f'WARNING: {cache_name} not in {sorted(self._cache_folders.keys())}')
            return

        cache_folder = path(self._cache_folders[cache_name])
        cache_folder.ensure_dir()
        cache_folder.rmtree()

    def update_df(self, df):
        """
        Helper function: can use inside on_epoch_start()
        Updates the df to be used. Can access self.df_orig
        """
        self.df = df.copy()
    ...
    def get_sample(self, row):
        """
        Given a row, return a DataSample()
