...
from ketacli.sdk.util import template_render
...
class KTable(BaseChart):
    def __init__(self, chart_config: dict, mark="", **kwargs):
        ...
        self.spl = template_render(spl, **kwargs)
    ...
    def search(self, spl=None):
        ...
        if spl:
            ...
            self.title = spl
        try:
            self.data = search_result_output(search_spl(spl=str(self.spl), start=self.start, end=self.end, limit=self.limit),
    ...
    def get_table(self):
        ...
        up_down = "上下键切换行"
        left_right = "左右键切换页码"
        self.subtitle = f"[#808080][bold green]{self.row_index + 1}/{self.page_size}[/bold green] {up_down} | [bold green]{self.page_num}/{math.ceil(self.total_row / self.init_page_size)}[/bold green] {left_right}{identification}{self.mark}[/#808080]"
    ...
    def main_table(self, console, options):
        ...
        if self.child_chart:
            ...
            table_panel = Panel(table, expand=True, padding=0, title=title_text,
                                ...
                                subtitle=self.subtitle, height=self.height)
