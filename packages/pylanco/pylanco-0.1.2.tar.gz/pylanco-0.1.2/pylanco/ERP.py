...
class Categories:
    ...
    def name(self, name=None):
        if name is None:
            raise ValueError("Category name must be provided")
        filtered_categories = self._filter_categories_by_name(name.lower())
        if not filtered_categories:
            raise ValueError(f"No customer category with name '{name}' in customer categories")
        return filtered_categories[0]
    
    def id(self, category_id=None):
        if category_id is None:
            raise ValueError("Category ID must be provided")
        filtered_categories = self._filter_categories_by_id(int(category_id))
        if not filtered_categories:
            raise ValueError(f"No customer category with ID '{category_id}' in customer categories")
        return filtered_categories[0]
...
class Groups:
    ...
    def name(self, name=None):
        ...
        filtered_groups = self._filter_groups_by_name(name.lower())
        if not filtered_groups:
            raise ValueError(f"No customer group with name '{name}' in customer groups")
        return filtered_groups[0]

    def id(self, group_id=None):
        if group_id is None:
            raise ValueError("Group ID must be provided")
        filtered_groups = self._filter_groups_by_id(int(group_id))
        if not filtered_groups:
            raise ValueError(f"No customer group with ID '{group_id}' in customer groups")
        return filtered_groups[0]
...
class EmployeeGroups:
    ...
    def employees(self, group=None):
        if group is None:
            raise ValueError("Employee group must be provided")
        if str(group).isdigit():
    ...
    def name(self, name=None):
        if name is None:
            ValueError("Group name must be provided")
        filtered_groups = self._filter_groups_by_name(name.lower())
        if not filtered_groups:
            raise ValueError(f"No employee group with name '{name}' in employee groups")
        return filtered_groups[0]
    
    def id(self, group_id=None):
        if group_id is None:
            raise ValueError("Employee group ID must be provided")
        filtered_groups = self._filter_groups_by_id(int(group_id))
        if not filtered_groups:
            raise ValueError(f"No employee group with ID '{group_id}' in employee groups")
        return filtered_groups[0]
...
class EmployeeTeams:
    ...
    def employees(self, team=None):
        if team is None:
            raise ValueError("Employee team must be provided")
        if str(team).isdigit():
    ...
    def name(self, name=None):
        if name is None:
            ValueError("Team name must be provided")
        filtered_teams = self._filter_teams_by_name(name.lower())
        if not filtered_teams:
            raise ValueError(f"No employee team with name '{name}' in employee teams")
        return filtered_teams[0]
    
    def id(self, team_id=None):
        if team_id is None:
            raise ValueError("Employee team ID must be provided")
        filtered_teams = self._filter_teams_by_id(int(team_id))
        if not filtered_teams:
            raise ValueError(f"No employee team with ID '{team_id}' in employee teams")
        return filtered_teams[0]
