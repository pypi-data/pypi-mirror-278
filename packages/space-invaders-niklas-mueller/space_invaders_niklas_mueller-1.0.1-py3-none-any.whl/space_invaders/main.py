...
import space_invaders.assets

player_im_path = str(pkg_resources.files(space_invaders.assets).joinpath("player.png"))
alien_im_path = str(pkg_resources.files(space_invaders.assets).joinpath("alien.gif"))
blockade_im_path = str(
    pkg_resources.files(space_invaders.assets).joinpath("blockade.png")
)


def load_assets() -> list[Any]:
    player_im = pygame.image.load(player_im_path)
    enemy_im = pygame.image.load(alien_im_path)
    blockade_im = pygame.image.load(blockade_im_path)
