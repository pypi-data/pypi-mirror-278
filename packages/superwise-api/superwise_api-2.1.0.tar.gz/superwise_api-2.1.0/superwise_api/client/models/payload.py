...
class Payload(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in Payload with anyOf schemas: ToolConfigBigQuery, ToolConfigPGVector, ToolConfigSQLDatabaseMSSQL, ToolConfigSQLDatabaseMySQL, ToolConfigSQLDatabaseOracle, ToolConfigSQLDatabasePostgres. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Payload:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Payload with anyOf schemas: ToolConfigBigQuery, ToolConfigPGVector, ToolConfigSQLDatabaseMSSQL, ToolConfigSQLDatabaseMySQL, ToolConfigSQLDatabaseOracle, ToolConfigSQLDatabasePostgres. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
