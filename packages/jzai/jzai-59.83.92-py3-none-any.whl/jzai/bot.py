...
class Bot:
    ...
    def extract_math_expression(self, text):
        math_match = re.search(r'([0-9+\-*/^(). ]+)', text)
        if math_match:
    ...
    def evaluate_math_expression(self, expr):
        try:
            expr = expr.replace('^', '**')
            result = eval(expr)
            return str(f"The answer to '{expr.replace('**', '^')}' is {result}.")
        except SyntaxError:
            return "You must only add the equation. Example '1+1'. Not 'What is 1+1' or whatsoever"
        except Exception as e:
            return f"Error: {e}"
        
    def generate_response(self, user_input):
        try:
            math_expression = self.extract_math_expression(user_input)
            if math_expression:
                return self.evaluate_math_expression(math_expression)
