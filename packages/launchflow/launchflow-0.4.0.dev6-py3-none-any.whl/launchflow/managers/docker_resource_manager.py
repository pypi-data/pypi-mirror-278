...
def get_container_filter_labels(*, resource_name: Optional[str], environment_name: str) -> List[str]:
    """
    Returns a list of labels that uniquely identify the resource's container for filtering.

    Args:
    - `resource_name`: The name of the resource, or None to get filter labels for the environment.
    - `environment_name`: The name of the environment.

    Returns:
    A list of labels for filtering.
    """
    if resource_name is None:
        return ["launchflow_managed=true", f"environment={environment_name}"]
    return ["launchflow_managed=true", f"environment={environment_name}", f"resource={resource_name}"]
...
class DockerResourceManager(BaseManager):
    ...
    def __reduce__(self) -> Tuple[Type["DockerResourceManager"], Tuple[str, str, str]]:
        return (self.__class__, (self.project_name, self.environment_name, self.resource_name))
    ...
    def get_running_container_id(self) -> Optional[str]:
        ...
        """
        containers = self._docker_client.client.containers.list(
            filters={
                "label": get_container_filter_labels(resource_name=self.resource_name, environment_name=self.environment_name)
            },
            all=True
        )
    ...
    async def load_resource(self) -> ResourceState:
        ...
        return ResourceState(
            ...
            status=ResourceStatus.READY if container.status == "running" else ResourceStatus.CREATE_FAILED,
