...
class ProjectSetup:
    ...
    def _copy_resources(self):
        ...
        if artifact_source_type == ArtifactSourceType.SOURCE_CODE.value:
            source_code_path = self.parameters.get(project_setup_constant.SOURCE_CODE_PATH_KEY)
            self._handle_source_code_packaging(source_code_path, project_setup_constant.APP_NAME)
        # 软件包
    ...
    def _handle_source_code_packaging(self, source_code_path, app_name):
        ...
        # 打包源代码目录到.tar.gz格式
        try:
            archive_path = shutil.make_archive(base_name=str(self.output_base_path / app_name), format='gztar',
                                               root_dir=source_code_path.parent, base_dir=source_code_path.name)
            print(f"Directory '{source_code_path}' has been packed into '{archive_path}'")
        except Exception as e:
            print(f"Packing failed: {e}")
            return

        print(f"Packed archive is available at '{archive_path}'")

        # 移动.tar.gz文件到指定目录
        destination_dir = self.output_base_path / project_setup_constant.OUTPUT_PACKAGE_DIR
        destination_dir.mkdir(parents=True, exist_ok=True)
        tar_gz_path = destination_dir / (app_name + '.tar.gz')
