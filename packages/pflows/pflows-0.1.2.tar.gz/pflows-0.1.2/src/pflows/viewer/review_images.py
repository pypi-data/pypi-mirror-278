...
def putTextWithShadow(image, label, position, font, font_scale, text_color, shadow_color, text_thickness=1, shadow_thickness=1, shadow_offset=2):
...
def create_legend(classes, colors):
    ...
    for i, class_name in enumerate(classes):
        ...
        cv2.putText(legend_image, class_name, (30, i * 25 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
    return legend_image
...
def load_image(image_path, selected_category, legend_image, classes, colors, initial=False):
    ...
    if confidence_path and os.path.exists(confidence_path):
        confidences = [
            ...
            for confidence in open(confidence_path, 'r').read().splitlines()
        ]
    ...
    for index, box in enumerate(yolo_output):
        ...
        putTextWithShadow(image, label, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, (0,0,0))

        if confidences is not None:
            confidence = f"{confidences[index]:.2f}"
            putTextWithShadow(image, confidence, (left, top + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, (0,0,0))
...
def main():
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            if x < legend_image.shape[1] and y < legend_image.shape[0]:
                ...
                processed_image = load_image(images[current_image_index], selected_category, legend_image, classes, colors)
    ...
    while True:
        ...
        if processed_image is None:
            processed_image = load_image(images[current_image_index], selected_category, legend_image, classes, colors, initial=True)
        key = cv2.waitKey(0)
        if key == ord('q'):
            break
