...
class Net:
    ...
    def __get_output_blobs(self, output_buffer=None):
        ...
        """
        r = []
        out_blobs = self.get_output_blob_list()
        if output_buffer is not None:
            if isinstance(output_buffer, collections.abc.Sequence):
                for i in range(min(len(out_blobs), len(output_buffer))):
                    r.append(self.get_blob_data(out_blobs[i], output_buffer[i]))
            else:
                return [self.get_blob_data(out_blobs[0], output_buffer)]
        else:
            for i in range(len(out_blobs)):
                r.append(self.get_blob_data(out_blobs[i], None))
        return r
    ...
    def predict(self, input, output=None):
        ...
        in_blobs = self.get_input_blob_list()
        compatibility = False
        if isinstance(input, dict):
            for k in input.keys():
                idx = 0
                if type(k) is str:
                    idx = self.find_blob_index_by_name(k)
                else:
                    idx = in_blobs[k]
                self.set_input_blob_data(input[k], idx)
    ...
    def copy_blob_data(self, dst_idx, src_idx, src_net):
        ...
        if type(src_idx) == str:
            src_idx = self.find_blob_index_by_name(src_idx)
    ...
    def get_results(self):
        ...
        """
        r = []
        for idx in self.get_output_blob_list():
            r.append(self.get_blob_data(idx))
        return r
