...
def dark_task(tmp_path, recipe_run_id, link_constants_db):
    ...
    with DarkCalibration(
        ...
        yield task, lamp_exp_time, solar_exp_time, polcal_exp_time, observe_exp_time, unused_exp_time, num_frames_per_exp
...
def test_dark_calibration_task(dark_task, mocker):
    ...
    for exp in [lamp_exp_time, solar_exp_time, polcal_exp_time, observe_exp_time]:
        dark_list = list(
            task.read(
                tags=[
    ...
    with file.open() as f:
        ...
        assert (
            data["total_frames"] == num_frames_per_exp * 5
        )  # For lamp, solar, polcal, obs, and unused
