...
from dkist_processing_dlnirsp.tasks.mixin.matrix_multiplication import MatrixMultiplicationMixin
...
class ScienceCalibration(
    ...
    def run(self) -> None:
        ...
        logger.info("Making remapped IFU mapping files for debug work.")
        self.make_debug_ifu_mapping_files(calibrations=calibrations)
        ...
        for mosaic_num in range(self.constants.num_mosaic_repeats):
            for tile_num in range(self.constants.num_mosaic_tiles):
                ...
                self.write_calibrated_array(
                    combined_frame, calibrated_header, mosaic_num, tile_X, tile_Y
                )
    ...
    def process_polarimetric_frames(
        ...
        demodulated_data = self.nd_left_matrix_multiply(
    ...
    def apply_telescope_polarization_correction(
        ...
        corrected_data = self.nd_left_matrix_multiply(
    ...
    def write_calibrated_array(
        ...
        for i, stokes in enumerate(stokes_targets):
            final_data = self._re_dummy_data(array[:, :, i])
            if self.constants.correct_for_polarization:
    ...
    def make_debug_ifu_mapping_files(self, calibrations: CalibrationCollection) -> None:
        """
        ...
        for name, array in zip(
            ["ID", "FXPOS", "FYPOS"],
            [
                self.parameters.group_id_array,
            ...
            try:
                ...
                tags = [DlnirspTag.debug(), DlnirspTag.task(f"REMAPPED_IFU_{name}")]
                path = self.write(
                    data=combined_array, tags=tags, encoder=fits_array_encoder, overwrite=True
                )
                logger.info(f"Wrote {name} to {str(path)}")
            except Exception as e:
                logger.info(f"Error while make debug output for {name}. Error was {e}.")
    ...
    # TODO: This is where we would do the IFU remapping
    def _re_dummy_data(self, data: np.ndarray):
        ...
        data : np.ndarray
            Corrected data
        """
        logger.info(f"Adding wavelength dimension to array with shape {data.shape}")
        # We need the dummy dimension to have length > 1 so that the dataset axis keys
        # describe a correct axis. Eventually this third axis *will* have a meaningul length
        # so we need to mock that now.
