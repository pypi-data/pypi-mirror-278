...
class GeometricCalibration(DlnirspTaskBase, GroupIdMixin, CorrectionsMixin, QualityMixin):
    """
    ...
    def compute_average_corrected_gain(self) -> None:
        ...
        lamp_array = next(
            self.read(
                tags=[DlnirspTag.intermediate(), DlnirspTag.frame(), DlnirspTag.task("LAMP_GAIN")],
    ...
    def compute_spectral_curvature_and_dispersion(
        ...
        self.write(
            reference_spectrum,
            tags=[DlnirspTag.debug(), DlnirspTag.task("GC_REF_SPEC")],
        ...
        for slitbeam in group_id_to_slitbeam_mapping.keys():
            ...
            for group_id in groups_in_slitbeam:
                ...
                for i, spatial_px in enumerate(group_spatial_px):
                    ...
                    # self.write(
                    #     data=corrected_spec,
                    #     tags=[DlnirspTag.debug(), DlnirspTag.task(f"TARGET_SPEC_{group_id}_{i}")],
                    #     encoder=fits_array_encoder,
                    # )
            ...
            self.write(
                data=debug_data,
                tags=[DlnirspTag.debug(), DlnirspTag.task(f"GC_RAW_FIT_VALUES_SLITB_{slitbeam}")],
                encoder=fits_array_encoder,
                overwrite=True,
            )
    ...
    def compute_reference_wavelength_axis(
        ...
        flat_shifts = np.hstack(shift_dict.values())
