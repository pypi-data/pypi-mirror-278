    ...
    def login_to_site(self) -> bool:
        ...
        for i in range(1, 4):
            try:
                ...
                self.wait_element('//input[@type="email"]', is_need_screenshot=False, timeout=10)

                self.check_scheduled_maintenance()

                self.wait_element('//input[@type="email"]')
                self.browser.input_text_when_element_is_visible('//input[@type="email"]', self.login)
