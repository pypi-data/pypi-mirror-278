...
class GoogleCloudAppEngineEmailNotifications(BaseNotifications):
    ...
    def _generate_notification_html(
        ...
        html_body += f"""
                ...
                <p><strong>Task Logic: </strong>{_to_human_readable(task_formula, "formula")}
        ...
        for placement in placements:
            html_body += f"""
                <ul class="placement-list">
                    ...
                    {placement["placement"]}
                    <br/>
                    Reason: {_to_human_readable(placement.reason, "reason")}</li>
                </ul>
            """
        html_body += """
            </div>
...
def _to_human_readable(text: str, description: str) -> str:
    return f'Missing {description}' if not text else text.lower().replace('_', ' ')
