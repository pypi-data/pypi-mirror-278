...
class GoogleCloudAppEngineEmailNotifications(BaseNotifications):
    ...
    def send(self, payload: MessagePayload, **kwargs: str) -> None:
        ...
        self._debug_stacktrace()
        message = self._prepare_message(payload)
        message.send()

    def _debug_stacktrace(self) -> None:
        # TODO: Remove after debugging
        current_frame = sys._getframe().f_back
        target_frame = current_frame.f_back if hasattr(current_frame, 'f_back') else current_frame
        stack_trace = traceback.format_stack(target_frame)
        stack_trace_str = ''.join(stack_trace)
        logging.info(f'Stack trace before calling GoogleCloudAppEngineEmailNotifications::send:\n{stack_trace_str}')
    ...
    def _prepare_message(self, payload: MessagePayload) -> mail.Message:
        ...
        if placements_excluded := payload.placements_excluded_sample:
            subject = f'''{'Detected' if not payload.total_placement_excluded else
                f'Excluded {payload.total_placement_excluded}'} placements'''
            body = self._generate_notification_html(
                payload.total_placement_excluded,
    ...
    def _generate_notification_html(self, total_placement_excluded: int, task_id: str, task_name: str, task_formula: str, placements: gaarf.report.GaarfReport) -> str:
        html_body = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        html_body += f"""
                <p>The CPR tool has {("detected" if not total_placement_excluded
                ...
                class="task-link" target="_blank" title="View Task Details">(task id {task_id})</a>
                </p>"""
