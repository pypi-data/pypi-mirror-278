"""Initializes the precision_timer module by wrapping the C++ extension binding (CPrecisionTimer) into the pure-python
PrecisionTimer class.

This allows using PrecisionTimer frontend to access C++ methods, which, in turn, provides code autocompletion,
docstrings, type-hints and other pure python features that are helpful for python developers.
"""
...
class PrecisionTimer:
    ...
    Attributes:
        __supported_precisions: A set of currently supported precision string-options. Used internally for input
            verification and error-messaging purposes.
        __timer: The inner binding-class generated by nanobind to expose C++ API. Since the class has no stubs at the
            time of writing, the best way to use this library is via the wrapper class.
    ...
    def __init__(self, precision: Literal["ns", "us", "ms", "s"] = "us") -> None:
        ...
        self.__timer = CPrecisionTimer(precision=precision)

    def __repr__(self):
        representation_string: str = (
            f"PrecisionTimer(precision={self.precision}, elapsed_time = {self.elapsed} {self.precision}.)"
        )
        return textwrap.fill(representation_string, width=120, break_long_words=False, break_on_hyphens=False)
    ...
    def precision(self) -> str:
        ...
        return self.__timer.GetPrecision()
    ...
    def delay_noblock(self, delay: int, *, allow_sleep: bool = False) -> None:
        ...
        Notes:
            Even if sleeping is allowed, the method will only sleep if the duration is sufficiently long to resolve the
        ...
        """
        self.__timer.DelayNoblock(delay, allow_sleep)
    ...
    def delay_block(self, delay: int, *, allow_sleep: bool = False) -> None:
        ...
        Args:
            ...
            allow_sleep: A boolean flag that allows using CPU-releasing sleep() method to suspend execution for
                durations above 1 millisecond. Defaults to False.
        """
        self.__timer.DelayBlock(delay, allow_sleep)
    ...
    def set_precision(self, precision: Literal["ns", "us", "ms", "s"]) -> None:
        ...
        if precision not in self.__supported_precisions:
            error_message = (
                ...
                f"{self.__supported_precisions}."
            )
            raise ValueError(textwrap.fill(error_message, width=120, break_long_words=False, break_on_hyphens=False))

        self.__timer.SetPrecision(precision=precision)
