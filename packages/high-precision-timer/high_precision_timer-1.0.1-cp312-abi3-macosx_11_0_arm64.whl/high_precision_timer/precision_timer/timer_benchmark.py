...
Successful installations of the library exposes a shorthand 'benchmark_timer' command that can be used to trigger the
...
@click.command()
@click.option("--interval-cycles", "-ic", type=int, default=60,
              help="Number of times to repeat interval benchmark for each precision. Default: 10.")
@click.option("--interval-delay", "-id", type=float, default=1, help="Interval delay in seconds. "
                                                                     "Default: 1")
@click.option("--delay-cycles", "-dc", type=str, default="1000,1000,1000,60",
...
@click.option("--delay-durations", "-dd", type=str, default="250,5,2,1",
              help="Delay durations for each precision (comma-separated list in the order of ns, us, ms, s). Default: "
                   "'250, 5, 2, 1'.")
def benchmark(interval_cycles, interval_delay, delay_cycles, delay_durations):
    ...
    for precision in precisions:  # Loops over all precisions
        ...
        for _ in tqdm(range(interval_cycles),
                      desc=f"Running interval timing benchmark for {precision} precision timer"):
        ...
        # Calculates the mean and std for the recorded elapsed times and adds them to the results list alongside some ID
        # information.
        interval_results.append(
            (precision, np.around(np.mean(elapsed_deltas), 3), np.around(np.std(elapsed_deltas), 3))
        )  # Appends cycle results to storage list
    ...
    for index, precision in enumerate(precisions):  # Loops over all precisions
        ...
        for _ in tqdm(range(delay_cycles_list[index]),
                      desc=f"Running busywait delay benchmark for {precision} precision timer"):
    ...
    for index, precision in enumerate(["ms", "s"], start=2):  # Loops over all precisions
        ...
        for _ in tqdm(range(delay_cycles_list[index]),
                      desc=f"Running sleep delay benchmark for {precision} precision timer"):
            # Tests blocking delay
            start = tm.perf_counter_ns()
