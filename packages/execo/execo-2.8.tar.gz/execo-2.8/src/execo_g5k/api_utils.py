...
def _get_api():
    ...
    data = {'version': get_resource_attributes('')['version'],
            'network': {},
            'sites': {},
            'clusters': {},
            'hosts':  {},
...
__g5k_host_group_regex = re.compile("^([a-zA-Z]+\d*)-\d+(\.(\w+))?")
...
def canonical_host_name(host):
    """Convert, if needed, the host name to its canonical form without interface part, kavlan part, ipv6 part.
