...
class IAPListener():
    ...
    def __init__(self,
        ...
        self.outputs = {}
        self.default_output = self.queue + "." + self.outputs_name[0]
        for entity in self._dag.pipeline:
    ...
    async def _run_async(self):
        ...
        async def send_reply(queue_out, uid, source, data, error=""):            
            _out = queue_out + "." + uid
            breply = b''
            if data is not None:
                match source:
        ...
        async def handle_msg(name, msgs, is_batch: bool):
            if is_batch:
                ...
                batch = list(batch)
                reply, queue_out, err = self._process_data(name, batch, content_types, params_lst, is_batch)
                if not err:
                    for data, uid, source in zip(reply, uids, sources):
                        await send_reply(queue_out, uid, source, data)
                    return
                for uid, source in zip(uids, sources):
                    await send_reply(queue_out, uid, source, None, str(err))
                return

            for msg in msgs:
                uid, source, data, params, content_type = await get_data(msg)
                reply, queue_out, err = self._process_data(name, [data], [content_type], [params], is_batch)
                if err:
                    await send_reply(queue_out, uid, source, reply, str(err))
                else:
                    await send_reply(queue_out, uid, source, reply[0])
                return
