...
class DataHandler():
    ...
    def items(self) -> dict:
        if self._is_multi:
            if not self._items:
                self._items, self.error = decode_multi_part(self._raw, self._conf["items"], self._content_type)
            return self._items
        else:
            if not self._items:
...
def decode_multi_part(data: bytes, items: list, content_type: str) -> tuple[dict, Error]:
    """
...
def encode_multi_part(data: dict) -> tuple[bytes, str, Error]:
    """
