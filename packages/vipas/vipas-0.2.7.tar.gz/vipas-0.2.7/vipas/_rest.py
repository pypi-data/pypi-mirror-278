...
class RESTClientObject:
    ...
    async def request(self, method, url, headers=None, body=None):
        ...
        for attempt in range(attempts):
            ...
            if payload_type == "url":
                try:
                    output_data_response = await self.client.request("GET", predict_data.get("payload_url"))
    ...
    def request_sync(self, method, url, headers=None, body=None):
        """Synchronous wrapper for the async request method."""
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # If the loop is already running, use a new loop to avoid conflicts
                new_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(new_loop)
                return new_loop.run_until_complete(self.request(method, url, headers, body))
            else:
                return loop.run_until_complete(self.request(method, url, headers, body))
        except RuntimeError:
