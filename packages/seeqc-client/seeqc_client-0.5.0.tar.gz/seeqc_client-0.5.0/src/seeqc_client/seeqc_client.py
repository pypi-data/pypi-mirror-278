...
class Client:
    ...
    def run_emulator(self, qasm_path: Union[Path, str], target_system: str, num_shots: int = 1):
        """Request for a qasm input to be run on an emulator node"""
        run_result_response = self._run_emulator_request(qasm_path, target_system, num_shots)
    ...
    def _run_get_emulator_target_system_request(self):
        ...
        def wrapped_run_emulator(url: str, headers: dict):
            """Get the response from a qasm string upload to the emulator"""
            response = requests.get(f"{url}/api/v1/emulator",
                                     headers=headers)
            return response
        return wrapped_run_emulator()
