...
def ui(
    ...
    try:
        ...
        logger.error("hamilton[ui] not installed -- you have to install this to run the UI. "
                     "Run `pip install \"sf-hamilton[ui]\"` to install and get started with the UI!")
        raise typer.Exit(code=1)

    ctx.invoke(commands.run, port=port, base_dir=base_dir, no_migration=no_migration, no_open=no_open)
