...
from typing import Awaitable, Union, Self
#-
...
class ActionChains:
    ...
    def __init__(self, driver: WebDriver, duration: int = 250,
        ...
        if devices is not None and isinstance(devices, list):
            for device in devices:
                ...
                if isinstance(device, WheelInput):
                    wheel = device
        self.w3c_actions = ActionBuilder(driver, mouse=mouse, keyboard=keyboard,
                wheel=wheel, duration=duration)


    async def perform(self) -> Awaitable[None]:
        """Performs all stored actions."""
        await self.w3c_actions.perform()


    async def reset_actions(self) -> Awaitable[None]:
        """Clears actions that are already stored locally and on the remote
        end."""
        await self.w3c_actions.clear_actions()
        for device in self.w3c_actions.devices:
            device.clear_actions()


    def click(self, on_element: WebElement | None = None) -> Self:
        """Clicks an element.
    ...
    def click_and_hold(self, on_element: WebElement | None = None) -> Self:
    ...
    def context_click(self, on_element: WebElement | None = None) -> Self:
    ...
    def double_click(self, on_element: WebElement | None = None) -> Self:
    ...
    def drag_and_drop(self, source: WebElement, target: WebElement) -> Self:
    ...
    def drag_and_drop_by_offset(self, source: WebElement, xoffset: int,
            yoffset: int) \
            -> Self:
        """Holds down the left mouse button on the source element, then moves
    ...
    def key_down(self, value: str, element: WebElement | None = None) -> Self:
    ...
    def key_up(self, value: str, element: WebElement | None = None) -> Self:
    ...
    def move_to_element_with_offset(self, to_element: WebElement, xoffset: int,
            yoffset: int) \
            -> Self:
        ...
        self.w3c_actions.pointer_action.move_to(to_element, int(xoffset),
                int(yoffset))
        self.w3c_actions.key_action.pause()
    ...
    def pause(self, seconds: float | int) -> Self:
    ...
    def release(self, on_element: WebElement | None = None) -> Self:
    ...
    def send_keys(self, *keys_to_send: str) -> Self:
        ...
        :Args:
         - keys_to_send: The keys to send.  Modifier keys constants can be found
                         in the 'Keys' class.
        """
    ...
    def scroll_by_amount(self, delta_x: int, delta_y: int) -> Self:
        ...
        :Args:
         - delta_x: Distance along X axis to scroll using the wheel. A negative
                    value scrolls left.
         - delta_y: Distance along Y axis to scroll using the wheel. A negative
                    value scrolls up.
        """
    ...
    def scroll_from_origin(self, scroll_origin: ScrollOrigin, delta_x: int,
        ...
        :Args:
         - origin: Where scroll originates (viewport or element center) plus
                   provided offsets.
         - delta_x: Distance along X axis to scroll using the wheel. A negative
                    value scrolls left.
         - delta_y: Distance along Y axis to scroll using the wheel. A negative
        ...
        if not isinstance(scroll_origin, ScrollOrigin):
            raise TypeError(
                f"Expected object of type ScrollOrigin, "
                f"got: {type(scroll_origin)}"
            )
    ...
    # Context manager so ActionChains can be used in a 'async with .. as' statements.

    async def __aenter__(self) -> Awaitable[Self]:
        return self  # Return created instance of self.

    async def __aexit__(self, _type, _value, _traceback) -> Awaitable[None]:
