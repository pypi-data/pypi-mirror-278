...
:LastEditTime: 2024-06-12 14:44:42
...
class PayCallbackHandler(SevenBaseHandler):
    def post_async(self):
        ...
        if wechat_helper.check_notify_sign(timestamp,nonce,data,signature):
            ...
            self.send_notice(f"微信验签失败,data:{data}")
            return self.write("success")
...
class OrderListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        order_list,is_next = OrderModel().get_dict_page_list(field="order_id,status,add_time,real_pay_price,postage,coupon_price",page_index=page_index,page_size=page_size,where=condition,params=param_list,order_by="add_time desc",page_count_mode="next")
        if order_list:
            # 订单id
...
class ApplyRefundHandler(SevenBaseHandler):
    ...
    def post_async(self):
        ...
        # 添加退款订单
        refund_order_id = "T" + self.create_order_number()
...
class RefundDetailHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if refund_model["status"] == 1:
            address_model = ShopModelEx().get_shop_address(shop_id)
        
        refund_model["address_info"] = address_model
...
class RefundListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        refund_order_list,is_next = OrderRefundModel().get_dict_page_list(where="user_code = %s and shop_id = %s",params=[user_code,shop_id],page_index=page_index,page_size=page_size,field="refund_order_id,status,goods_refund_price,real_refund_price,postage",order_by="id desc",page_count_mode="next")
