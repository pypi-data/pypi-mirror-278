...
def GetStockItemInfoList():
    ...
    res = urlProc.requests_url_call(url, headers=common_headers)    
...
def GetDividendInfo(selYear, currentPageSize = 100, marketType='', settlementMonth='', yearCnt = 3, prtInd=False):
    ...
    while True:                
        ...
        res = urlProc.requests_url_call(url, headers=common_headers, prtInd=prtInd)
...
def GetTradeVolumeByInvestor(shCode, sDate, eDate, shName='', isuCd=''):
    ...
    res = urlProc.requests_url_call(url.format(shCode, shName, isuCd, isuCd, shName, sDate, eDate), headers=common_headers )
    ...
    res = urlProc.requests_url_call(url.format(eDate,  shCode, shName, isuCd, isuCd, shName, sDate, eDate), headers=common_headers )
    dList = json.loads(res.text)['output']
...
def GetShortSelling(shCode, sDate, eDate, shName='', isuCd=''):
    ...
    res = urlProc.requests_url_call(url.format(eDate, shCode, shName, isuCd, shCode, shName, sDate, eDate), headers=common_headers )
    dList = json.loads(res.text)['OutBlock_1']
