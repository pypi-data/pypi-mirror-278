...
class GaussianShapeSystematics(BaseSystematics):
    ...
    def get_systematics_data_with_mean_IQR_estimator(self, df_nom, df_sys,
                                                     xmin:float, xmax:float, n_bins:int,
        ...
        h_nom = TH1.from_numpy_data(x=x_nom, bins=n_bins, bin_range=(xmin, xmax),
                                    normalize=True, weights=weight_nom)
        h_sys = TH1.from_numpy_data(x=x_sys, bins=n_bins, bin_range=(xmin, xmax),
