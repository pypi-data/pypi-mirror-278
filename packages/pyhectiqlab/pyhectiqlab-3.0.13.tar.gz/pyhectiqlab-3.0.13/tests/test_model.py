...
def test_model_create_basic():
    from pyhectiqlab import Model
    model = Model.create(name="test", path="dummy")
    logger.info(f"Model created: {model}")

def test_model_create_cli():
    import os
    project = Project.get()
    os.system(f"hectiq-lab Model.create --name test --path dummy --project {project}")

def test_model_create_with_run():
...
def test_download(save_path):
    from pyhectiqlab import Model
    model = Model.download(name="test", 
                               version="1.2", 
                               project="hectiq-ai/test", 
                               path=save_path, 
                               wait_response=True)
