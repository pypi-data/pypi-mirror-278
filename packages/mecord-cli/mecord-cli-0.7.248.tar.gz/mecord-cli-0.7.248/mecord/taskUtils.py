...
task_counter_bak = os.path.join(os.path.dirname(os.path.abspath(__file__)), "task_counter_bak.txt")
def notifyCounterIfNeed(service_country):
    if os.path.exists(task_counter_file) == False:
        return
    with open(task_counter_file, 'r') as f:
        data = json.load(f)
    with open(task_counter_bak, 'w') as f:
        f.write("")
...
def saveCounter(taskUUID, service_country, duration, isSuccess):
    ...
    try:
        ...
        if os.path.exists(task_counter_bak):
            with open(task_counter_file, 'w') as f:
                json.dump({}, f)
            os.remove(task_counter_bak)
        with open(task_counter_file, 'r') as f:
            data = json.load(f)
        #update
