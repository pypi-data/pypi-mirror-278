...
from bsx_py_beta.client.rest.base import AuthRequiredClient
from bsx_py_beta.client.rest.market.types import Order as EIP712Order
from bsx_py_beta.common import X18_DECIMALS
from bsx_py_beta.common.acc_info import AccountInfo
from bsx_py_beta.common.types.market import CreateOrderParams, Side, CancelOrderResult, CancelMultipleOrdersParams, \
    CancelMultipleOrdersResult, OrderListingResult, GetOrderHistoryParams, BatchOrderParams
from bsx_py_beta.common.types.market import Order
...
class MarketClient(AuthRequiredClient):
    ...
    def batch_order(self, params: BatchOrderParams) -> Order:
        operations = []
        for p in params.operations:
            if isinstance(p, CreateOrderParams):
                ...
                operations.append({"type": "CREATE", "create_order_detail": payload})
            else:
                operations.append({"type": "CANCEL", "cancel_detail": {
                    "order_id": p.order_id,
                    "nonce": p.nonce,
                    "client_order_id": p.client_order_id
                }})

        resp = self.post(endpoint="/orders/batch", body={"operations": operations})
        return resp
