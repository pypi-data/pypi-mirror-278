...
def convert_google_apps_mime_to_office_mime(mime_type):
    mime_mapping = {
...
def download_file(service, file_id, file_path, modified_time, mime_type):
    try:
        file_path = truncate_filename(file_path)  # Truncate filename if necessary
...
def append_file_extension(file_path, mime_type):
    extensions = {
...
def drive_download(service_account_file, parent_folder_id, max_files, output_folder='./data', force=False):
    ...
    print("== Begin downloading files from Google Drive v4.3.10.a ==")
