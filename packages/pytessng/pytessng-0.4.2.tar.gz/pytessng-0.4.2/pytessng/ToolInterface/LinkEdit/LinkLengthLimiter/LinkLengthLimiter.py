...
from ...public.Line.LinkPointsDivider import LinkPointsDivider
from ...public.Line.LinkPointsSplitter import LinkPointsSplitter
...
class LinkLengthLimiter(BaseLinkEditor):
    ...
    def edit(self, max_link_length: float = LinkEditConfig.DEFAULT_MAX_LINK_LENGTH, min_connector_length: float = LinkEditConfig.DEFAULT_MIN_CONNECTOR_LENGTH):
        # 获取分割信息
    ...
    def delete_links(self, link_groups: dict, result_create_links: dict) -> None:
        ...
        for link_id, link_group in pgd.progress(link_groups.items(), "原有路段及连接段删除中（7/7）"):
            new_link_ids = [result_create_links[link_id_] for link_id_ in link_group]
