...
from .utils import generate_random_string, LanesPointsCalculator
from ...public.Line.CenterPointCalculator import CenterPointCalculator
from ...public.Line.LinkPointsDeviator import LinkPointsDeviator
...
class ShapeNetworkAnalyser(BaseNetworkAnalyser):
    ...
    def analyse_all_data(self, network_data, params: dict = None) -> dict:
        ...
        if not is_use_center_line:
            links_data = self.analyse_gradient_section(links_data)
        else:
            # 虚拟进度条
        ...
        standard_connectors_data = [
            dict(
                ...
                points=[lane["points"] for lane in connector_data] if all([lane.get("points") for lane in connector_data]) else None
            )
    ...
    def analyse_links(self, lanes_data: tuple, file_proj_string: str, is_use_lon_and_lat: bool, is_use_center_line: bool, proj_mode: str) -> (dict, dict):
        ...
        if is_use_lon_and_lat:
            x_0, y_0 = CenterPointCalculator.calculate_center_point(min(x_list), max(x_list), min(y_list), max(y_list))
        else:
        ...
        for road_id in pgd.progress(list(links_info.keys()), "数据解析中（2/9）"):
            ...
            new_lanes_points = LanesPointsCalculator.calc_lanes_with_same_points(
                lanes_points,
            ...
            if is_use_center_line:
                ...
                for lane_id, lane in links_info[road_id]["lanes_data"].items():
                    ...
                    lane_points_threeLine = LinkPointsDeviator.deviate_points(lane_points_centerLine, {
    ...
    def analyse_connectors(self, lane_connectors_data: tuple, lanes_info) -> dict:
        ...
        for index, laneConnector_data_shp in pgd.progress(enumerate(all_data_shp_laneConnector), "数据读取中（3/9）"):
            ...
            lane_points_threeLine = LinkPointsDeviator.deviate_points(points_3D, {
                "right": ['right', lane_width * 0.5],
                "center": ['right', lane_width * 0],
                "left": ['right', lane_width * -0.5],
            })
    ...
    def analyse_gradient_section(self, links_info: dict):
