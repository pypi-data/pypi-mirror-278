import copy
from PySide2.QtWidgets import QLabel, QRadioButton, QPushButton, QComboBox, QTextBrowser, QDockWidget
...
class FileExportGrid(BaseUserInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 按钮
        self.action = GlobalVar.action_file_export_grid
        # 浮动组件
    ...
    def show(self):
        if self.action.isChecked():
            ...
            self.dockWidget = QDockWidget(self.name, win)
            # self.dockWidget.setTitleBarWidget(QWidget())  # 隐藏标题栏
            self.dockWidget.setFeatures(QDockWidget.NoDockWidgetFeatures)  # 不能浮动
            self.dockWidget.setAllowedAreas(Qt.LeftDockWidgetArea)  # 只能停靠在左侧区域
            self.dockWidget.setWidget(self)
            self.iface.guiInterface().addDockWidgetToMainWindow(Qt.DockWidgetArea(1), self.dockWidget)
    ...
    def close(self, actual_close: bool = False):
        ...
        if actual_close:
            ...
            # 移除左侧界面
            if self.dockWidget:
                self.iface.guiInterface().removeDockWidgetFromMainWindow(self.dockWidget)
    ...
    def start_select_points(self):
        # 结束选择
        if GlobalVar.is_need_select_file_export_grid == True:
            # 设置按钮文字
            self.button_select.setText("选择")
            # 设置全局变量
            GlobalVar.is_need_select_file_export_grid = False
        ...
        # 开始选择
            ...
            # 设置全局变量
            GlobalVar.is_need_select_file_export_grid = True
            ...
            GlobalVar.function_file_export_grid = self.scene.handle_mouse_event
