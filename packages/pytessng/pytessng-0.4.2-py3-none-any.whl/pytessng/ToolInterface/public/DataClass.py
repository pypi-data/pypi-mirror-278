...
class Connector(BaseModel):
    ...
    def check(cls, values):
        ...
        values["from_lane_numbers"] = list(from_lane_numbers)
        values["to_lane_numbers"] = list(to_lane_numbers)
        return values
...
class DecisionPoint(BaseModel):
    ...
    def check(cls, values):
        ...
        assert len(routings) == len(ratios), "routings 和 ratios 的长度必须一致"
        return values
