...
try:
    ...
    EXTRA_LLM_DEPS_INSTALLED = True
except ImportError:
    EXTRA_LLM_DEPS_INSTALLED = False
...
###############################################################################
# SentenceEmbeddingBlocking ###################################################
###############################################################################

if EXTRA_LLM_DEPS_INSTALLED:

    class SentenceEmbeddingBlocking(BaseBlocking):
        def __init__(self, ref_attr_index, target_attr_index, model_name, threshold):
            super().__init__(ref_attr_index, target_attr_index)
        ...
        def _item_to_document(self, item, settype, item_idx):
            index = self.ref_attr_index if settype == "ref" else self.target_attr_index
            return Document(page_content=item[index], metadata=dict(idx=item_idx))

        def _fit(self, refset, targetset):
            """Fit a dataset in an index using the callback"""
            self._set_to_embed = refset if len(refset) < len(targetset) else targetset
            self._set_to_compare = refset if len(refset) >= len(targetset) else targetset

            self._set_to_embed_label = "ref" if len(refset) < len(targetset) else "target"
            self._set_to_compare_label = "ref" if len(refset) >= len(targetset) else "target"

            print("Prepare Vectors database")

            # How to use cosine similarity instead of Euclidean sim ?
            self._db = FAISS.from_documents(
                [
                    self._item_to_document(item, self._set_to_embed_label, idx)
                    for idx, item in enumerate(self._set_to_embed)
                ],
                self.embedding,
            )

            print("Vectors database ready")
        ...
        def _iter_blocks(self):
            ...
            index_compare = (
                self.ref_attr_index
                if self._set_to_compare_label == "ref"
                else self.target_attr_index
            )
            for idx, item in enumerate(
                tqdm(self._set_to_compare, desc="Perform blocking using embeddings DB")
            ):
                candidates_with_score = self._db.similarity_search_with_score(item[index_compare])
                docs = []
                for doc, score in candidates_with_score:
                    if score < self.threshold:
                        doc_idx = doc.metadata["idx"]
                        docs.append((doc_idx, self._set_to_embed[doc_idx][0]))
                if docs:
                    if self._set_to_compare_label == "ref":
                        yield ([(idx, item[0])], docs)
                    else:
                        yield (docs, [(idx, item[0])])
        ...
        def _cleanup(self):
            ...
            self.target_index = {}
            del self._db
            del self._set_to_compare
            del self._set_to_embed
    ...
    class SentenceEmbeddingBlocking(BaseBlocking):
        def __init__(self, ref_attr_index, target_attr_index, model_name, threshold):
            raise ImportError(
                "you need to install 'nazca[llm]' to use the SentenceEmbeddingBlocking blocking"
            )
