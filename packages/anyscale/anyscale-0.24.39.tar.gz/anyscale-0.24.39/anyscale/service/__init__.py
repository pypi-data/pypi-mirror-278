...
class ServiceSDK:
    ...
    def rollback(  # noqa: F811
        ...
        return self._private_sdk.rollback(
            name=name, max_surge_percent=max_surge_percent
        )

    @sdk_docs(
        doc_py_example=_TERMINATE_EXAMPLE, arg_docstrings=_TERMINATE_ARG_DOCSTRINGS,
    )
    def terminate(self, name: Optional[str] = None) -> str:  # noqa: F811
        """Terminate a service.
        ...
        """
        return self._private_sdk.terminate(name=name)

    @sdk_docs(
        doc_py_example=_STATUS_EXAMPLE, arg_docstrings=_STATUS_ARG_DOCSTRINGS,
    )
    def status(self, name: str,) -> ServiceStatus:  # noqa: F811
        """Get the status of a service."""
        return self._private_sdk.status(name=name,)
