...
class WebullOptions:
    def __init__(self, database:str='market_data', user:str='chuck'):
        ...
        self.headers = {
            "Access_token": "dc_us_tech1.18fe642781e-6fcbd45260cd406b895661fef3c1211c",
            "App": "global",
            "App-Group": "broker",
            "Appid": "wb_web_app",
    ...
    async def all_options(self, ticker, direction='all', expireDate =  datetime.now()):
        try:
      
            # Calculate the nearest Friday
            today = datetime.now()
            nearest_friday = today + timedelta((4-today.weekday()) % 7)  # 4 represents Friday
            expireDate == today if expireDate == None else today
                
            ticker_id = await self.get_ticker_id(ticker)

            params = {"tickerId":ticker_id,"count":-1,"direction":direction,"type":0,"quoteMultiplier":100,"expireDate":expireDate,"unSymbol":ticker}
