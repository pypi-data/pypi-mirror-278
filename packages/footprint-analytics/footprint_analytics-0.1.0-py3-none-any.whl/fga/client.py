...
class FGA:
    ...
    def track(self, event: BaseEvent):
        ...
        if type(event) == 'Event':
            if any(x is None for x in [event.event_id, event.event_name, event.user_id, event.event_type]):
                raise InvalidEventError("Invalid event properties.")
        if type(event) == 'Asset':
            if any(x is None for x in [event.event_id, event.user_id, event.source_id, event.source_type, event.type]):
                raise InvalidEventError("Invalid asset properties.")
        if type(event) == 'Session':
            if any(x is None for x in [event.event_id, event.user_id, event.session_id]):
