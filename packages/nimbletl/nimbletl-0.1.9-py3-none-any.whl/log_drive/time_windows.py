...
def save_log_for_time_window_2_param(etl_name: str, datetime_tmp: Tuple[datetime, datetime], target_table_name: str,
    ...
    log_drive_table = LogDriveTable(etl_name=etl_name, target_table_name=target_table_name, drive_type=drive_type.value,
                  drive_value=convert_tuple_to_string(datetime_tmp),
                  process_start_time=process_start_time, etl_result=EtlStatusEnum.RUNNING)
    save_obj(log_drive_table)
...
if __name__ == '__main__':
    ...
    time_interval = 1440
    target_table_name = ''
    drive_type = LogDriveType.TIME_WINDOW_2_PARAM
    save_log_for_time_window_2_param(etl_name, datetime_tmp, target_table_name, drive_type)
