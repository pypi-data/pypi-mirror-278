...
class Assistant(PineconePlugin):
    ...
    def __init__(self, config, client_builder):
        ...
        self.host = os.getenv("PINECONE_PLUGIN_ASSISTANT_CONTROL_HOST", "https://api-staging.pinecone.io")
        if self.host.endswith("/"):
            self.host = self.host[:-1]
        self.assistant_control_api = client_builder(ApiClient, ControlApiClient, 'unstable', host=self.host)
    ...
    def create_assistant(
        ...
        assistant = self.assistant_control_api.create_assistant(inline_object=inline_object)

        if timeout == -1:
            # still in processing state
            return AssistantModel(assistant=assistant, client_builder=self.client_builder, config=self.config)
        if timeout is None:
    ...
    def describe_assistant(
        ...
        assistant = self.assistant_control_api.get_assistant(assistant_name=assistant_name)
        return AssistantModel(assistant=assistant, client_builder=self.client_builder, config=self.config)
    ...
    def list_assistants(
        ...
        >>> print(model)
         [{'created_at': '2024-06-02T19:01:17Z',
          ...
          'updated_at': '2024-06-02T19:01:17Z'}]
        """ 
        assistants_resp = self.assistant_control_api.list_assistants()
        return [AssistantModel(assistant=assistant, client_builder=self.client_builder, config=self.config) for assistant in assistants_resp.assistants]
    ...
    def delete_assistant(
        ...
        self.assistant_control_api.delete_assistant(assistant_name=assistant_name)
