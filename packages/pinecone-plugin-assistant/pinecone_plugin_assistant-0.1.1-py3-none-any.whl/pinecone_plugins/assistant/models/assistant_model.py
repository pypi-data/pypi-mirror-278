...
class AssistantModel:
    def __init__(self, assistant: OpenAIAssistantModel, client_builder, config):
        ...
        self.host = os.getenv("PINECONE_PLUGIN_ASSISTANT_DATA_HOST", "https://staging-data.ke.pinecone.io")
        self.config = config if config else {}

        if self.host.endswith("/"):
            self.host = self.host[:-1]
        self.assistant_data_api = client_builder(ApiClient, DataApiClient, 'unstable', host=self.host)
    ...
    def upload_file(
        ...
        try:
            with open(file_path, 'rb') as file:
                upload_resp = self.assistant_data_api.upload_file(assistant_name=self.assistant.name, file=file)
    ...
    def describe_file(self, file_id: str) -> FileModel:
        ...
        file = self.assistant_data_api.describe_file(assistant_name=self.name, kb_file_id=file_id)
    ...
    def list_files(self) -> List[FileModel]:
        ...
        >>> km.list_files()
          ...
          'updated_on': '2024-06-02T19:48:00Z'}, ...]
        """
        files_resp = self.assistant_data_api.list_files(self.name)
    ...
    def delete_file(
        ...
        >>> km.list_files()
          []
        """
        self.assistant_data_api.delete_file(assistant_name=self.name, kb_file_id=file_id)
    ...
    def _chat_completions_single(self, messages: List[Message]) -> ChatResultModel:
        ...
        search_result = self.assistant_data_api.chat_completion_assistant(
