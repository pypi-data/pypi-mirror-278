...
void Op_PNS::forward(Eigen::VectorXd &X, Eigen::VectorXd &out, 
...
void Op_PNS::transpose(Eigen::VectorXd &X, Eigen::VectorXd &out, 
...
void Op_PNS::prox(Eigen::VectorXd &X)
{
    for (int i = 0; i < (N-1); i++) {
        double upper_bound = balance_mod(0) * (target(0)+tol(0));
...
void Op_PNS::check(Eigen::VectorXd &X, int iiter)
{
    double check = 0.0;
