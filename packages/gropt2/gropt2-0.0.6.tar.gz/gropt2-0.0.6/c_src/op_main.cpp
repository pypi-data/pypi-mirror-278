...
void GroptOperator::forward(Eigen::VectorXd &X, Eigen::VectorXd &out, bool apply_weight, int norm, bool no_balance)
{
    out = X;
}


void GroptOperator::transpose(Eigen::VectorXd &X, Eigen::VectorXd &out, bool apply_weight, int norm, bool repeat_balance)
{
...
void GroptOperator::add2b(Eigen::VectorXd &b)
{
    ...
    transpose(Ax_temp, x_temp, false, 2, false);
    b += x_temp;
}
...
{   
    ...
    if (!(row_constraints)) {
        ...
        b.array() = U0.array() + weight.array()*Y0.array();
    }
