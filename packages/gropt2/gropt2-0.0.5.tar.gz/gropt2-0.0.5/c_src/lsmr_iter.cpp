...
void get_Atx(std::vector<GroptOperator*> all_op, std::vector<GroptOperator*> all_obj, Eigen::VectorXd &in, Eigen::VectorXd &out) {
    ...
    for (int i = 0; i < all_op.size(); i++) {
        ...
        all_op[i]->transpose(all_op[i]->Ax_temp, all_op[i]->x_temp, true, 0, true);
        out += all_op[i]->x_temp;
        ii += all_op[i]->Ax_size;
    }
    ...
    for (int i = 0; i < all_obj.size(); i++) {
        ...
        all_obj[i]->transpose(all_obj[i]->Ax_temp, all_obj[i]->x_temp, true, 0, true);
...
void get_Ax(std::vector<GroptOperator*> all_op, std::vector<GroptOperator*> all_obj, Eigen::VectorXd &in, Eigen::VectorXd &out) {
    ...
    for (int i = 0; i < all_op.size(); i++) {
        ...
        all_op[i]->forward(in, all_op[i]->Ax_temp, true, 0, false);
    ...
    for (int i = 0; i < all_obj.size(); i++) {
        ...
        all_obj[i]->forward(in, all_obj[i]->Ax_temp, true, 0, false);
    ...
    : n(n), maxiters(maxiters)
{
    ...
    // We could consider making A a big square instead of a column, so x is mutiple repeats of itself...
...
{   
    ...
    //             savename.str("");
    //             savename << "lsmr_A_row" << j << "_ii" << iiter << ".bin" ;
    //             write_vector_xd(savename.str(), all_op[i]->A.row(j));
    //         }
    //     }
    // }

    // get_Ax(all_op, all_obj, x0, utemp);  // Delete this
    // savename.str("");
    // savename << "lsmr_Ax_ii" << iiter << ".bin" ;
    // write_vector_xd(savename.str(), utemp);
    ...
    // savename.str("");
    // savename << "lsmr_b_ii" << iiter << ".bin" ;
    // write_vector_xd(savename.str(), b);

    // get_Atx(all_op, all_obj, u, v); // Delete this
    // savename.str("");
    // savename << "lsmr_Atb_ii" << iiter << ".bin" ;
    // write_vector_xd(savename.str(), v);
    ...
    // log_print(LOG_DEBUG, "LSMR   beta = %8.1e   alpha = %8.1e",beta, alpha);
    ...
    while (iter < maxiters) {
        ...
        // log_print(LOG_DEBUG, "LSMR   x update  (zeta/(rho*rhobar)) = %8.1e   hbarnorm = %8.1e   xnorm = %8.1e",(zeta/(rho*rhobar)), hbar.norm(), x.norm());
        ...
        // log_print(LOG_DEBUG, "-- iter = %d --",iter);
        // log_print(LOG_DEBUG, "LSMR   normr = %8.1e   normb = %8.1e",normr, normb);
        // log_print(LOG_DEBUG, "LSMR   normAr = %8.1e   normA = %8.1e",normAr, normA);

        // check tests
        if ((iter > 2) && (test3 <= ctol || test2 <= a_tol || test1 <= rtol) ) {
    ...
    // savename.str("");
    // savename << "lsmr_x1_ii" << iiter << ".bin" ;
    // write_vector_xd(savename.str(), x);

    return x; 
}

int LSMR_Iter::get_n_iter()
{
    return n_iter;
}
