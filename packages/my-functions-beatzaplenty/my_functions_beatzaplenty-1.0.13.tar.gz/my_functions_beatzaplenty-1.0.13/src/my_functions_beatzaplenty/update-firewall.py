...
def update_config(config_file, ip_address, tzinfo, tz_string_format):
    ...
    config_items["last_change"] = datetime.datetime.now(tzinfo).strftime(tz_string_format)
    with open(config_file, "w") as config:
        edit.write(config)

# Function to update firewall rules
def update_firewall_rules(current_ip, last_ip, rules_config, log_file):
...
def update_before_rules(current_ip, config_file, log_file):
    ...
    # Use sudo to read the before.rules file content
    read_cmd = f"sudo cat {before_rules_path}"
    result = subprocess.run(read_cmd, shell=True, capture_output=True, text=True, check=True)
    ...
    # Use sudo to write the updated content back to the before.rules file
    temp_file_path = "/tmp/before.rules.tmp"
    with open(temp_file_path, "w") as temp_file:
        temp_file.write("\n".join(updated_lines) + "\n")

    write_cmd = f"sudo cp {temp_file_path} {before_rules_path}"
    subprocess.run(write_cmd, shell=True, check=True)
    
    # Remove the temporary file
    subprocess.run(f"sudo rm {temp_file_path}", shell=True)

    # Restart the ufw service to apply the changes
    subprocess.run(["sudo", "ufw", "reload"], check=True)
    
    # Log the updates
    log_file.write(f"Updated before.rules with IP {current_ip}/32.\n")
    if input_line_updated:
        log_file.write(f"Updated input line: {input_line_identifier} -s {new_ip_with_cidr}\n")
    if forward_line_updated:
        log_file.write(f"Updated forward line: {forward_line_identifier} -s {new_ip_with_cidr}\n")
...
def main():
    ...
    with open(LOG_FILE, "w") as log_file:
        ...
        if current_ip != last_ip:
            ...
            update_firewall_rules(current_ip, last_ip, rules_config, log_file)
            # Call the new function to update before.rules
            update_before_rules(current_ip, f'{parent_dir}/config.ini', log_file)
