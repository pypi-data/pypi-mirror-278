...
class Ok(Generic[T], _Resultable[T, Any]):
    ...
    def unwrap_or_else(self, default_fn: Callable[[object], T]) -> T:
        return self.value
    ...
    def map_or_else(self, default_fn: object, fn: Callable[[T], R]) -> R:
        return fn(self.value)

    def map_error(self, fn: object) -> Self:
        return self

    def and_then(self, fn: Callable[[T], _Resultable[U, Any]]) -> _Resultable[U, Any]:
        return fn(self.value)

    def or_else(self, fn: object) -> Self:
        return self
...
class Error(Exception, _Resultable[Any, "Error"]):
    ...
    def map_or(self, default: R, fn: object) -> R:
        return default

    def map_or_else(self, default_fn: Callable[[Error], R], fn: object) -> R:
        return default_fn(self)

    def map_error(self, fn: Callable[[Error], R]) -> R:
        return fn(self)

    def and_then(self, fn: object) -> Self:
        return self
...
class ForeignError(Generic[ExceptionT], _Resultable[Any, "ForeignError[ExceptionT]"]):
    ...
    def map_or_else(
        self, default_fn: Callable[[ForeignError[ExceptionT]], R], fn: object
    ) -> R:
        return default_fn(self)
