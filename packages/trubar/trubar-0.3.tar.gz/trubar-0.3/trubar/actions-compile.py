...
class StringTranslator(cst.CSTTransformer):
    def __init__(self, context: MsgDict, module: cst.Module):
        ...
        self.concatenated_string_stack: List[cst.SimpleString] = []
    ...
    def __translate(
        ...
        if config.auto_prefix \
            ...
            try:
                new_node = cst.parse_expression(
                    f'f{prefix}{quote}{translation}{quote}')
        ...
        try:
            if "f" in prefix + node.prefix:
                trans, transe = '_tr_eval(_tr_compile(', ', "", "eval"))'
            else:
                trans = transe = ""
            nq = "\"'"[quote[0] == '"']
            enq = ("\\" + nq) * len(quote)
            rnq = rf"(?<!\\){nq}" * len(quote)
            nq *= len(quote)
            expr = f"""{trans}{{"si": r{nq}{prefix}{quote}{re.sub(rnq, enq, translation)}{quote}{nq},
                }}.get(lang(), r{nq}{node.prefix}{quote}{re.sub(rnq, enq, original)}{quote}{nq})
                {transe}"""
            print(expr)
    ...
    def visit_ConcatenatedString(
            self,
            node: cst.ConcatenatedString) -> bool:
        return False
    ...
    def leave_ConcatenatedString(
        ...
        node = updated_node
        while True:
            nodes.append(self.__translate(node.left, node.left))
            if not isinstance(node.right, cst.ConcatenatedString):
                nodes.append(self.__translate(node.right, node.right))
                break
            node = node.right

        #if not any(isinstance(node, cst.FormattedString) for node in nodes):
         #   return updated_node

        node = None
        for next in reversed(nodes):
            if node is None:
                node = next
            else:
                node = cst.BinaryOperation(
                    ...
                    operator=cst.Add(),
                    right=node)
        #node.lpars = cst.LeftParen()
        #node.rpars = cst.RightParen()
        return node

    def leave_ConcatenatedString1(
            self,
            original_node: cst.ConcatenatedString,
            updated_node: cst.ConcatenatedString) -> cst.CSTNode:
        node = cst.BinaryOperation(
            left=self.__translate(original_node.left, updated_node.left),
            operator=cst.Add(),
...
def translate(translations: MsgDict,
    ...
    for name, fullname in walk_files(source, pattern, select=False):
        ...
        try:
            ...
            raise
        trans_source = 'from AnyQt.QtCore import QSettings\ndef lang(): return {"English": "en", "Slovenian": "si"}[QSettings().value("general/language", "English", type=str)]\n' \
            + 'from builtins import eval as _tr_eval, compile as _tr_compile\n\n' \
            + trans_source
