...
class StringTranslator(cst.CSTTransformer):
    message_tables: Dict[str, List[str]] = {"en": [], "si": []}
    ...
    def __translate(
        ...
        # If the translation contains a brace and auto_prefix is on, test if
        # we can and need to use an f-string
        prefix = node.prefix
        trans_str = f"{prefix}{quote}{translation}{quote}"
        auto_f_ed = False
        if config.auto_prefix \
                and "f" not in node.prefix and not re_braced.search(original) \
                and re_braced.search(translation) :
            try:
                new_node = cst.parse_expression('f' + trans_str)
        ...
        # Otherwise, test that the translation is a valid string
        if not auto_f_ed:
            try:
                trans_str = f"{prefix}{quote}{translation}{quote}"
                cst.parse_expression(trans_str)
            except cst.ParserSyntaxError:
        ...
        idx = len(self.message_tables['en'])
        if prefix or node.prefix:
            trans = f'_tr.e(_tr.c({idx}))'
            self.message_tables["en"].append(orig_str)
            self.message_tables["si"].append(trans_str)
        else:
            trans = f"_tr.m[{idx}]"
            self.message_tables["en"].append(original)
            self.message_tables["si"].append(translation)
    ...
    def leave_ConcatenatedString(
        ...
        def compose_concatenation(node: cst.ConcatenatedString):
            left = self.__translate(node.left, node.left)
            if isinstance(node.right, cst.ConcatenatedString):
                right = compose_concatenation(node.right)
            else:
                right = self.__translate(node.right, node.right)
...
def translate(translations: MsgDict,
    ...
    if not any_reports and verbosity > ReportCritical:
        print("No changes.")
    for code, lang in (("en", "English"), ("si", "Slovenian")):
        json.dump(
            StringTranslator.message_tables[code],
            open(os.path.join(destination, f"{lang}.json"), "wt")
        )
