...
if os_type == "Windows":
    DLL_FILES = [pkg_resources.resource_filename('reportXlsx', f'resources/windows/{dll_name}') for dll_name in [
        'Spire.XLS.dll','SkiaSharp.dll','Aspose.Cells.dll'
    ]]
    Dll_CLASS = [pkg_resources.resource_filename('reportXlsx',f'resources/windows/{class_name}') for class_name in ['Spire.XLS','SkiaSharp','Aspose.Cells']]
else:
    ...
    Dll_CLASS = [pkg_resources.resource_filename('reportXlsx',f'resources/linux/{class_name}') for class_name in ['Spire.XLS','SkiaSharp','Aspose.Cells']]

import clr
clr.AddReference(DLL_FILES[0])
clr.AddReference(DLL_FILES[1])
clr.AddReference(DLL_FILES[2])
