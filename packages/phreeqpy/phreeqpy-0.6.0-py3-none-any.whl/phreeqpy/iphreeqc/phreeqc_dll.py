...
class IPhreeqc(object):
    ...
    def __init__(self, dll_path=None):
        ...
        method_mapping = [
            (
                '_accumulate_line',
                phreeqc.AccumulateLine,
                [c_int, ctypes.c_char_p],
                c_int,
            ),
            ('_add_error', phreeqc.AddError, [c_int, ctypes.c_char_p], c_int),
            ('_add_error', phreeqc.AddWarning, [c_int, ctypes.c_char_p], c_int),
            (
            ...
            ('_destroy_iphreeqc', phreeqc.DestroyIPhreeqc, [c_int], c_int),
            (
                '_get_component',
                phreeqc.GetComponent,
                [c_int, c_int],
                ctypes.c_char_p,
            ),
            ('_get_component_count', phreeqc.GetComponentCount, [c_int], c_int),
            (
                '_get_error_string',
                phreeqc.GetErrorString,
                [c_int],
                ctypes.c_char_p,
            ),
            (
                '_get_selected_output_column_count',
                phreeqc.GetSelectedOutputColumnCount,
                [c_int],
                c_int,
            ),
            (
                '_get_selected_output_row_count',
                phreeqc.GetSelectedOutputRowCount,
                [c_int],
                c_int,
            ),
            (
                ...
                [c_int, c_int, c_int, ctypes.POINTER(VAR)],
                c_int,
            ),
            (
                '_load_database',
                phreeqc.LoadDatabase,
                [c_int, ctypes.c_char_p],
                c_int,
            ),
            (
                '_load_database_string',
                phreeqc.LoadDatabaseString,
                [c_int, ctypes.c_char_p],
                c_int,
            ),
            ('_run_string', phreeqc.RunString, [c_int, ctypes.c_char_p], c_int),
            (
                '_set_selected_output_file_on',
                phreeqc.SetSelectedOutputFileOn,
                [c_int, c_int],
                c_int,
            ),
            (
                '_set_output_file_on',
                phreeqc.SetOutputFileOn,
                [c_int, c_int],
                c_int,
            ),
        ]
    ...
    def run_string(self, cmd_string):
        ...
        errors = self._run_string(
            self.id_, ctypes.c_char_p(cmd_string.encode('utf-8'))
        )
        if errors != 0:
            self.raise_string_error(errors)
...
class VAR(ctypes.Structure):
    ...
    _fields_ = [('type', ctypes.c_int), ('value', VARUNION)]
