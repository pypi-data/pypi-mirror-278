...
from nomad_media_pip.src.admin.content_definition.get_content_definition_group import _get_content_definition_group
...
def _update_content_definition(self, AUTH_TOKEN, URL, ID, NAME, CONTENT_FIELDS, CONTENT_DEFINITION_GROUP, 
    ...
    if CONTENT_DEFINITION_GROUP:
        CONTENT_DEFINIITON_GROUPS = _get_content_definition_group(self, AUTH_TOKEN, URL, DEBUG)
        CONTENT_DEFINITION_GROUP_INFO = [group for group in CONTENT_DEFINIITON_GROUPS["items"] if group["title"] == CONTENT_DEFINITION_GROUP][0]
    else:
    ...
    if CONTENT_DEFINITION_TYPE:
        ...
        CONTENT_DEFINITION_TYPE_INFO = [type for type in CONTENT_DEFINITION_TYPES["items"] if type["description"] == CONTENT_DEFINITION_TYPE][0]
    else:
        CONTENT_DEFINITION_TYPE_INFO = None

    try:
        CONTENT_DEFINITION_INFO = _get_content_definition(self, AUTH_TOKEN, URL, ID, DEBUG)
        if DISPLAY_FIELD: DISPLAY_FIELD_INFO = [field for field in CONTENT_DEFINITION_INFO["contentFields"] if field["properties"]["title"] == DISPLAY_FIELD][0]
        if ROUTE_ITEM_NAME_FIELD: ROUTE_ITEM_NAME_FIELD_INFO = [field for field in CONTENT_DEFINITION_INFO["contentFields"] if field["properties"]["title"] == ROUTE_ITEM_NAME_FIELD][0]
    except:
        CONTENT_DEFINITION_INFO = None
        DISPLAY_FIELD_INFO = [field for field in CONTENT_FIELDS if field["properties"]["title"] == DISPLAY_FIELD][0] if DISPLAY_FIELD else None
        ROUTE_ITEM_NAME_FIELD_INFO = [field for field in CONTENT_FIELDS if field["properties"]["title"] == ROUTE_ITEM_NAME_FIELD][0] if ROUTE_ITEM_NAME_FIELD else None
