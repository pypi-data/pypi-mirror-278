...
def deduplicate(
        ...
        search_key="sha256",
        preview_top=15,
        id_column_name=None, # Id column name for grouping inside the dedup report. Can be Test Case ID, Jira ID etc.
                ...
                if cutoff_op(score, cutoff):
                    ...
                    if isinstance(doc_a_data, str):
                        ...
                        for col in doc_a_data.keys():
                            # For the Id columns in the initial document don't do the difference analysis since they are used only for grouping the results
                            if id_column_name and col == id_column_name:
