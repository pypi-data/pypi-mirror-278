...
config_file_path = os.path.join(package_dir, 'neuromeka_indycare', 'indycare_utils', 'config.yml')
ngrok_file_path = os.path.join(package_dir, 'neuromeka_indycare', 'setup', 'ngrok-v3-stable-linux-386.tgz')
ngrok_executable_path = '/usr/bin/ngrok'
install_script_path = os.path.join(package_dir, 'neuromeka_indycare', 'daliworks_software', 'install.sh')
create_service_path = os.path.join(package_dir, 'neuromeka_indycare', 'create_service.sh')
...
def main():
    ...
    run_command(f'echo {passwd} | sudo -S -k pip3 install -r {os.path.join(base_path, "setup/requirements.txt")}',
                "Install Python requirements")
    ...
    for file in daliworks_files:
        file_path = os.path.join(base_path, 'daliworks_software', file)
