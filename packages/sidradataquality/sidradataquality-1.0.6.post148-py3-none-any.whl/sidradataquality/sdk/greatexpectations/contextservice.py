...
class ContextService():
    ...
    def get_context(self, provider, entity, asset_id, intake_run_id, execution_date):
        ...
        if (intake_run_id is None or intake_run_id == ''):
            ...
            self.context_path = self._get_context_path(f'{provider.database_name}/{entity.table_name}')
            storage_entity_path = f'{provider.database_name}/{entity.table_name}/{execution_date.strftime("%Y/%m/%d")}'
            ...
            self.base_path = f"{self.report_path}/{entity.table_name}-{asset_id}.html"
