...
from llm.optimized.inference.constants import VLLMSpecialModels, ExtraParameters
...
class VLLMEngine(BaseEngine):
    ...
    def server_url(self) -> str:
        ...
        logger.debug(f"server_url: http://{self._vllm_args['host']}:{self._vllm_args['port']}")
    ...
    def _load_vllm_defaults(self):
        ...
        # TODO: Allow all env variables to overwrite any kwargs
        if "tensor-parallel-size" in kwargs:
            ...
            if user_tensor_parallel:
                ...
                kwargs.update({"tensor-parallel-size": user_tensor_parallel})
                if user_tensor_parallel < custom_tensor_parallel:
                    logger.warning(f"Expected tensor parallel for this model is {custom_tensor_parallel}, however, "
                                   f"the user inputted tensor parallel of {user_tensor_parallel}. This may result "
                                   f"in deployment failures")
