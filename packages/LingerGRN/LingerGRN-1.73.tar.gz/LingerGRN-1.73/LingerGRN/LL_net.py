...
def cell_type_specific_TF_RE_binding_chr(adata_RNA,adata_ATAC,GRNdir,chrN,genome,celltype,outdir,method,mat):
    ...
    RE=pd.DataFrame(adata_ATAC.X.toarray().T[:,np.array(label)==celltype].mean(axis=1),index=adata_ATAC.var['gene_ids'].values,columns=['values'])
    TG=pd.DataFrame(adata_RNA.X.toarray().T[:,np.array(label)==celltype].mean(axis=1),index=adata_RNA.var['gene_ids'].values,columns=['values'])
## cell annotation
## extact the overlapped peaks.
    RE=RE.loc[N_overlap]
    TFbinding=load_TFbinding(GRNdir,O_overlap,O_overlap_u,O_overlap_hg19_u,chrN)
