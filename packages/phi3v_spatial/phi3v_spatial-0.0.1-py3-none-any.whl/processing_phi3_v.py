...
class Phi3VProcessor(ProcessorMixin):
    ...
    def __init__(self, image_processor, tokenizer):
        self.image_processor: Phi3VImageProcessor = image_processor
        self.tokenizer: LlamaTokenizerFast = tokenizer
    ...
    def __call__(
        ...
        #     "image_sizes": [],
        # }
        # for t, i in zip(text, image_inputs):
    ...
    def _convert_images_texts_to_inputs(self, images, texts, padding=False, truncation=None, max_length=None, return_tensors=None):
        ...
        for image, text in zip(images, texts):
            # print(type(image), len(image['pixel_values']), type(image['pixel_values']), len(image['image_sizes']), type(image['image_sizes']))
            # print(text)
            pattern = r"<\|image_\d+\|>"
            ...
            image, image_sizes = image['pixel_values'], image['image_sizes']
            ...
            assert len(unique_image_ids) == len(image), f"total images must be the same as the number of image tags, got {len(unique_image_ids)} image tags and {len(image)} images"
        ...
        # print(len(input_ids[0]))
            ...
            image_sizes_batch.append(image_sizes)
            pixel_values_batch.append(image)
        
                # New code to make all sequences the same length
        max_len = max([seq.size(1) for seq in input_ids_batch])
        input_ids_batch = [torch.cat([seq, self.tokenizer.pad_token_id * torch.ones((1, max_len - seq.size(1)), dtype=torch.long)], dim=1) for seq in input_ids_batch]
        # print(input_ids_batch[-1])

        input_ids = torch.vstack(input_ids_batch)
        images = torch.vstack(pixel_values_batch)
        image_sizes = torch.vstack(image_sizes_batch)  
