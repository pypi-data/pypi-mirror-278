...
def _make_labels(plotkeys, bjd_ref, extra_basis_funcs=None):
    labels = []
    xbf = {} if extra_basis_funcs==None else extra_basis_funcs
    for key in plotkeys:
        if key == 'T_0':
...
#----
 ...
 # Eclipse and transit fitting
    ...
    def __factor_model__(self, scale, extra_basis_funcs):
    ...
    def lmfit_transit(self, 
            ...
            glint_scale=None, logrhoprior=None, extra_basis_funcs=None,
            log_sigma=None):
        """
        ...
        * as an lmfit Parameter object.
        ...
        Arbitrary basis functions specified by the user, each with its own
        linear coefficient, can be included in the model using the
        extra_basis_funcs keyword. Use the keyword extra_basis_funcs to specify
        the basis functions in the following format ...
     
          extra_basis_funcs = {'t':t, 'a':{'x':a}, 'b',{'x':b}}

        The times at which the basis functions are sampled can be specified
        using the key 't'. Times are specfied using the same time scale as 
        dataset.lc['time'], i.e. BJD_TDB-dataset.bjd_ref. Each basis function
        ...
        The array of values provided for each basis function are used to
        create an linear interpolating function that can be used to evaluate
        the basis function at arbitrary times. By default, the first/last
        value in the array is used to extrapolate to times before/after the
        input array of times. To specify different extrapolated values, use
        the 'fill_value' key to specify a value of the fill_value keyword to
        be used in scipy.interpolate.interp1d, e.g. 

          extra_basis_funcs = { 'a':{'x':a, 'fill_value':0},
                                'b':{'x':b, 'fill_value':np.mean(b)},
                                'c':{'x':c, 'fill_value':'extrapolate'} }

        Summary statistics for each basis function are printed on
        initialisation of FactorModel. It is advisable to use basis functions
        with a mean value of 0 and with a range or standard deviation of about
        1. It is also advisable to avoid basis functions that are strongly
        correlated with one another. A warning is printed for basis functions
        that are strongly correlated. Use 'corr_warn_lim' to set the minimum
        absolute  value of the correlation coefficient used to trigger this
        warning. N.B. correlation between extra basis functions and the
        following functions are not checked: t^2, sin2phi, cos2phi, sin3phi,
        cos3phi.

        By default, the coefficients for each basis function are labeled in
        plots using the key prefixed by 'dfd'. Alternative labels can be
        specified using the 'label' key, e.g. 

          extra_basis_funcs={'x2':{'x':dx**2, 'label':'$d^2f/d(\Delta x)^2$'}}
        ...
        dataset.lmfit_eclipse() using the 'init' key,  e.g. 
          ...
          extra_basis_funcs = { 'a':{'x':a, 'init':(-2,2)},
        ...
        if extra_basis_funcs == None:
            self.extra_basis_funcs = None
            extra_basis_funcs_t = None
        else:
            self.extra_basis_funcs = extra_basis_funcs
            extra_basis_funcs_t = extra_basis_funcs.copy()
            if 't' in extra_basis_funcs_t:
                # pop 't' so it gets skipped when we loop over parameters
                t = extra_basis_funcs_t.pop('t') 
                if (min(t) > max(time)) or (max(t) < min(time)):
                    raise ValueError('time array for extra basis functions do'
                                     ' not overlap times in light curve')
            else:
                t = time

            for p in extra_basis_funcs_t:
                if not 'init' in extra_basis_funcs_t[p]:
                    p_init = (-1, 1)
                else:
                    p_init = extra_basis_funcs_t[p]['init']
                params['dfd'+p] = _kw_to_Parameter('dfd'+p, p_init)

            # Restore/add time array 't' to extra_basis_funcs_t
            extra_basis_funcs_t['t'] = t
        ...
        if True in [p in l for p in params]:
            ...
            self.__scale__ = None
        model = TransitModel()*self.__factor_model__(scale,extra_basis_funcs_t)
    ...
    def lmfit_eclipse(self, 
            ...
            dfdsinphi=None, dfdcosphi=None, dfdsin2phi=None, dfdcos2phi=None,
            dfdsin3phi=None, dfdcos3phi=None, dfdt=None, d2fdt2=None,
            glint_scale=None, extra_basis_funcs=None, log_sigma=None):
        """
        ...
        if True in [p in l for p in params]:
            ...
            self.__scale__ = None
        model = EclipseModel()*self.__factor_model__(scale,extra_basis_funcs_t)
    ...
    def aperture_scan(self, xy_detrend_fixed=True, data_match=True,
                        ...
                        extra_basis_funcs=None, copy_initial=False):
        """
    ...
    def trail_plot(self, plotkeys=['T_0', 'D', 'W', 'b'],
        ...
        if n == 1: ax = [ax,]
        labels = _make_labels(plotkeys, self.bjd_ref, self.extra_basis_funcs)
