...
class BasisFunctionCorrelationWarning(Warning):
...
class FactorModel(Model):
    ...
    See Dataset.lmfit() for details of the extra_basis_funcs keyword.
    ...
    def __init__(self, independent_vars=['t'], prefix='', nan_policy='raise',
                 ...
                 extra_basis_funcs=None, corr_warn_lim=0.9, **kwargs):
        ...
        def factor(t, c=1.0,dfdt=0, d2fdt2=0, dfdbg=0,
            ...
            if hasattr(self, 'extra_basis_funcs'):
        ...
        t = extra_basis_funcs.pop('t')
        all_basis_funcs = {
                't':interp1d(t, t-np.median(t), fill_value='extrapolate'),
                'bg':bg, 'contam':contam, 'smear':smear, 'deltaT':deltaT,
                'dx':dx, 'dy':dy, 'sinphi':sinphi, 'cosphi':cosphi}

        print('Adding extra basis functions.')
        basis_funcs= {}
        for p in extra_basis_funcs:
            x = extra_basis_funcs[p]['x']
            for k in all_basis_funcs:
                cc = np.abs(np.corrcoef(x, all_basis_funcs[k](t))[0,1])
                if cc > corr_warn_lim:
                    msg = f'Basis function {p} correlated with {k} ({cc:0.3f})'
                    warnings.warn(msg, BasisFunctionCorrelationWarning)
            try:
                fv = extra_basis_funcs[p]['fill_value']
            except KeyError:
                fv = (x[0],x[-1])
            ix = interp1d(t, x, bounds_error=False, fill_value=fv, copy=False)
            all_basis_funcs[p] = ix
            basis_funcs[p] = ix
            txt = f'{p:8s}: mean = {x.mean():8.3f}, std. dev. = {x.std():8.3f}'
            txt += f', min = {x.min():8.3f}, max = {x.max():8.3f}'
            print(txt)
            self.set_param_hint('dfd'+p, value=0, vary=False)

        self.extra_basis_funcs = basis_funcs
