    ...
    def check(self, email: str, *token: str) -> None:
        ...
        if "emailAlreadyRegistered" in response.json():
            if response.json().get("emailAlreadyRegistered") is True:
                ...
                exit(0)
        elif "b2b-user-with-regular-client-error" in response.text:
            c.print(
                f"\n:green_circle: [green][bold]{email}[/bold] is already registered on Onoff.app as a [bold]Business user[/bold].[/green]\n"
            )
            exit(0)
        elif (
