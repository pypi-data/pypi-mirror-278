...
from .conftest import pytest, rpc_called, get_client
...
def test_list_sessions_running():
    sessions = session.list_sessions(get_client("Sessions"), SessionFieldFilter.STATUS == SessionStatus.RUNNING)
    assert rpc_called("Sessions", "ListSessions", 2)
    assert sessions == []

def test_list_sessions_cancelled():
    sessions = session.list_sessions(get_client("Sessions"), SessionFieldFilter.STATUS == SessionStatus.CANCELLED)
...
def test_cancel_multiple_sessions():
    session.cancel_sessions(get_client("Sessions"), ["session-id1","session-id2","session-id3"] )
