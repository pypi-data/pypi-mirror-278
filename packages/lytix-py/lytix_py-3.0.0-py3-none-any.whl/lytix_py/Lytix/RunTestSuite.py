...
async def runTestSuite():
    ...
    for func in asyncFunctions:
        ...
        @lytix.test(
            **func["testArgs"], _testGroupId=testGroupId, _testName=functionName
        )
    ...
    finalizeRes = MetricCollector._finalizeTestGroup(testGroupId)
    if finalizeRes["valid"] is not True:
        print(
            f"{colors.RED}Failed to finalize test group {testGroupId} with error: {finalizeRes['error']}{colors.RESET}"
        )
    # testRunning = False
