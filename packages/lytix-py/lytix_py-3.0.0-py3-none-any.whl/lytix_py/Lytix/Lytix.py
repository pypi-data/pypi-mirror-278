...
class LytixWrapper:
    ...
    def test(
        ...
        *kwargs,
        ...
        def decorator(func: F) -> F:
            ...
            """
            if isAsyncFunction:
                return self._asyncTestWrapper(func, testsToRun, _testGroupId, _testName)
            else:
                return self._syncTestWrapper(func, testsToRun, _testGroupId, _testName)
    ...
    def _syncTraceWrapper(self, func: F, modelName: str):
        ...
        def syncDecorator(*args, **kwargs):
            self._prepTraceDecoratorContext(modelName)
            startTime = time.time()
    ...
    def _syncTestWrapper(
        self, func: F, testsToRun: List[str], testGroupId: str, testName: str
    ):
        """
        ...
        @wraps(func)
        def syncDecorator(*args, **kwargs):
            self._prepTestDecoratorContext(testsToRun)
            func(*args, **kwargs)

            """
            Before returning to the user, send the test + IO back to HQ
            """
            testResult = self._startAndWaitForLytixTest(testGroupId, testName)
    ...
    def _startAndWaitForLytixTest(self, testGroupId: str, testName: str):
        """
        ...
        testRunIds = MetricCollector._kickoffTestRun(
            input, output, testsToRun, traceId, testGroupId, testName
        )
    ...
    def _prepTestDecoratorContext(self, testsToRun: List[str]):
        """
    ...
    def _prepTraceDecoratorContext(self, modelName: str):
        """
        Prepares the decorator context
        """
        try:
