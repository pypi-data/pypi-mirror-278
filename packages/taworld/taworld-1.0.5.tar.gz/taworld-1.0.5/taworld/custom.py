...
def import_dir(path, verbose=True):
    ...
    for d in dirs:
        ...
        # only look in directories which are valid taworld categories
        if dirname not in [*taworld.Category]:
            if verbose:
                print(f"[i] Skipping the sub-directory '{dirname}' since it's not a valid taworld category.")
            continue
        ...
        for module in glob(abspath(join(path, dirname, "*.py"))):
            ...
            # figure out which of the modules functions to bind to taworld
            ...
            if module_name not in taworld.Category[dirname]:
                taworld.Category[dirname].append(module_name)
...
import_dir.__doc__ = \
"""
Import a directory of custom indicators into taworld
...
together with the existing taworld functions just like if they were part of
taworld.

If you at some late point would like to push them into the taworld library
you can do so very easily by following the step by step instruction.

A brief example of usage:

1. Loading the 'ta' module:
>>> import pandas as pd
>>> import taworld as ta

2. Create an empty directory on your machine where you want to work with your
indicators. Invoke taworld.custom.import_dir once to pre-populate it with
sub-folders for all available indicator categories, e.g.:

>>> import os
>>> from os.path import abspath, join, expanduser
>>> from taworld.custom import create_dir, import_dir
>>> ta_dir = abspath(join(expanduser("~"), "my_indicators"))
>>> create_dir(ta_dir)

3. You can now create your own custom indicator e.g. by copying existing
ones from taworld core module and modifying them.
...
available in categories under the taworld-folder. The only difference will
...
like all other native indicators in taworld, including help functions.
"""
