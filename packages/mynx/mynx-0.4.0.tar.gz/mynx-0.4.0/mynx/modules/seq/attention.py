    ...
    def __call__(self, q:Array, k:Optional[Array]=None, v:Optional[Array]=None, mask:Optional[Array]=None, cache:Optional[list[Array]]=None) -> Any:
        ...
        if not self.is_training and self.use_kvcache:
            if k is None:
                ...
                kx = k_layer(k)
                kx = jnp.concat([cache[0], kx], axis=1)

            if v is None:
                vx = cache[0]
            else:
                vx = k_layer(v)
                vx = jnp.concat([cache[0], vx], axis=1)
        ...
        qx = jnp.transpose(qx, (0, 2, 1, 3))
        kx = jnp.transpose(kx, (0, 2, 1, 3))
        vx = jnp.transpose(vx, (0, 2, 1, 3))
        scores = jnp.einsum("bhjk,bhik->bhji", qx, kx)
        scores /= jnp.sqrt(head_dim)
        if mask is not None:
            scores -= 1 / mask + 1
        scores = nn.softmax(scores)

        out = jnp.einsum("bhjk,bhki->bhji", scores, vx)
