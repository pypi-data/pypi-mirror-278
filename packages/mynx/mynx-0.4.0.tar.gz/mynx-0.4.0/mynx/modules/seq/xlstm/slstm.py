...
class SLSTMCell(nn.Module):
    ...
    def __call__(self, xz:Array, xi:Array, xf:Array, xo:Array, hiden_state:Optional[Tuple[Array]]=None):
        ...
        zero_state = (
                    jnp.zeros((xz.shape[0], 1, self.head_num, self.head_dim)), 
                    jnp.zeros((xz.shape[0], 1, self.head_num, self.head_dim)), 
                    jnp.zeros((xz.shape[0], 1, self.head_num, self.head_dim))
                    )
        ...
        xz = xz.reshape(xz.shape[0], xz.shape[1], self.head_num, -1)
        ...
        szt = jnp.einsum("bshd,dho->bsho", xz, wz) + bz
        sit = jnp.einsum("bshd,dho->bsho", xi, wi) + bi
        sft = jnp.einsum("bshd,dho->bsho", xf, wf) + bf
        sot = jnp.einsum("bshd,dho->bsho", xo, wo) + bo

        out = []
        for zt, it, ft, ot in zip(*list(map(lambda x:jnp.split(x, x.shape[1], axis=1), (szt, sit, sft, sot)))):
            zt += jnp.einsum("bshd,dho->bsho", ht, rz)
            it += jnp.einsum("bshd,dho->bsho", ht, ri)
            ft += jnp.einsum("bshd,dho->bsho", ht, rf)
            ot += jnp.einsum("bshd,dho->bsho", ht, ro)
