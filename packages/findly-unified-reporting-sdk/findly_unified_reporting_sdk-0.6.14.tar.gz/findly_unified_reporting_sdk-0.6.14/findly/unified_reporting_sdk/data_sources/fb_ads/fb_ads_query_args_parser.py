...
class FbAdsQueryArgsParser(CommonParser):
    ...
    def get_filter_from_expression(
        ...
        """
        # If we have parsed expression, use that
        if parsed_expr is not None and parsed_expr != []:
            unformatted_filter_list = parsed_expr

        # If don't have parsed expression, parse the expression
        elif (parsed_expr is None or parsed_expr == []) and (
            expr is not None and expr.strip() != ""
        ):
