...
#
# Imported and adjusted useful conan hooks from https://github.com/conan-io/hooks
#
...
DISABLED_CONAN_HOOKS = tools.get_env("DISABLED_CONAN_HOOKS", "").split(",")

# backward compatibility, old projects used PMI-H001 instead of KB-H001
DISABLED_CONAN_HOOKS = DISABLED_CONAN_HOOKS + [
    h.replace("PMI", "KB") for h in DISABLED_CONAN_HOOKS
]

# Checks not needed for pmi CI
# DISABLED_CONAN_HOOKS.extend([])
...
kb_errors = {
    ...
    "KB-H072": "CRLF IS NOT ALLOWED",
    "KB-H073": "CLEAN SOURCE CODE",
    "KB-H074": "PMI MACROS",
}
...
class _HooksOutputErrorCollector(object):
    ...
    def __init__(self, output, kb_id=None):
        ...
        self._report_folder = os.getenv("CONAN_HOOK_REPORT_FOLDER", "")
        if not self._report_folder and os.getenv("WORKSPACE"):
            self._report_folder = os.path.join(
                os.getenv("WORKSPACE"), "build", "conan_hooks"
            )

        if self._report_folder and not os.path.exists(self._report_folder):
            os.makedirs(self._report_folder)

    def save_to_file(self, message, level):
        report_file = os.path.join(self._report_folder, f"conan_hooks_{level}.txt")
    ...
    def _get_message(self, message):
        if self._test_name:
            name = (
                "{} ({})".format(self._test_name, self.kb_id)
                if self.kb_id
                else self._test_name
            )
    ...
    def warn(self, message):
        self.save_to_file(self._get_message(message), "warn")
    ...
    def error(self, message):
        self.save_to_file(self._get_message(message), "error")
        self._error = True
...
def kb_url(kb_id):
    return "https://bitbucket.org/pmi_dev_team/pmi_scripts/src/master/docs/pmi_conan_hooks.md#{}".format(
        kb_id
    )
...
def run_test(kb_id, output):
    def tmp(func):
        ...
        if kb_id in DISABLED_CONAN_HOOKS:
            out.warn("DISABLED")
            return
        try:
            ret = func(out)
            ...
            out.error(
                "Exception raised from hook: {} (type={})".format(
                    e, type(e).__name__
                )
            )
            raise
...
def pre_export(output, conanfile, conanfile_path, reference, **kwargs):
    ...
    installer = (
        settings is not None and "os_build" in settings and "arch_build" in settings
    )
    ...
    def test(out):
        ...
        _message_attr(
            ["url", "license", "description", "homepage", "topics"], out.error
        )
    ...
    def test(out):
        ...
        for num, line in enumerate(conanfile_content.splitlines(), 1):
            if vrange_match.match(line):
                out.error(
                    "Possible use of version ranges, line %s:\n %s" % (num, line)
                )
            ...
            for exports_it in exports:
                for license_it in ["copying", "license", "copyright"]:
                    if license_it in exports_it.lower():
                        out.error(
                            "This recipe is exporting a license file. "
    ...
    def test(out):
        def _search_for_metaline(from_line, to_line, lines):
            for index in range(from_line, to_line):
                line_number = index + 1
                if "# -*- coding:" in lines[index] or "# coding=" in lines[index]:
                    out.error(
                ...
                if "#!" in lines[index]:
                    out.error(
                        "Shebang (#!) detected in your recipe. "
                        "Remove the line {}".format(line_number)
                    )
        ...
        last_lines_range = (
            len(conanfile_lines) - 3
            if len(conanfile_lines) > 8
            else len(conanfile_lines)
        )
        _search_for_metaline(
            last_lines_range, len(conanfile_lines), conanfile_lines
        )
            ...
            if os.path.exists(cmake_path):
                ...
                for line in cmake_content.splitlines():
                    if (
                        ...
                        or "cmake_minimum_required (version" in line
                    ):
                        break
                    else:
                        out.error(
    ...
    def test(out):
        ...
        if "RunEnvironment" in test_package_conanfile and not re.search(
            r"self\.run\(.*, run_environment=True\)", test_package_conanfile
        ):
            out.error(
    ...
    def test(out):
        if conanfile.version == "system":
            out.info(
                "'system' versions are allowed to install system requirements."
            )
            return
        if (
            "def system_requirements" in conanfile_content
            and "SystemPackageTool" in conanfile_content
        ):
            import re

            match = re.search(
                r"(\S+)\s?=\s?(tools.)?SystemPackageTool", conanfile_content
            )
    ...
    def test(out):
        ...
        allowed_first_level = ["sources", "patches", "pmi"]
        ...
        if not conandata_yml:
            return
        entries = _not_allowed_entries(
            list(conandata_yml.keys()), allowed_first_level
        )
        if entries:
            out.error(
        ...
        for entry in conandata_yml:
            if entry in ["sources", "patches"]:
                if not isinstance(conandata_yml[entry], dict):
                    out.error(
                        "Expecting a dictionary with versions as keys under '{}' element".format(
                            entry
                        )
                    )
                else:
            ...
            def validate_recursive(e, data, name, allowed):
                if (
                    isinstance(e, str)
                    and e not in allowed_sources
                    and not isinstance(data[e], str)
                ):
                    for child in data[e]:
                    ...
                    if isinstance(e, dict):
                        for k, v in e.items():
                            if k in checksums:
                                ...
                                if not v:
                                    out.error(
                                        f"The entry '{k}' cannot be empty in conandata.yml."
                                    )
                            if k == "url":
            ...
            for element in conandata_yml[entry][version]:
                if entry == "sources":
                    has_sources = True
                    validate_checksum_recursive(
                        element, conandata_yml[entry][version]
                    )
            if not found_checksums and has_sources and not is_google_source:
                out.error(
                    "The checksum key 'sha256' must be declared and can not be empty."
                )
            elif found_checksums and "sha256" not in found_checksums:
                out.warn(
                    f"Consider 'sha256' instead of {weak_checksums}. It's considerably more secure than others."
                )
    ...
    def test(out):
        ...
        if match:
            out.error(
                "CCI uses the name of the package for cmake generator and filename by default."
                " Replace 'cpp_info.{0}' by 'cpp_info.{0}s[<generator>]'.".format(
                    match.group(1)
                )
            )

        match = re.search(
            r"self\.cpp_info\.(names|filenames)\[\s?['\"](cmake|cmake_multi)['\"]\s?\]",
            conanfile_content,
        )
        if match:
            out.error(
                ...
                "Use 'cmake_find_package' and 'cmake_find_package_multi' instead.".format(
    ...
    def test(out):
        if getattr(conanfile, "isPmiProject", None) or getattr(
            conanfile, "scm", None
        ):
            return
    ...
    def test(out):
        for forbidden in [
            "self.requires.add",
            "self.build_requires.add",
            "self.build_requires",
            "export_conandata_patches(self)",
            "apply_conandata_patches(self)",
        ]:
            if forbidden in conanfile_content:
                out.error("The method '{}()' is not allowed".format(forbidden))
    ...
    def test(out):
        ...
        if os.path.isfile(cmake_test_pkg):
            ...
            if re.search(
                r"cmake_minimum_required\(version [\"']?2", cmake_content.lower()
            ):
                out.error(
        ...
        if os.path.isfile(cmake_path):
            ...
            if (
                re.search(
                    r"cmake_minimum_required\(version [\"']?2",
                    cmake_content.lower(),
                )
                and "cxx_standard" in cmake_content.lower()
            ):
                out.error(
                ...
                for cmake_def in [
                    ...
                    if cmake_def in cmake_content:
                        out.error(
                            ...
                            "CMakeLists.txt to 'cmake_minimum_required(VERSION 3.4)'.".format(
                                cmake_def
                            )
                        )
                        break
    ...
    def test(out):
        ...
        test_package_dir = os.path.join(
            os.path.dirname(conanfile_path), "test_package"
        )
    ...
    def test(out):
        for prefix in ["", "build_"]:
            if hasattr(conanfile, "{}requires".format(prefix)) and callable(
                getattr(conanfile, "{}requirements".format(prefix), None)
            ):
                out.error(
    ...
    def test(out):
        def _check_content(content, path):
            if "os.rename" in content:
                out.warn(
                    "The 'os.rename' in {} may cause permission error on Windows."
                    " Use 'conan.tools.files.rename(self, src, dst)' instead.".format(
                        path
                    )
                )
            elif "tools.rename(" in content and not "tools.rename(self," in content:
                out.warn(
                    "The 'tools.rename' in {} is outdated and may cause permission error on Windows."
                    " Use 'conan.tools.files.rename(self, src, dst)' instead.".format(
                        path
                    )
                )
        ...
        test_package_path = os.path.join(
            os.path.dirname(conanfile_path), "test_package", "conanfile.py"
        )
        if os.path.exists(test_package_path):
            test_package_content = tools.load(test_package_path)
        ...
        for root, _, files in os.walk(recipe_folder):
            for file in files:
                if any(it in disallowed_chars for it in file):
                    out.error(
                        "The file '{}' uses illegal charecters ({}) for its name."
                        " Please, rename that file.".format(file, disallowed_chars)
                    )
                if file.endswith("."):
                    out.error(
                        "The file '{}' ends with a dot. Please, remove the dot from the end.".format(
                            file, disallowed_chars
                        )
                    )
    ...
    def test(out):
        ...
        if class_name in ("LibnameConan", "ConanFileDefault"):
            camel_name = "".join(
                s.title() for s in re.split("[^a-zA-Z0-9]", conanfile.name)
            )
            out.warn(
                "Class name '{}' is not allowed. For example, use '{}Conan' instead.".format(
    ...
    def test(out):
        Location = collections.namedtuple(
            "Location", ("line", "column", "line_end", "column_end")
        )
        ...
        class BuildInfoVisitor(ast.NodeVisitor):
            ...
            def visit_Attribute(self, node):
                methods_stack_no_build_info_allowed = [
                    fdef
                    for fdef in self.function_def_stack
                    if fdef in self.METHODS_NO_BUILDINFO
                ]
                if methods_stack_no_build_info_allowed:
                    ...
                    if (
                        ...
                        node.attr.startswith("is_")
                        and isinstance(node.value, ast.Name)
                        and node.value.id == "os_info"
                    ):
        ...
        to_test = [
            (conanfile_path, conanfile_content),
        ]
        test_conanfile_path = os.path.join(
            export_folder_path, "test_package", "conanfile.py"
        )
        ...
        for dut_conanfile_path, dut_conanfile_contents in to_test:
            try:
                ...
                out.error(
                    "A SyntaxError was thrown while parsing '{}'".format(
                        dut_conanfile_path
                    )
                )
                continue
            ...
            for build_info in visitor.invalids:
                out.error(
                    "{}:{} Build system dependent functions detected. (Use of {} is forbidden in {})".format(
                        dut_conanfile_path,
                        build_info.loc.line,
                        build_info.what,
                        build_info.func,
    ...
    def test(out):
        def _check_content(content, path):
            if "tools.cross_building(self.settings)" in content:
                out.warn(
                    "The 'tools.cross_building(self.settings)' syntax in {} may not work correctly "
                    "in some scenarios. Consider using tools.cross_building(self).".format(
                        path
                    )
                )
        ...
        if topics and isinstance(topics, (list, tuple)):
            ...
            for topic in topics:
                if topic in invalid_topics:
                    out.warn(
                        "The topic '{}' is invalid and should be removed from topics "
                        "attribute.".format(topic)
                    )
                if topic != topic.lower():
                    out.warn(
                        "The topic '{}' is invalid; even names and acronyms should be formatted "
        ...
        if os.path.isfile(test_package_path):
            try:
                ...
                if test_type and test_type != "explicit":
                    out.error(
                        f"The attribute 'test_type' only should be used with 'explicit' value.: {test_type}"
                    )
            except Exception as e:
    ...
    def test(out):
        ...
        if settings:
            settings = (
                settings if isinstance(settings, (list, tuple)) else [settings]
            )
            missing = [
                x
                for x in ["os", "arch", "compiler", "build_type"]
                if x not in settings
            ]
            if missing:
                out.warn(
                    "The values '{}' are missing on 'settings' attribute. Update settings with the missing "
...
def post_export(output, conanfile, conanfile_path, reference, **kwargs):
    ...
    def test(out):
        ...
        for path, dirs, files in os.walk(dir_path):
            dirs[:] = [
                d for d in dirs if d not in [".conan"]
    ...
    def test(out):
        ...
        if conanfile.name in allowlist:
            out.info(
                "'{}' is part of the allowlist, skipping.".format(conanfile.name)
            )
            return
        ...
        if (
            default_options
            and isinstance(default_options, dict)
            and default_options.get("shared") is True
        ):
            out.error(
                "The option 'shared' must be 'False' by default. Update 'default_options'."
            )

    @run_test("KB-H056", output)
    def test(out):
        if str(conanfile.license).lower() in [
            "public domain",
            "public-domain",
            "public_domain",
        ]:
            out.error(
                "Public Domain is not a SPDX license. Use 'Unlicense' instead."
            )

    @run_test("KB-H072", output)
    def test(out):
        export_folder_path = os.path.dirname(conanfile_path)
        out.info(f"Check that all files in {export_folder_path} does not use CRLF")
        for root, _, files in os.walk(export_folder_path):
            for filename in files:
                lines = open(os.path.join(root, filename), "rb").readlines()
                if any(line.endswith(b"\r\n") for line in lines):
                    out.error(
                        "The file '{}' uses CRLF. Please, replace by LF.".format(
                            filename
                        )
                    )
...
def post_source(output, conanfile, conanfile_path, **kwargs):
    ...
    def _is_pure_c():
        if not _is_recipe_header_only(conanfile):
            ...
            c_extensions = ["c", "h"]
            return not _get_files_with_extensions(
                conanfile.source_folder, cpp_extensions
    ...
    def test(out):
        if _is_pure_c():
            ...
            if conanfile.settings.get_safe("compiler") and (
                "del self.settings.compiler.libcxx" not in low
                and 'self.settings.rm_safe("compiler.libcxx")' not in low
                and "self.settings.rm_safe('compiler.libcxx')" not in low
                and 'self.settings.compiler.rm_safe("libcxx")' not in low
                and "self.settings.compiler.rm_safe('libcxx')"
            ):
                out.error(
                    "Can't detect C++ source files but recipe does not remove "
    ...
    def test(out):
        if _is_pure_c():
            ...
            low = conanfile_content.lower()
            if conanfile.settings.get_safe("compiler") and (
                "del self.settings.compiler.cppstd" not in low
                and 'self.settings.rm_safe("compiler.cppstd")' not in low
                and "self.settings.rm_safe('compiler.cppstd')" not in low
                and 'self.settings.compiler.rm_safe("libcxx")' not in low
                and "self.settings.compiler.rm_safe('libcxx')"
            ):
    ...
    def test(out):
        if not os.path.exists(os.path.join(conanfile.source_folder, ".git")):
            return

        if not os.path.exists(
            os.path.join(conanfile.source_folder, "conanfile.py")
        ):
            # workaround for the conan bug https://github.com/conan-io/conan/issues/5415
            subprocess.run(
                ["git", "checkout", "conanfile.py"], cwd=conanfile.source_folder
            )

        git_status_output = (
            subprocess.check_output(
                ["git", "status", "--porcelain"], cwd=conanfile.source_folder
            )
            .decode()
            .strip()
        )
        if git_status_output:
            out.warn(
                f"{conanfile.source_folder} is not a clean git repo! git status returned: {git_status_output}"
            )

    @run_test("KB-H074", output)
    def test(out):
        conanfile_content = tools.load(conanfile_path)
        if "pmi_pyreq.CommonCmakeRecipe" not in conanfile_content:
            return

        cmake_files = _get_files_following_patterns(
            conanfile.source_folder, ["CMakeLists.txt"]
        )
        print("cmake_files: {}".format(str(cmake_files)))

        for cmake_file in cmake_files:
            cmake_content = tools.load(cmake_file)
            if re.search(r"[\s*]add_library", cmake_content, re.IGNORECASE):
                out.error(
                    "Usage of add_library in not allowed in {}! use pmi_add_library instead".format(
                        cmake_file
                    )
                )
            if re.search(r"[\s*]add_executable", cmake_content, re.IGNORECASE):
                out.error(
                    "Usage of add_executable in not allowed in {}! use pmi_add_executable instead".format(
                        cmake_file
                    )
                )
...
def pre_build(output, conanfile, **kwargs):
    ...
    def test(out):
        ...
        if conanfile.settings.get_safe("os") == "Windows" and has_fpic:
            out.error(
                "'fPIC' option not managed correctly. Please remove it for Windows "
                "configurations: del self.options.fPIC"
            )
            error = True
        if has_fpic and conanfile.options.get_safe("shared"):
            out.error(
                "'fPIC' option not managed correctly. Please remove it for shared "
...
def post_package(output, conanfile, conanfile_path, **kwargs):
    ...
    def test(out):
        ...
        if not licenses:
            out.warn("No valid licenses files found at: %s\n" % licenses_folder)

    @run_test("KB-H013", output)
    def test(out):
        if not "installer" in str(conanfile.name):  # only check installer deps
            return

        for require, dependency in conanfile.dependencies.items():
            if (
                not require.build
                and not require.test
                and not "system" in str(dependency.ref.version)
            ):
                licenses_folder = os.path.join(
                    dependency.package_folder, "licenses"
                )
                if not os.path.exists(licenses_folder):
                    out.warn(
                        "No 'licenses' folder found in package: %s "
                        % dependency.ref.name
                    )
                    continue
    ...
    def test(out):
        ...
        if not _files_match_settings(conanfile, conanfile.package_folder, out):
            out.error(
                "Packaged artifacts does not match the settings used: os=%s, compiler=%s"
    ...
    def test(out):
        if not _shared_files_well_managed(conanfile, conanfile.package_folder):
            out.error(
                "Package with 'shared' option did not contains any shared artifact"
            )
    ...
    def test(out):
        if conanfile.name in [
            ...
            return
        bad_files = _get_files_following_patterns(
            conanfile.package_folder, ["*.pc"]
        )
        if bad_files:
            out.error(
                "The conan-center repository doesn't allow the packages to contain `pc` "
    ...
    def test(out):
        ...
        bad_files = _get_files_following_patterns(
            ...
            ["Find*.cmake", "*Config.cmake", "*-config.cmake"],
        )
        if bad_files:
            out.error(
                "The conan-center repository doesn't allow the packages to contain CMake "
    ...
    def test(out):
        bad_files = _get_files_following_patterns(
            conanfile.package_folder, ["*.pdb"]
        )
        bad_files = list(
            filter(lambda pdb_file: "protein_3d_files" not in pdb_file, bad_files)
        )  # allow QAL protein_3d_files
    ...
    def test(out):
        bad_files = _get_files_following_patterns(
            conanfile.package_folder, ["*.la"]
        )
        if bad_files:
            out.error(
                "Libtool files found (*.la). Do not package *.la files "
                "but library files (.a) "
            )
            out.error("Found files: {}".format("; ".join(bad_files)))
    ...
    def test(out):
        ...
        if bad_files:
            out.error(
                "The conan-center repository doesn't allow Microsoft Visual Studio runtime files."
            )
            out.error("Found files: {}".format("; ".join(bad_files)))
    ...
    def test(out):
        ...
        needed_system_libs = set(dict_deplibs_libs.keys()).intersection(
            all_system_libs
        )
        ...
        conanfile_system_libs = set(
            m.group(2)
            for m in re.finditer(
                r"""(["'])([a-zA-Z0-9._-]+)(\1)""", tools.load(conanfile_path)
            )
        ).intersection(all_system_libs)

        missing_system_libs = needed_system_libs.difference(
            deps_system_libs.union(conanfile_system_libs)
        )
        ...
        for missing_system_lib in missing_system_libs:
            ...
            for lib in libs:
                out.warn(
                    "Library '{}' links to system library '{}' but it is not in cpp_info.{}.".format(
                        lib, missing_system_lib, attribute
                    )
                )
...
def post_package_info(output, conanfile, reference, **kwargs):
    ...
    def test(out):
        if conanfile.name in ["android-ndk", "cmake", "msys2", "strawberryperl"]:
            return
        bad_files = _get_files_following_patterns(
            conanfile.package_folder, ["*.cmake"]
        )
        build_dirs = {
            bd.replace("\\", "/") for bd in conanfile.cpp_info.build_paths
        }
        for component in conanfile.cpp_info.components.values():
            build_dirs.update(
                {bd.replace("\\", "/") for bd in component.build_paths}
            )
        files_missplaced = []
        ...
        build_modules.extend(
            conanfile.cpp_info.build_modules["cmake_find_package_multi"]
        )
        for component in conanfile.cpp_info.components.values():
            build_modules.extend(component.build_modules["cmake_find_package"])
            build_modules.extend(
                component.build_modules["cmake_find_package_multi"]
            )
        ...
        if files_missplaced:
            out.warn(
                "The *.cmake files have to be placed in a folder declared as "
                "`cpp_info.builddirs`. Currently folders declared: {}".format(
                    build_dirs
                )
            )
        ...
        def _test_component(component):
            ...
            for p in component.libdirs:
                ...
                libs_to_search = [
                    ...
                    if not os.path.isfile(os.path.join(p, l))
                ]
            for l in libs_to_search:
                out.error(
                    f"Component {conanfile.name}::{component.name} library '{l}' is listed in the recipe, "
...
def _shared_files_well_managed(conanfile, folder):
    ...
    try:
        options_dict = {
            key: value for key, value in conanfile.options.values.as_list()
        }
    except Exception:
...
def _files_match_settings(conanfile, folder, output):
    ...
    if _is_recipe_header_only(conanfile):
        if not has_header and (has_visual or has_mingw or has_linux or has_macos):
            output.error(
                "Package for Header Only does not contain artifacts with these extensions: "
                "%s" % header_extensions
            )
            return False
        else:
    ...
    if settings_os == "Windows":
        if conanfile.settings.get_safe("compiler") == "Visual Studio":
            if not has_visual:
                output.error(
                    "Package for Visual Studio does not contain artifacts with these "
            ...
            if not has_mingw:
                output.error(
                    "Package for MinGW does not contain artifacts with these extensions: "
                    "%s" % mingw_extensions
                )
            return has_mingw
        else:
    ...
    if settings_os == "Linux":
        if not has_linux:
            output.error(
                "Package for Linux does not contain artifacts with these extensions: "
                "%s" % linux_extensions
            )
        return has_linux
    if settings_os == "FreeBSD":
        if not has_freebsd:
            output.error(
                "Package for FreeBSD does not contain artifacts with these extensions: "
    ...
    if settings_os == "Macos":
        if not has_macos:
            output.error(
                "Package for Macos does not contain artifacts with these extensions: "
                "%s" % macos_extensions
            )
        return has_macos
    if settings_os is None:
        if not has_header and (
            has_visual or has_mingw or has_linux or has_freebsd or has_macos
        ):
            output.error(
                "Package for Header Only does not contain artifacts with these extensions: "
                "%s" % header_extensions
...
def _is_recipe_header_only(conanfile):
    ...
    header_only_id = "self.info.header_only()" in inspect.getsource(
        package_id_method
    )
    settings_clear = "self.info.settings.clear()" in inspect.getsource(
        package_id_method
    )
...
def _skip_test_package(root, filename):
    ...
    return filename.startswith("test_package/build") or filename.startswith(
...
def _deplibs_from_shlibs(conanfile, out):
    ...
    shlext = {"Windows": "dll", "Macos": "dylib"}.get(os_, "so")
    libraries = _get_files_with_extensions(conanfile.package_folder, [shlext])
    if not libraries:
        return deplibs
    if (
        os_ == "Linux"
        or tools.is_apple_os(os_)
        or _get_compiler(conanfile) not in ["Visual Studio", "msvc"]
    ):
        ...
        for library in libraries:
            ...
            try:
                objdump_output = subprocess.check_output(
                    cmd, cwd=conanfile.package_folder
                ).decode()
                ...
                continue
            if _get_os(conanfile) == "Windows":
                for dep_lib_match in re.finditer(
                    r"DLL Name: (.*).dll", objdump_output, re.IGNORECASE
                ):
                ...
                dep_libs_fn = list(
                    l.replace("NEEDED", "").strip()
                    for l in objdump_output.splitlines()
                    if "NEEDED" in l
                )
                for dep_lib_fn in dep_libs_fn:
                    dep_lib_match = re.match(
                        r"lib(.*).{}(?:\.[0-9]+)*".format(shlext), dep_lib_fn
                    )
                    if not dep_lib_match:
                        continue
                    deplibs.setdefault(dep_lib_match.group(1), []).append(library)
    elif (
        _get_compiler(conanfile) in ["Visual Studio", "msvc"]
        or _get_os == "Windows"
    ):
            ...
            for library in libraries:
                try:
                    dumpbin_output = subprocess.check_output(
                        ["dumpbin", "-dependents", library],
                        cwd=conanfile.package_folder,
                    ).decode()
                except subprocess.CalledProcessError:
                    out.warn("Running dumpbin on '{}' failed.".format(library))
                    continue
                for l in re.finditer(
                    r"([a-z0-9\-_]+)\.dll", dumpbin_output, re.IGNORECASE
                ):
...
def _load_conanfile(conanfile_path):
    ...
    _, conanfile_obj = parse_conanfile(
        conanfile_path, python_requires=python_requires, generator_manager=None
    )
