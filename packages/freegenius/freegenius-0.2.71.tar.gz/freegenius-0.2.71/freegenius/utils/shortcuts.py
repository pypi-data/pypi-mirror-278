...
def createShortcuts():
    ...
    # additional shortcuts on Linux
        ...
        def desktopFileContent():
            ...
            freegenius = f'''bash -c ". {os.path.join(os.path.dirname(sys.executable), "activate")}" && freegenius'''
...
"""
        ...
        def desktopSystemTrayFileContent():
            ...
            if not os.path.isfile(iconPath):
                iconPath = os.path.join(config.freeGeniusAIFolder, "icons", "LetMeDoIt.png")
            freegeniushub = f'''bash -c ". {os.path.join(os.path.dirname(sys.executable), "activate")}" && freegeniushub'''
            return f"""#!/usr/bin/env xdg-open
