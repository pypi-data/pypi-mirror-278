...
class PostgresPersistenceAsyncBase(AbstractPipelineStep):
    ...
    def save_dataset(self, dataset: DataFrame, table_name: str) -> None:
        ...
        Saves a dataset.
        """
        PostgresPersistenceAsyncBase.logger.info('Saving %s To Postgres...' % self.descriptive_label)
        ...
        dataset.write.jdbc(url=url, table=table_name, mode='append', properties=properties)

        PostgresPersistenceAsyncBase.logger.info('Saved %s to Postgres' % self.descriptive_label)
