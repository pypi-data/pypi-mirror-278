...
def aiops_encrypt_simple_aes(plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundAndVoidOutboundAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./krausening/base')
...
def aiops_encrypt_with_vault_key(key, plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundAndVoidOutboundAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./config')
            os.environ['KRAUSENING_BASE'] = 'krausening/base/'
...
class NativeInboundAndVoidOutboundAsyncBase(AbstractPipelineStep):
    ...
    def check_and_apply_encryption_policy(self, inbound: DataFrame) -> None:
        """
        ...
        NativeInboundAndVoidOutboundAsyncBase.logger.info('Checking encryption policies')
        ...
        for key, encrypt_policy in retrieved_policies.items():
            ...
            if encrypt_policy.encryptPhase:
                if self.step_phase.lower() == encrypt_policy.encryptPhase.lower():
                    ...
                    NativeInboundAndVoidOutboundAsyncBase.logger.info('Encryption policy does not apply to this phase: ' + self.step_phase)
    ...
    def apply_encryption_to_dataset(self, inbound: DataFrame, fields_to_update: List[str], algorithm: str) -> DataFrame:
        ...
        '''
        NativeInboundAndVoidOutboundAsyncBase.logger.info('applying encryption')
