...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors=10, coverage_radius=50):
        ...
        self.observation_space = Dict({
            'remaining_energy': Box(low=0, high=initial_energy, shape=(self.n_sensors,), dtype=np.float64),
            'sensor_positions': Box(low=lower_bound, high=upper_bound, shape=(self.n_sensors, 2), dtype=np.float64),
            'number_of_packets': Box(low=0, high=np.inf, shape=(self.n_sensors,), dtype=np.float64)
        })
