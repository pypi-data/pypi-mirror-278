...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors, coverage_radius=50):
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            if len(neighbors_i) == 0:
                continue
            if action not in keys_neighbors_i:
                continue
            ...
            if action == self.n_sensors:
                ...
                if neighbors_i[action]['transmission_energy'] < 0:
                    print("=====================================")
                    print("Energy consumption should be positive!")
                    print("Eelec: ", Eelec)
                    print("Eamp: ", Eamp)
                    print("number_of_packets: ", self.number_of_packets[i])
                    print("info_amount: ", info_amount)
                    print("distance: ", neighbors_i[action]['distance'])
                    print("Sensor: ", i)
                    print("action: ", action)
                    print("Neighbors_i: ", neighbors_i)
                    print("Transmission energy: ", neighbors_i[action]['transmission_energy'])
                    print("=====================================")
                    raise ValueError("Energy consumption should be positive!")
                self.update_sensor_energies(i, neighbors_i[action]['transmission_energy'])                
            else:
                if neighbors_i[action]['transmission_energy'] < 0 or neighbors_i[action]['reception_energy'] < 0:
                    print("=====================================")
                    print("Energy consumption should be positive!")
                    print("Eelec: ", Eelec)
                    print("Eamp: ", Eamp)
                    ...
                    print("Neighbors_i: ", neighbors_i)
                    print("Transmission energy: ", neighbors_i[action]['transmission_energy'])
                    print("Reception energy: ", neighbors_i[action]['reception_energy'])
                    print("=====================================")
                    raise ValueError("Energy consumption should be positive!")
                self.update_sensor_energies(i, neighbors_i[action]['transmission_energy'])  
