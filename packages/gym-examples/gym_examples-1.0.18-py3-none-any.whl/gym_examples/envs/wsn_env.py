...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors=10, coverage_radius=50):
        ...
        low = np.array([0, lower_bound, lower_bound, 0], dtype=np.float64)
        high = np.array([initial_energy, upper_bound, upper_bound, np.inf], dtype=np.float64)

        # Create low and high arrays for all sensors
        low = np.tile(low, (self.n_sensors, 1))
        high = np.tile(high, (self.n_sensors, 1))
