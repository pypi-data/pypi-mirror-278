...
def carousel_content(request):
    ...
    for year in range(selected_year, selected_year - 5, -1):
        publication_count = User.objects.annotate(
            publication_count=Count(
                "publicationdata__metadata",
    ...
    latest_publications = (
        ...
        .order_by("-creation_time")[:number_of_latest_pub_to_display]
    )
    ...
    return render(
        ...
        "NEMO_publications/carousel.html",
        {
            ...
            **get_carousel_times(latest_publications.count()),
        },
    )


def get_carousel_times(number_of_results):
    number_of_latest_pub_to_display = PublicationsCustomization.get_int("publications_carousel_latest_display_count")
    ...
    refresh_time = (math.ceil(min(number_of_latest_pub_to_display, number_of_results) / 2) + 1) * interval_time_in_ms
    return {"interval_time": interval_time_in_ms, "refresh_time": refresh_time}
...
def jumbotron(request):
    return render(
        request,
        "NEMO_publications/jumbotron/jumbotron.html",
    )
