...
from superblocks_agent.client.Config import Config as ClientConfig
...
def _get_connection_monitor(client: Client) -> Callable[[grpc.ChannelConnectivity], None]:
    ...
    def _get_connection(self) -> grpc.Channel:
        ...
        if self._channel is None:
            channel_func = (
                ...
                else partial(grpc.secure_channel, credentials=grpc.ssl_channel_credentials())
            )
            self._channel = channel_func(target=self.config.endpoint, options=self._get_options())
