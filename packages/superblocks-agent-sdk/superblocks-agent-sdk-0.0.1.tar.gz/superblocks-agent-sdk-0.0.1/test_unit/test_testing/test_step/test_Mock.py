...
from superblocks_agent.testing.step import Mock, Params, on
...
class Test_Mock(unittest.TestCase):
    ...
    def test_return__invalid_type_given(self):
        ...
        self.assertEqual("invalid type for return: '<class 'Exception'>'", str(context.exception))
    ...
    def test_to_proto_return(self):
        ...
        self.assertEqual(
            ProtoMock.Return(static=to_protobuf_value({"foo": "bar"})), api_mock._to_proto_return()
        )
        ...
        self.assertEqual(
            ProtoMock.Return(dynamic=get_unique_id_for_object(func)), api_mock._to_proto_return()
        )
