...
class TestResult(unittest.TestCase):
    ...
    def test_from_proto_stream_responses__empty_list(self):
        ...
        self.assertEqual(Result(Result._Result__create_key, events=[], output=None), actual)
    ...
    def test_from_proto_stream_responses__without_error(self):
        ...
        expected = Result(
            Result._Result__create_key, events=[end_event, response_event], errors=[], output=output
        )
        self.assertEqual(expected, actual)
    ...
    def test_get_result__happy_path(self):
        result = Result(Result._Result__create_key, output=Output(result=to_protobuf_value("foo")))
    ...
    def test_get_block_result__has_result_and_no_error(self):
        event = Event(name="e1", end=Event.End(output=Output(result=to_protobuf_value("foo"))))
    ...
    def test_get_block_result__has_error(self):
        ...
        event = Event(
            name="e1", end=Event.End(output=Output(result=to_protobuf_value("foo")), error=error)
        )
        result = Result(Result._Result__create_key, events=[event])
        with self.assertRaises(Exception) as context:
