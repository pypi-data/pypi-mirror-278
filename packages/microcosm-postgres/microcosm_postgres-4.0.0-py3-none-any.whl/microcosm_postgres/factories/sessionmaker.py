...
def configure_sessionmaker(graph):
    ...
    """
    if graph.config.shards:
        return configure_sharded_sessionmaker(graph)
    return configure_single_sessionmaker(graph)


def configure_single_sessionmaker(graph):
    """
    Create the SQLAlchemy session class.

    """
    engine_routing_strategy = getattr(
        graph, graph.config.sessionmaker.engine_routing_strategy
    )
