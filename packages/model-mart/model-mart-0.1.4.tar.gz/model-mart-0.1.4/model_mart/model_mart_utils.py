...
class PandoraBox:
    ...
    def _upload_file(self, local_path, destination, retry_count=5, sleep_interval=5):
        attempt = 1
        while attempt <= retry_count:
            try:
                self.client.fput_object(
                    ...
                    destination,
                    local_path,
                    progress=Progress(tqdm),  # 在回调中调用线程对象的run方法
                )
                logging.info("文件上传成功")
                return None
    ...
    def _download(self, source, destination, file_hash, retry_count=5, sleep_interval=5):
        attempt = 1
        while attempt <= retry_count:
            try:
                ...
                if object_hash == file_hash:
                    ...
                    logging.error(f"文件 {source} 校验失败,当前hash: {object_hash},目标hash: {file_hash}")
                    if attempt < retry_count:
...
class Progress(tqdm):
    ...
    def set_meta(self, object_name: str, total_length: int) -> None:
        ...
        """
        self.set_description_str(f"Processing {object_name.split('/')[-1]}")
