...
def test_benchmark_print():
    ...
    console = pg.benchmark_print(
        pg.benchmark(pg.create_demo_filters(), loader), out=io.StringIO(""), err=None
    ).getvalue()
    loader = pg.load_datasets_one_community(["graph9", "bigraph"])
    ret = pg.benchmark_dict(
        pg.benchmark(pg.create_demo_filters(), loader, sensitive=pg.MannWhitneyParity)
    )
...
def test_algorithm_selection():
    for _ in supported_backends():
        _, graph, communities = next(
            pg.load_datasets_multiple_communities(["bigraph"], max_group_number=3)
        )
        ...
        supervised_algorithm = pg.AlgorithmSelection(
            algorithms.values(), measure=pg.AUC, tuning_backend="numpy"
        )
        ...
        assert (
            abs(
                ...
                - sum(modularity_aucs) / len(modularity_aucs)
            )
            < 0.05
        )
...
def test_unsupervised_vs_auc():
    ...
    algorithms = pg.create_variations(
        pg.create_many_filters(), pg.create_many_variation_types()
    )
    time_scores = pg.benchmark_scores(pg.benchmark(algorithms, loader(), pg.Time))
    assert sum(time_scores) > 0

    measures = {
        "AUC": lambda ground_truth, exclude: pg.MultiSupervised(
            pg.AUC, ground_truth, exclude
        ),
        "NDCG": lambda ground_truth, exclude: pg.MultiSupervised(
            pg.NDCG, ground_truth, exclude
        ),
        "Density": lambda graph: pg.MultiUnsupervised(pg.Density, graph),
        "Conductance": lambda graph: pg.MultiUnsupervised(
        ...
        "CCcos": lambda graph: pg.ClusteringCoefficient(
            graph, similarity="cos", max_positive_samples=5
        ),
        "CCdot": lambda graph: pg.ClusteringCoefficient(
            graph, similarity="dot", max_positive_samples=5
        ),
        "LinkAUCcos": lambda graph: pg.LinkAssessment(
            graph, similarity="cos", max_positive_samples=5
        ),
        "LinkAUCdot": lambda graph: pg.LinkAssessment(
            graph, similarity="dot", max_positive_samples=5
        ),
        "HopAUCcos": lambda graph: pg.LinkAssessment(
            graph, similarity="cos", hops=2, max_positive_samples=5
        ),
        "HopAUCdot": lambda graph: pg.LinkAssessment(
            graph, similarity="dot", hops=2, max_positive_samples=5
        ),
    }

    scores = (
        {}
    )  # measure: pg.benchmark_scores(pg.benchmark(algorithms, loader(), measures[measure])) for measure in measures}
    for (
        measure
    ) in (
        measures
    ):  # do this as a for loop, because pytest becomes a little slow above list comprehension
        print(measure)
        scores[measure] = pg.benchmark_scores(
            pg.benchmark(algorithms, loader(), measures[measure])
        )
    supervised = {"AUC", "NDCG"}
    evaluations = dict()
    for measure in measures:
        evaluations[measure] = abs(
            pg.SpearmanCorrelation(scores["AUC"])(scores[measure])
        )
    # for measure in measures:
    #    print(measure, evaluations[measure])
    assert (
        max([evaluations[measure] for measure in measures if measure not in supervised])
        == evaluations["LinkAUCdot"]
    )
...
def test_one_community_benchmarks():
    ...
    algorithms = {
        "ppr0.85": pg.PageRank(
            alpha=0.85, preprocessor=pre, max_iters=10000, tol=1.0e-9
        ),
        "ppr0.99": pg.PageRank(
            alpha=0.99, preprocessor=pre, max_iters=10000, tol=1.0e-9
        ),
        "hk3": pg.HeatKernel(t=3, preprocessor=pre, max_iters=10000, tol=1.0e-9),
        "hk5": pg.HeatKernel(t=5, preprocessor=pre, max_iters=10000, tol=1.0e-9),
        "tuned": pg.ParameterTuner(preprocessor=pre, max_iters=10000, tol=1.0e-9),
    }
    ...
    pg.benchmark_print(
        pg.benchmark_average(
            pg.benchmark_ranks(
                pg.benchmark(algorithms, loader, pg.AUC, fraction_of_training=0.8)
            )
        )
    )
...
def test_all_communities_benchmarks():
    ...
    mistreatment = lambda known_scores, sensitive_signal, exclude: pg.AM(
        [
            pg.Disparity(
                [
                    pg.TPR(
                        known_scores, exclude=1 - (1 - exclude.np) * sensitive_signal.np
                    ),
                    pg.TPR(
                        known_scores,
                        exclude=1 - (1 - exclude.np) * (1 - sensitive_signal.np),
                    ),
                ]
            ),
            pg.Disparity(
