...
def test_gnn_errors():
    ...
    class APPNP(tf.keras.Sequential):
        def __init__(self, num_inputs, num_outputs, hidden=64):
            super().__init__(
                [
                    ...
                    Dense(hidden, activation="relu", kernel_regularizer=L2(1.0e-5)),
                    Dropout(0.5),
                    Dense(num_outputs, activation="relu"),
                ]
            )
            self.ranker = pg.PageRank(
                0.9,
                renormalize=True,
        ...
        def call(self, features, graph, training=False):
            ...
            propagate = self.ranker.propagate(
    ...
    with pytest.raises(Exception):
        pg.gnn_train(
            model, graph, features, labels, training, validation, test=test, epochs=2
        )
...
def test_appnp_tf():
    ...
    with pg.Backend("tensorflow"):  # pygrank computations in tensorflow backend
        graph = pg.preprocessor(renormalize=True, cors=True)(
            graph
        )  # cors = use in many backends
        pg.gnn_train(
            model,
            features,
            graph,
