...
def _fraction2str(num, decimals=2):
    ...
    if num < 0.5 / mult:
        return "0"
    ret = str(int(num * mult + 0.5) / float(mult))
...
def benchmark_print_line(
    *line,
    ...
    print(
        delimiter.join(
            [
                _fill(_fraction2str(value, decimals=decimals), tab)
                for tab, value in zip(tabs, line)
            ]
        )
        + end_line,
        file=out,
...
def benchmark_print(
    ...
    try:
        for line in benchmark:
            if tabs is None:
                tabs = [
                    len(value) + 1 for value in line
                ]  # first line should be algorithm names
                tabs[0] = 14
