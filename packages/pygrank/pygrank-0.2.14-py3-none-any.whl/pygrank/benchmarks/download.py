...
datasets = {
    ...
    "highschool": {
        "url": "http://www.sociopatterns.org/datasets/high-school-contact-and-friendship-networks",
    ...
    "graph5": {
        "url": "https://github.com/maniospas/pygrank-datasets",
    ...
    "pubmed": {
        ...
        "pair_process": lambda row: (
            None if len(row) < 4 else [row[0].split(":")[-1], row[3].split(":")[-1]]
        ),
        "features": "Pubmed-Diabetes/data/Pubmed-Diabetes.NODE.paper.tab",
        "feature_process": lambda row: (
            None
            if len(row) < 3 or "cat=" in row[0]
            else [row[0]]
            + [val for val in row[2:] if "summary=" not in val]
            + [row[1].split("=")[-1]]
        ),
    },
    "cora": {
...
def download_dataset(
    dataset,
    path: str = os.path.join(os.path.expanduser("~"), ".pygrank/data"),
    verbose=True,
):  # pragma: no cover
    ...
    credentials = (
        "REQUIRED CITATION: Please visit the url "
        + source["url"]
        + " for instructions on how to cite the dataset "
        + dataset
        + " in your research"
    )
    ...
    if not os.path.exists(download_path + "/pairs.txt") or not os.path.exists(
        download_path + "/groups.txt"
    ):
        ...
        if "pairs" in source and not os.path.exists(download_path + "/pairs.txt"):
            if source["pairs"].startswith("http"):
                ...
                shutil.move(
                    download_path + "/" + source["pairs"], download_path + "/pairs.txt"
                )
        ...
        if "features" in source and "groups" not in source:
            if source["features"].startswith("http"):
                features_path = (
                    download_path + "/features." + source["features"].split(".")[-1]
                )
            ...
            if "features_to_groups" not in source:
                with open(
                    download_path + "/features.txt", "w", encoding="utf-8"
                ) as file:
                    for p in features:
        ...
        if "features" in source and "groups" in source:
            if source["features"].startswith("http"):
                ...
                shutil.move(
                    download_path + "/" + source["features"],
                    download_path + "/features.txt",
                )

        if (
            "groups" in source
            and not os.path.exists(download_path + "/groups.txt")
            and not os.path.exists(download_path + "/groups.txt.debug")
        ):
