    ...
    def rank(
        ...
        **kwargs
        ...
        if self.method == "safe":
            ...
            threshold = min(
                ranks[u] for u in personalization if personalization[u] == 1
            )
            ...
            personalization = {
                ...
                if ranks[v] >= threshold or personalization.get(v, 0) == 1
            }
            return self.ranker.rank(graph, personalization, **kwargs)
...
class BoostedSeedOversampling(Postprocessor):
    ...
    def __init__(
        ...
        self._weight_convergence = (
            ConvergenceManager(error_type=MaxDifference, tol=0.001, max_iters=100)
            if weight_convergence is None
            else weight_convergence
        )

    def _boosting_weight(self, r0_N, Rr0_N, RN):
        if self._objective == "partial":
            a_N = sum(r0_N[u] * Rr0_N[u] ** 2 for u in r0_N) / float(
                sum(Rr0_N[u] ** 2 for u in Rr0_N)
            ) - sum(r0_N[u] * RN.get(u, 0) * Rr0_N[u] for u in r0_N) / float(
                sum(Rr0_N[u] ** 2 for u in Rr0_N)
            )
        elif self._objective == "naive":
            a_N = 0.5 - 0.5 * sum(
                r0_N[u] * RN.get(u, 0) * Rr0_N[u] for u in r0_N
            ) / float(sum(Rr0_N[u] ** 2 for u in Rr0_N))
        else:
        ...
        **kwargs
        ...
        while not self._weight_convergence.has_converged(a_N):
            if self._oversample_from_iteration == "previous":
                ...
                threshold = min(
                    RN[u] for u in personalization if personalization[u] == 1
                )
            else:
                raise Exception(
                    "Boosting only supports oversampling from iterations: previous, original"
                )
    ...
    def _reference(self):
        return (
            "iterative "
            + self._objective
            + " boosted seed oversampling of "
            + self._oversample_from_iteration
            + " node scores \\cite{krasanakis2019boosted}"
        )
