    ...
    with client.connect(
        **backend_client.get_playground_websocket_args(
        ...
        for message in websocket:
            processed_message = (
                wire_model.PlaygroundExecuteLlmProgramRequest.model_validate_json(  # pylint: disable=line-too-long
                    message))

            execution_results = []
            for execution_spec in processed_message.specs:
                ...
                with execution.capture_default_hparams() as default_hparams:
                    execution_details, _ = test_suite_execution.execute_test_case(
                        llm_program_fully_qualified_name=llm_program_fqn,
                        test_case=test_case,
                        hparams=hparams,
                        mode="PLAYGROUND")
            ...
            response = wire_model.PlaygroundExecuteLlmProgramResponse(
