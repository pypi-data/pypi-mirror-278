...
def db_get_indexes(errors: list[str],
    ...
    If *omit_pks* is set to 'True' (the default value),
    indexes created on primary key columns will no be included.
    ...
    if not op_errors:
        ...
        match curr_engine:
            case "mysql":
                pass
            case "oracle":
                sel_stmt: str = f"SELECT ai.index_name FROM all_indexes ai "
                if omit_pks:
