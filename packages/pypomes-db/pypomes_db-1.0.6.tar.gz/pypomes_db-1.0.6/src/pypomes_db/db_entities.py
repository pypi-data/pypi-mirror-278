...
def db_get_views(errors: list[str],
    ...
    if not op_errors:
        ...
        if curr_engine == "oracle":
            ...
            sel_stmt: str = "SELECT table_name FROM information_schema.views"
            if schema:
...
def db_get_indexes(errors: list[str],
    ...
    If the list of table names *tables* is provided, only the indexes created on any of these names are returned.

    :param errors: incidental error messages
    :param schema: optional name of the schema to restrict the search to
    :param tables: optional list of tables that the indexes were created on
    :param engine: the database engine to use (uses the default engine, if not provided)
    :param connection: optional connection to use (obtains a new one, if not provided)
    :param committable: whether to commit upon errorless completion ('False' requires 'connection' to be provided)
    :param logger: optional logger
    ...
    if not op_errors:
        ...
        match curr_engine:
            case "mysql":
                pass
            case "oracle":
                sel_stmt: str = f"SELECT index_name FROM all_indexes"
                if schema or tables:
                    sel_stmt += " WHERE "
                    if schema:
