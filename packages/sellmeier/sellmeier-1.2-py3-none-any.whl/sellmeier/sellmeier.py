...
def λλλ(w1,w2,w3=None):
    return qpmwavelengths(w1,w2,w3)
...
def index(w,sell,temp=20,warn=True): # x is wavelength in nm
    ...
    if sell=="che" or sell=="chez":
        ...
        n = n + (temp-20) * ( 12.415e-6/(x)**3 -44.414e-6/(x)**2 +59.129e-6/(x) -12.101e-6 )  ##temp dep (Wiechmann KTP)
    if sell=="chestepx":
        n = index(1000*x,'rtpx',20)-index(1000*x,'chex',20)
    if sell=="chestepy":
        n = index(1000*x,'rtpy',20)-index(1000*x,'chey',20)
    if sell=="chestepz":
        n = index(1000*x,'rtpz',20)-index(1000*x,'chez',20)
    if sell=="rtpx":
    ...
    if sell=="rtpmik" or sell=="rtpmikz":
        ...
        n = n + (temp-20) * ( 12.415e-6/(x)**3 -44.414e-6/(x)**2 +59.129e-6/(x) -12.101e-6 )  ##temp dep (Wiechmann KTP)
    if sell=="mikstepx":
        n = index(1000*x,'rtpmikx',20)-index(1000*x,'katx',20)
    if sell=="mikstepy":
        n = index(1000*x,'rtpmiky',20)-index(1000*x,'katy',20)
    if sell=="mikstepz":
        n = index(1000*x,'rtpmikz',20)-index(1000*x,'katz',20)
    if sell=="rtax":
    ...
    if sell=="ktpsurfx": # copied from ktpsurfy as a best estimate
        n = nsurffunc(1000*x,29.0815525865133,-6.58500255570493,2.13893700130814,0,0.00960546909966567,39.2004747033197)
        assert 20==temp, "ktpsurfx temperature dependence not defined"
    if sell=="slt":
...
def pairrate(w1,w2=None,temp=20,sell='ktpwg',Type='zzz',w3=None,npy=None,npz=None,L=10,ηSHG=100,nW=False,pernm=False,units=False): # L in mm, ηSHG = SHG equivalent con. eff. in %/W/cm²
    # todo: "input on pair rates" email 2-19-24 #   Ps = ηSFG hc² Pp L² (λp²/λi) (Δλs/λs⁴)
...
def phasematchcurve(w1=None,w2=None,Λ=None,x0=500,x1=3000,dx=5,prop=None,plot=False,qpmargs=None,**ppargs): # prop=(sign(w1),sign(w2)), only needed for Λ input (w1=w2=None)
...
def loadshgscan(file,skip=None,returnce=True,pumpince=False,check=True):
    def findlineinfile(s,file):
        with open(file,'r') as f:
            for n,line in enumerate(f):
                if line.startswith(s):
    ...
    assert not check or np.allclose(0,ce-100*shg/pumpout**2)
