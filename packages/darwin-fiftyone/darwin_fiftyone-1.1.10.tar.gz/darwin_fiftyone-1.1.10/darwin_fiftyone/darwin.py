...
class DarwinBackend(foua.AnnotationBackend):
    ...
    def upload_annotations(self, samples, anno_key, launch_editor=False):
        ...
        if anno_key is None:
            ...
            logger.info(f"No annotation key provided. Generating one with key: {anno_key}")
...
class DarwinAPI(foua.AnnotationAPI):
    ...
    def upload_annotations(self, samples, anno_key, backend):
        ...
        if Groups:
            ...
            for sample in samples:
                if sample["group"]["id"] not in group_sample_id_map:
                    group_sample_id_map[sample["group"]["id"]] = {sample["group"]["name"]: sample.id}
                else:
                     group_sample_id_map[sample["group"]["id"]].update({sample["group"]["name"]: sample.id})


        voxel_file_list = [k for k in filename_sample_id_map.keys()] if not Groups else [k for k in group_sample_id_map.keys()]
        ...
        #Can be used to handle the duplicate name issue. Doesn't support groups
            ...
            assert not Groups, "Custom filename sample id map not currently supported with groups"
            item_sample_map = {}
            for item in annotation_run_items:
                try:
                    item_sample_map[item.get("id")] = {
                        "filename": item.get("name"),
                        "sample_slots": {"0":custom_filename_sample_id_map[item.get("id")]},
                    }
                except:
        ...
        #Groups external storage
            ...
            item_sample_map = {
                item.get("id"): {
                    "filename": item.get("name"),
                    "sample_slots": {"0": group_sample_id_map[item.get("name")]},
                }
                for item in annotation_run_items
            }
    ...
    def _convert_image_annotations_to_v7(
        self, sample, frame_size, label_schema, id_maps, backend, slot_name, frame_val=None
    ):
        '''
                ...
                for annotation in annotations:
                    ...
                    darwin_annotations.extend(
                        self._convert_image_annotation_to_v7(
                            annotation, label_type, frame_size, sample, backend,slot_name, attributes
                        )
                    )
    ...
    def _convert_image_annotation_to_v7(
        self, annotation, label_type, frame_size, sample,backend, slot_name = "0", attributes=None
    ):
        """
        ...
        if label_type == "polylines":
            if annotation.closed:
                ...
                if len(annotation.points) == 1:
                    ...
                    logger.info("Multiple open polylines not supported in V7 Darwin. Please add these as separate annotations.")
            return [darwin_annotation]
        ...
        with (
            ...
            for label_field, label_info in label_schema.items():
                    ...
                    if is_video:
                        ...
                        for frame_number, frame in sample.frames.items():
                            ...
                            annotations = self._convert_image_annotations_to_v7(
                                frame, frame_size, label_schema, id_maps, backend,slot_name, frame_val
                            )
                            for annotation in annotations:
                        ...
                        annotations = self._convert_image_annotations_to_v7(
                            sample, frame_size, label_schema, id_maps, backend,slot_name
                        )
                ...
                if backend.config.Groups:
                    importer.import_annotations(dataset, parser, files_to_import, append=True, class_prompt=False)
                else:
    ...
    def _upload_props(self,backend,team_slug,class_schema,label_type):
        ...
        for prop in class_schema["attributes"].keys():
            if "text" in class_schema["attributes"][prop]["type"]:
                logger.info("Text attribute not supported in V7 Darwin. Please use the test subattribute instead.")
                continue
            ...
            for cls in class_schema["classes"]:
                ...
                if not properties_check:
                    ...
                    if response.ok:
                        ...
                        logger.info(f"Property {prop} already exists in class {cls} with the same values")
                    else:
                ...
                for annotation_path in export_path.glob("*.json"):
                    ...
                    if is_darwin_video:
                        ...
                        for annotation in video_annotations:
                            ...
                            sample_id = item_sample_map[item_id]["sample_slots"][slot_name]
                            ...
                            if "polygon" not in frame_annotation and "bounding_box" not in frame_annotation and "tag" not in frame_annotation and "keypoint" not in frame_annotation and "line" not in frame_annotation:
                            ...
                            if "polygon" in frame_annotation and label_type in ["polygons", "polylines"]:
                                if len(frame_annotation["polygon"]["paths"]) == 1 and not OBB:
                                ...
                                if OBB:
                                    ...
                                    OBB_path = _v7_polygon_to_obb(frame_annotation["polygon"])
                                    voxel_annotation = _v7_to_51_polygon(
                                        annot_name,
                                        OBB_path,
                                        height,
                            ...
                            elif "line" in frame_annotation and label_type in ["polyline","polylines"]:
                            ...
                            elif "keypoint" in frame_annotation and label_type in ["keypoints","keypoint"]:
                            ...
                            if sample_id in annotations:
                                sample_annotations[sample_id].update(annotations)
                    ...
                    #Image annotations
                        ...
                        for annotation in data["annotations"]:
                            ...
                            #Checks for unsupported annotation types
                            if "polygon" not in annotation and "bounding_box" not in annotation and "tag" not in annotation and "keypoint" not in annotation and "line" not in annotation:
                                logger.warn("WARNING, unsupported annotation type", annotation)
                                continue
                            ...
                            elif "line" in annotation and label_type in ["polyline","polylines"]:
                                voxel_annotation = _v7_to_51_polyline(
                                        annot_name,
                                        annotation["line"],
                                        height,
                            ...
                            elif "keypoint" in annotation and label_type in ["keypoints","keypoint"]:
                                voxel_annotation = _v7_to_51_keypoint(
                                    annot_name,
                                    annotation["keypoint"],
                                    height,
                            ...
                            if sample_id in sample_annotations:
                                sample_annotations[sample_id].update({voxel_annotation.id: voxel_annotation})

                            else:
                                sample_annotations[sample_id] = {voxel_annotation.id: voxel_annotation}
...
class DarwinResults(foua.AnnotationResults):
    ...
    def cleanup(self):
        ...
        api = self.connect_to_api()
        client = api.client
        answer = input("Are you sure you want to delete the V7 dataset and workflow for this annotation run? (y/n)")
        if answer.lower() == "y":
...
def _multislot_registration(groups, api_key, dataset_slug, team_slug, external_storage, base_url):
    """
    ...
    for k in groups:
        ...
        for sample in group:
            ...
            if sample.media_type in ["image","video"]:
                ...
                logger.warning(f"Media type {sample.media_type} not supported for multi slot item registration. Skipping item registration")
        ...
        if response.ok:
            ...
            logger.error(f"Multi slot item with id {group_id} registration failed with status code {response.status_code}. Skipping item registration")
...
def _upload_multislot_items(groups, api_key, dataset_slug, team_slug, base_url):
    ...
    for k in groups:
        ...
        if response.ok:
            ...
            logger.error(f"Multi slot item with id {group_id} upload failed with status code {response.status_code}. Skipping item upload")
...
def _51_to_v7_polygon(points, frame_size):
    ...
    """
    width, height = frame_size
    new_poly = {"paths": [[{"x": x * width, "y": y * height} for x, y in point_list] for point_list in points]}
    return new_poly
...
def _v7_to_51_polyline(label, polyline, height, width, attributes=None):
    ...
    """
    line_list = [[(keypoint["x"] / width, keypoint["y"] / height) for keypoint in polyline["path"]]]
...
def _v7_to_51_polygon(label, polygon, height, width, attributes=None,OBB=False):
    ...
    poly_list = [[(keypoint["x"] / width, keypoint["y"] / height) for keypoint in poly_path] for poly_path in polygon["paths"]]
...
def _v7_polygon_to_obb(polygon_paths):
    ...
    """
    points= np.array([[p['x'], p['y']] for path in polygon_paths["paths"] for p in path])
