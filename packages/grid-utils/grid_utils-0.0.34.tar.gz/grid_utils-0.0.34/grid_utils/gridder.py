...
__all__ = [
    'NullProj',
    'GridderBase', 'XYGridderBase',
    'XYProjGridder',
    'LonLatSurroundingGridder',
    'XYIrregularGridder',
    'GribGridder',
]
...
class XYProjGridder(XYGridderBase):
    ...
    def proj(self, p):
        if p is None:
            ...
            if isinstance(p, NullProj):
                self._proj = p
            else:
                raise ModuleNotFoundError("pyproj is required here, which is not installed!")
...
class GribGridder(GridderBase):
    RES_MAPPING = {
        '1p': 1.0, '1p0': 1.0, '1p00': 1.0,
        '0p5': 0.5, '0p50': 0.5,
        '0p25': 0.25,
        '0p125': 0.125,
        '0p1': 0.1, '0p10': 0.1,
    ...
    def __init__(self, resolution='0p25'):
        self.resolution = resolution
        if isinstance(self.resolution, str):
            self._float_res = self.RES_MAPPING[self.resolution]
        else:
            self._float_res = self.resolution

        self.nx = int(360 / self._float_res)
        self.ny = int(180 / self._float_res) + 1

    def i2x(self, i, j):
        if i < 0 or j < 0 or i >= self.nx or j >= self.ny:
            raise OutOfGridBound("i: {}, j: {} is out of bound!".format(i, j))
        x = i * self._float_res
        y = 90.0 - j * self._float_res
        return x, y

    def x2i(self, x, y):
        x_off = (x + 360.0 + self._float_res / 2) % 360
        y_off = 90 - y + self._float_res / 2

        i = int(x_off / self._float_res)
        j = int(y_off / self._float_res)
        return i, j
