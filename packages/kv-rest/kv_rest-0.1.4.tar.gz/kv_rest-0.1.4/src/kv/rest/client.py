...
class ClientKV(KV[A], Generic[A]):
  ...
  async def _read(self, key: str):
    ...
    return root_validate(r, self.ReadModel()).unsafe().root.unsafe()
  
  @E.do[DBError]()
  async def _insert(self, key: str, value: A):
    r = (await self._req('POST', f'insert?key={key}', json=self.Model()(value).model_dump())).unsafe()
    return root_validate(r, self.DBErrModel()).unsafe().root.unsafe()
  ...
  async def _items(self, batch_size: int | None = None) -> AsyncIterable[Either[DBError | InvalidData, tuple[str, A]]]:
    ...
    if r.tag == 'left':
      yield Left(r.value)
      return
    items = root_validate(r.value, self.ItemsModel())
    if items.tag == 'left':
  ...
  async def _keys(self) -> Sequence[str]:
    ...
    return root_validate(r, self.KeysModel()).unsafe().root.unsafe()
