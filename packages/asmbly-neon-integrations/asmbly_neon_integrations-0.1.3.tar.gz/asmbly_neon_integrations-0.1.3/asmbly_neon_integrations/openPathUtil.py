...
from asmbly_neon_integrations.credentials import AltaCredentials, GmailCredentials
...
#################################################################################
def createUser(creds: AltaCredentials, neonAccount):
    ...
    if not dryRun:
        ...
        if userAge.seconds > 300:
            ...
            deleteAllCredentialsForId(creds, opUser.get("id"))
            ...
            response = requests.patch(url, data=data, headers=creds.headers)
            if response.status_code != 200:
                raise ValueError(
        ...
        neonUtil.updateOpenPathID(creds, neonAccount)
    else:
...
def updateOpenPathByNeonId(
    creds: AltaCredentials, gmail_creds: GmailCredentials, neonId
):
    logging.info("Updating Neon ID %s", neonId)
    account = neonUtil.getMemberById(creds, neonId)
    # logging.debug(account)
    if account.get("OpenPathID"):
        updateGroups(creds, gmail_creds, account, email=True)
    elif neonUtil.accountHasFacilityAccess(account):
        account = createUser(creds, account)
        updateGroups(
            creds, gmail_creds, account, openPathGroups=[]
        )  # pass empty groups list to skip the http get
        createMobileCredential(creds, account)
