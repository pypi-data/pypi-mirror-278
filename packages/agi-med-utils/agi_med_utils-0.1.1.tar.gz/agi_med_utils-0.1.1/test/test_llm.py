...
def test_giga_emb():  
    ...
    embs = giga.get_embeddings([warmup_sent, warmup_sent])
    more_embs = giga.get_embeddings([warmup_sent for _ in range(1024)])
    assert isinstance(emb1, list)
    assert isinstance(embs, list)
    assert isinstance(more_embs, list)
    assert len(emb1) == giga.DIM
    assert len(embs) == 2
    assert len(more_embs) == 1024
...
def test_giga_plus_emb():  
    ...
    embs = giga_plus.get_embeddings([warmup_sent, warmup_sent])
    more_embs = giga_plus.get_embeddings([warmup_sent for _ in range(1024)])
    assert isinstance(emb1, list)
    assert isinstance(embs, list)
    assert isinstance(more_embs, list)
    assert len(emb1) == giga.DIM
