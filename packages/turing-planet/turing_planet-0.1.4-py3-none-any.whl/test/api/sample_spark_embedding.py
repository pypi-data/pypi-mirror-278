...
## 本demo中调用两个服务：
# 1、embeddingp ： 用于将客户传入的知识内容进行向量化
# 2、embeddingq ： 用于将提的问题进行向量化
...
def get_embq_embedding(text, appid, apikey, apisecret):
    host = 'http://cn-huabei-1.xf-yun.com/v1/private/s50d55a16'
...
def get_embp_embedding(text, appid, apikey, apisecret):
    host = 'http://cn-huabei-1.xf-yun.com/v1/private/sa8a05c27'
    url = assemble_ws_auth_url(host, method='POST', api_key=apikey, api_secret=apisecret)
    content = get_Body(appid, text)
    print(time.time())
    response = requests.post(url, json=content, headers={'content-type': "application/json"}).text
...
if __name__ == '__main__':
    ...
    APPID = '5e815bfb'
    APISecret = '9fe7e550b771b61d356373aa53dc646f'
    APIKEY = '69c1da82a1cfd419fd5ba1bba32e2a66'
    ...
    res = get_embp_embedding(text=ques, appid=APPID, apikey=APIKEY, apisecret=APISecret)
    # print(res)
    parser_Message(res)
