...
class ImageBuild(BaseModel):
    ...
    def build(self, client: "docker.DockerClient", image_version: str, deps: list["ImageBuild"] = []) -> str:
        ...
        self.tag = f"{self.prefix}{separator}{self.name}-{image_version}"
        ...
        if exists:
            ...
            print(f"Building {self.name} as {self.tag} with dockerfile {self.dockerfile} in {self.context}")
            ...
            default_platform = "linux/arm64" if "arm64" in os.uname().machine else "linux/amd64"
            docker_command = [
                "docker",
                "buildx",
                "build",
                "--platform",
                ",".join(self.platforms) if self.platforms else os.getenv('BUILD_PLATFORMS', default_platform),
                "-t",
                self.tag,
                self.context,
                "-f",
