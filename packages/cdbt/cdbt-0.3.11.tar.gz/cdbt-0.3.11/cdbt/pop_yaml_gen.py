...
class BuildCBCUtilsYAML:
    ...
    def build_pop_yaml(self, select_target: str, include_avg: bool = False):
        ...
        if not query_args:
            raise Exception('Macro not found in the file. Please check the macro name and try again.')
        ...
        output += self.build_pop_lag_output(column_names, lookback_values, grain, column_prefix, include_avg)
    ...
    def extract_macro_args(self, query: str, macro_name: str):
        # remove newlines
        query = query.replace('\n', '').replace(' ', '')
        pattern = fr'cbc_utils\.{macro_name}\((.*?)\)'
    ...
    def build_pop_lag_output(self, column_names, look_back_values, grain, column_prefix=None, include_avg=False):
        output = ''
        ...
        for column_name in column_names:
            for look_back_val in look_back_values:
                if 'revenue' in column_name.lower() or 'cost' in column_name.lower():
                    ...
                    format_str = ''
                if column_prefix:
                    column_alias = f'{column_prefix}_{column_name}_{look_back_val}{grain}'
                else:
