...
def check_prefixes(key):
    ...
    from seachad.common_libraries import common
    import os
    ...
    def replace_at_start(string, old, new):
        """
        Replace 'old' with 'new' only if 'old' appears at the start of 'string'.
        
        :param string: The original string.
        :param old: The substring to be replaced if it appears at the start.
        :param new: The substring to replace 'old' with.
        :return: The modified string if 'old' appears at the start, otherwise the original string.
        """
        if string.startswith(old):
            return new + string[len(old):]
        return string        
    # key = key.replace(deploy_prefix, "") # ? esto da error si no se recupera el deploy_prefix
    key = replace_at_start(key, deploy_prefix, "")
...
def show_cache_prefixed_ns_fast(cache_prefix_to_clean = None, singleton = False):
    """
    Muestra todas las claves que comienzan con un prefijo concreto
    :param ns: str, namespace i.e your:prefix
...
# ? uso de claves como si se tratara de un diccionario. Si se quiere usar como un diccionario, se puede hacer así
# ? usar un diccionario al que se pueda acceder, cambiar y obtener la información en un chache de REDIS sin bloqueos puede dar problemas
# ? dado que varios threads pueden acceder y cambiar información de forma simultánea y desincronizada, produciendo errores
# ? las funciones de abajo permiten usar un diccionario y descomponer sus claves, por defecto se descomponen pero podrían seguir usándose como diccionario

# ? -------------------------------------------------------------------------------------------------------------------------------------------------------

DICT_OR_INDIVIDUAL_KEYS_MODE = "individual" # ? establece el modo en que se opera con el cache, para evitar problemas de bloqueo...

def clean_cache_keys(cache_prefix, cache_sub_prefix = "", dict_or_individual_keys = DICT_OR_INDIVIDUAL_KEYS_MODE, **kwargs):
    """
    (c) Seachad - FGV
    ...
    ---------------------- 
        clean_cache_keys
        Borra todas las claves que comiencen por un prefijo dado
    ...
    Args:
    -----
            - cache_prefix (_type_): _description_
            - cache_sub_prefix (str, optional): _description_. Defaults to SUB_PREFIX_CARGAS.
            - dict_or_individual_keys (str, optional): _description_. Defaults to "dict". -> dict funciona cogiendo un diccionario y actualizándolo, pero puede dar problemas de LOCKS cuando varios procesos acceden a la misma clave, individual_keys no daría problemas
            porque cada proceso accede a su clave (no tiene LOCKS)
    """
    singleton = kwargs.get("singleton", False)
    
    if dict_or_individual_keys == "dict":
        _CACHE_set(f"{cache_prefix}{cache_sub_prefix}", {}, singleton = singleton)
    else:
        if cache_prefix == "ALL":
            clear_cache_prefixed_ns_fast(force_clean = True, singleton = singleton)
        else:
            clear_cache_prefixed_ns_fast(cache_prefix_to_clean = cache_prefix, force_clean = True, singleton=singleton)        
            

def update_cache_keys(key, dict_original, **kwargs):
    """
    (c) Seachad - FGV
    ...
    ---------------------- 
        update_cache_keys
        Actualiza una clave en el cache con un diccionario
        No guarda nada en el cache, sólo actualiza el diccionario para luego poder operar con él con las funciones get_cache_keys y set_cache_keys
    ...
    -----
            - key (_type_): _description_
            - dict_to_update (_type_): _description_
    """
    
    for k,v in kwargs.items():
        dict_original[key] = dict_original.get(key, {})
        dict_original[key][k] = v
    return dict_original    

def get_cache_keys(cache_prefix, cache_sub_prefix = "", dict_or_individual_keys = "dict", get_rid_of_prefix_in_return = True, key = None, **kwargs):
    """
    (c) Seachad - FGV
    ...
    -----
            - cache_prefix (_type_): _description_
            - cache_sub_prefix (str, optional): _description_. Defaults to SUB_PREFIX_CARGAS.
            - dict_or_individual_keys (str, optional): _description_. Defaults to "dict".
    ...
    if dict_or_individual_keys == "dict":
        dict_to_return = _CACHE_get(f"{cache_prefix}INFORMATION", {}, singleton=singleton)
    else:
        if not key:
            # ? cogemos todas las claves que comienzan por cache_prefix
            dict_to_return = get_cache_prefixed_ns_fast(cache_prefix_to_clean = f"{cache_prefix}*", singleton=singleton)
        else:
            return _CACHE_get(f"{cache_prefix}{key}", {},singleton=singleton)
        pass
    # cogemos todas las claves, las cargamos en un diccionario, lo enviamos
    # el diccionario contiene las claves con prefijos, así que si me lo piden, se lo quito
    if get_rid_of_prefix_in_return:
        clean_dict_to_return = {}
        for k,v in dict_to_return.items():
            clean_dict_to_return[k.replace(cache_prefix, "")] = v
        return clean_dict_to_return
    return dict_to_return

def set_cache_keys(dict_to_set, cache_prefix, cache_sub_prefix = "", dict_or_individual_keys = "dict", key = None, **kwargs):
    """
    (c) Seachad - FGV
    ...
    ---------------------- 
        set_cache_keys
        coge un diccionario y genera tantas claves con el cache_prefix como claves tenga el diccionario
    ...
    Args:
    -----
            - dict_to_set (_type_): _description_
            - cache_prefix (_type_): _description_
            - cache_sub_prefix (str, optional): _description_. Defaults to SUB_PREFIX_CARGAS.
            - dict_or_individual_keys (str, optional): _description_. Defaults to "dict".
    """
    singleton = kwargs.get("singleton", False)        
    if dict_or_individual_keys == "dict":
        _CACHE_set(f"{cache_prefix}{cache_sub_prefix}", dict_to_set, singleton=singleton)
    else:
        if key == None: # si no me mandan una clave, meto el diccionario completo
            for k,v in dict_to_set.items():
                _CACHE_set(f"{cache_prefix}{k}", v, singleton=singleton)        
        else:
            _CACHE_set(f"{cache_prefix}{key}", dict_to_set[key], singleton=singleton)


# ? fin de tratamiento como diccionarios


def test_cache():
    
    tc._print(f"Test SIMPLE", tc.Y)
    
    # ? creo una clave, la obtengo y la borro
    key = "TMP_TEST"
    ...
    value = _CACHE_get(key)
    if value == None:
        tc._print(f"Test simple correcto", tc.G)
    else:
        tc._print(f"Test simple incorrecto", tc.R)


    tc._print(f"Test DICT", tc.Y)
    # cargo información en el caché, la obtengo y la borro
    PREFIX_CARGAS = "CRG_"
    SUB_PREFIX_CARGAS = "CARGAS"
    
    # creo cargas para 3 elementos
    cargas_information_dict = {}
    for i in range(3):
        str_uuid = f"{i}"
        cargas_information_dict = update_cache_keys(str_uuid, cargas_information_dict, status = "started", timestamp = datetime.datetime.now().strftime("%y:%m:%d %H:%M:%S"))
        set_cache_keys(dict_to_set =  cargas_information_dict, cache_prefix = PREFIX_CARGAS, cache_sub_prefix = SUB_PREFIX_CARGAS, dict_or_individual_keys = DICT_OR_INDIVIDUAL_KEYS_MODE, key = str_uuid)
    
    # leo la información para los elementos
    for i in range(3):
        str_uuid = f"{i}"
        cargas_information_dict = get_cache_keys(cache_prefix = PREFIX_CARGAS, cache_sub_prefix = SUB_PREFIX_CARGAS, dict_or_individual_keys = DICT_OR_INDIVIDUAL_KEYS_MODE, key = str_uuid)
        tc._print(f"{cargas_information_dict}", tc.G, prompt = False)
    
    # cambio valores en claves
    for i in range(3):
        str_uuid = f"{i}"
        cargas_information_dict = update_cache_keys(str_uuid, cargas_information_dict, status = "finished", timestamp = datetime.datetime.now().strftime("%y:%m:%d %H:%M:%S"))
        set_cache_keys(dict_to_set =  cargas_information_dict, cache_prefix = PREFIX_CARGAS, cache_sub_prefix = SUB_PREFIX_CARGAS, dict_or_individual_keys = DICT_OR_INDIVIDUAL_KEYS_MODE, key = str_uuid)    
    ...
    for i in range(3):
        str_uuid = f"{i}"
        cargas_information_dict = get_cache_keys(cache_prefix = PREFIX_CARGAS, cache_sub_prefix = SUB_PREFIX_CARGAS, dict_or_individual_keys = DICT_OR_INDIVIDUAL_KEYS_MODE, key = str_uuid)
        tc._print(cargas_information_dict, tc.B, prompt=False)
        
    # borro las claves
    clean_cache_keys(PREFIX_CARGAS, cache_sub_prefix = SUB_PREFIX_CARGAS, dict_or_individual_keys = DICT_OR_INDIVIDUAL_KEYS_MODE)    
    ...
    # comprobamos que efectivamente no hay claves que comiencen con PREFIX_CARGAS
    show_cache_prefixed_ns_fast(cache_prefix_to_clean = PREFIX_CARGAS, singleton = False)        
