    ...
    for g, df in feature_location.groupby(['FEATURE_DATABASE', 'FEATURE_TABLE']):
        ...
        if time_manager is not None:
            if 'date' in time_manager.data_type.lower():
                ...
                INNER JOIN (SEL FEATURE_ID, FEATURE_NAME, DATA_DOMAIN FROM {tdfs4ds.SCHEMA}.{tdfs4ds.FEATURE_CATALOG_NAME} WHERE DATA_DOMAIN = '{tdfs4ds.DATA_DOMAIN}') B
            ...
            INNER JOIN ({validtime_statement}  SEL FEATURE_ID, FEATURE_NAME, DATA_DOMAIN FROM {tdfs4ds.SCHEMA}.{tdfs4ds.FEATURE_CATALOG_NAME} WHERE DATA_DOMAIN = '{tdfs4ds.DATA_DOMAIN}') B
            ON A.FEATURE_ID = B.FEATURE_ID                
            WHERE  ({condition})
            ) A{counter}
            """
