    ...
    def run(self, **kwargs):
        for node in self.document.traverse(translation_link):
            if 'html' in self.app.builder.name:
                ...
                # 获取相对于 srcdir 的文档路径
                doc_relative_path = os.path.relpath(env.doc2path(docname), start=env.srcdir)
                ...
                # 生成新的 URL
                if current_language in new_doc_path:
                    new_doc_path = new_doc_path.replace(current_language, target_language)
                else:
                    new_doc_path = f"{target_language}/html/{new_doc_path}"

                url = "{}.html".format(new_doc_path)
