    ...
    def run(self, **kwargs):
        ...
        for node in self.document.traverse(translation_link):
            if 'html' in self.app.builder.name:
                ...
                if not doc_path.startswith(env.srcdir, doc_path):
                    doc_path = os.path.join(env.srcdir, doc_path)
                current_html_dir=os.path.dirname(doc_path)
                return_path = os.path.relpath(current_html_dir, start=os.path.join(env.srcdir, 'html'))
                target_path = os.path.join(return_path, language, 'html', docname)
