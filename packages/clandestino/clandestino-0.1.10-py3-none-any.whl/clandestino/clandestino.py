...
def _load_migration_repository() -> cdti.IMigrateRepository:
    ...
    elif config("CLANDESTINO_MIGRATION_REPO", default=False) == "SQLITE":
        return _cdts.repository.SQLiteMigrateRepository()
...
def _get_ref_migration(migration_type: str = None) -> str:
    ...
    elif migration_type == "SQLITE":
        return _cdts.ref.__file__
    else:
        from .src import ref
    ...
    if not _loaded:
        ...
        try:
            ...
            if config("CLANDESTINO_MIGRATION_REPO", default=False) == "ELASTICSEARCH":
                raise Exception(
                    "You define to use ELASTICSEARCH but not install the extra package clandestino_elasticsearch"
                )

        try:
            import clandestino_sqlite

            global _cdts
            _cdts = clandestino_sqlite
        except ImportError as error:
            if config("CLANDESTINO_MIGRATION_REPO", default=False) == "SQLITE":
                raise Exception(
                    "You define to use SQLITE but not install the extra package clandestino_sqlite"
                )
    _loaded = True
