...
class Generator(nn.Module):
    ...
    def forward(self, input, labels):
        ...
        g_input = torch.cat((noise, label_embeddings), 1)
...
if __name__=="__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            gen_lab = torch.randint(0, 2, (batch_size,), device=device)
    ...
    os.makedirs(f"result/gan/{task}/{fold}", exist_ok=True)

    torch.save(netG.state_dict(), f"result/gan/{task}/{fold}/gan_weights")

    with open(f"result/gan/{task}/{fold}/G_losses.pickle", 'wb') as f:
        pickle.dump(G_losses, f)

    with open(f"result/gan/{task}/{fold}/D_losses.pickle", 'wb') as f:
