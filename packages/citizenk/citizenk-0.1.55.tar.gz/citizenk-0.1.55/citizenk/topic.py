...
class Topic:
    ...
    def __init__(
        ...
        self.sent_keys_sample = set()
        self.receive_keys_sample = set()
    ...
    def info(self, lags: dict[str, int] = {}, assignments: dict[str, list[int]] = {}):
        topic_info = {
            ...
            "sent_keys_sample": self.sent_keys_sample,
            "receive_keys_sample": self.receive_keys_sample,
    ...
    def send(
        ...
        # Update keys stats
        self.sent_keys_sample.add(key)
        if len(self.sent_keys_sample) > self.MAX_KEYS_IN_STAT:
            self.sent_keys_sample.pop()
    ...
    def deserialize(self, key: str | bytes, value: bytes) -> BaseModel:
        ...
        # Update keys stats
        self.receive_keys_sample.add(key)
        if len(self.receive_keys_sample) > self.MAX_KEYS_IN_STAT:
            self.receive_keys_sample.pop()
