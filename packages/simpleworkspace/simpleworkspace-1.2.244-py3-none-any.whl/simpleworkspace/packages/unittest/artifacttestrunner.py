from typing import Tuple as _Tuple, Type as _Type, TypeAlias as _TypeAlias
from types import TracebackType as _TracebackType
import unittest as _unittest
from enum import Enum as _Enum

class CaseStatusEnum(_Enum):
    Success = 0
...
class _CaseResult:
    ...
    def __init__(self,
            case: _unittest.TestCase, status:CaseStatusEnum, timeElapsed:float = 0,
    ...
    def IsSuccess(self):
        ...
        return self.status in (CaseStatusEnum.Success, CaseStatusEnum.Skip, CaseStatusEnum.ExpectedFail)
...
class _ArtifactTestResult(_unittest.TestResult):
    ...
    def addSubTest(self, test, subtest, err):
        ...
        if err is None:
            self.successes.append(_CaseResult(
                case=subtest, status=CaseStatusEnum.Success, timeElapsed=self._last_testCaseDuration,
            ...
            if issubclass(err[0], test.failureException):
                target, status = self.failures, CaseStatusEnum.Fail
            else:
                target, status = self.errors, CaseStatusEnum.Error
            target.append(_CaseResult(
                case=subtest, status=status, timeElapsed=self._last_testCaseDuration,
                captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue(),
    ...
    def addSuccess(self, test:_unittest.TestCase):
        self.successes.append(_CaseResult(
            case=test, status=CaseStatusEnum.Success, timeElapsed=self._last_testCaseDuration,
            captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue()
        ))
    ...
    def addError(self, test:_unittest.TestCase, err:_TypeExcInfo):
        ...
        self.errors.append(_CaseResult(
            case=test, status=CaseStatusEnum.Error, timeElapsed=self._last_testCaseDuration,
            captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue(),
            exception=err, exceptionString=self._exc_info_to_string(err, test)
        ))
    ...
    def addFailure(self, test:_unittest.TestCase, err:_TypeExcInfo):
        ...
        self.failures.append(_CaseResult(
            case=test, status=CaseStatusEnum.Fail, timeElapsed=self._last_testCaseDuration,
            captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue(),
            exception=err, exceptionString=self._exc_info_to_string(err, test)
        ))

    def addSkip(self, test, reason):
        self.skipped.append(_CaseResult(
            case=test, status=CaseStatusEnum.Skip, timeElapsed=self._last_testCaseDuration,
            captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue(),
            skipReason=reason
        ))
    ...
    def addExpectedFailure(self, test, err):
        ...
        self.expectedFailures.append(_CaseResult(
            case=test, status=CaseStatusEnum.ExpectedFail, timeElapsed=self._last_testCaseDuration,
            captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue(),
            exception=err, exceptionString=self._exc_info_to_string(err, test)
        ))
    ...
    def addUnexpectedSuccess(self, test):
        ...
        self.expectedFailures.append(_CaseResult(
            case=test, status=CaseStatusEnum.UnexpectedSuccess, timeElapsed=self._last_testCaseDuration,
            captured_stdout=self._stdout_buffer.getvalue(), captured_stderr=self._stderr_buffer.getvalue(),
        ))
...
class ArtifactTestRunner:
    ...
    def Run(self, testSuite: _unittest.TestSuite):
        import time

        result = _ArtifactTestResult()
