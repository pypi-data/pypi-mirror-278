...
def list_group(list: list[E], callback: Callable[[list[list[T]], int, int, E, int, list[E]],T], columns: int)->list[list[T]] :
    c = 0
    rows = []
    cols = []
    for i in range(len(list)):                 
        try:
            cols.append(callback(rows, len(rows), c, list[i], i, list))
        except:
            try:
                cols.append(callback(rows, len(rows), c, list[i], i))                
            except:
                try:
                    cols.append(callback(rows, len(rows), c, list[i]))   
                except:
                    try:
                        cols.append(callback(rows, len(rows), c))                           
                    except:
                        try:
                            cols.append(callback(rows, len(rows)))                         
                        except:
                            cols.append(callback(rows))  
        c += 1
        if c == columns:
            rows.append(cols)
            cols = []
