...
class GoogleDocDialect(IndentDocstringDialect):
    ...
    def get_parameters_info(self, docstring: str) -> Dict[str, ParameterInfo]:
        ...
        for fragment in parameters_section:
            ...
            # Split name_part to handle (type) information
            param_match = re.match(r"(\w+)\s*\(([\w\s]+)\)?", name_part)
            if not param_match:
                warnings.warn(
                    f"Invalid parameter definition in docstring: {name_part.strip()}"
                )
                continue
            ...
            if param_type:
                handle_type_repr(parameter_info, param_type)
            
            parameters[param_name.strip()] = parameter_info
    ...
    def get_return_info(self, docstring: str) -> Optional[ReturnInfo]:
        ...
        return_type_match = re.match(r"(\w+)\s*:\s*", description)
        if return_type_match:
            return_type = type_repr_to_type(return_type_match.group(1))
            description = description[return_type_match.end():]
        else:
            return_type = None
