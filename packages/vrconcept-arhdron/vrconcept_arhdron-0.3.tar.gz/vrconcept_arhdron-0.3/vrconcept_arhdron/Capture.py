...
class Capture:
    ...
    object_rotate = 360.0
    object_scale = 0.01
    object_value_x_index = 10
    object_value_y_index = 11
    object_value_z_index = 12
    ...
    def run(self, send_array):
        if self.is_button_pressed:
            radius = math.sqrt(math.pow((self.move_x - send_array[self.drone_value_x_index]), 2) + math.pow((self.move_y - send_array[self.drone_value_y_index]), 2) + math.pow((self.move_z - send_array[self.drone_value_z_index]), 2))
            ...
            self.object_position = [send_array[self.object_value_x_index], send_array[self.object_value_y_index], send_array[self.drone_value_z_index]]
            self.is_captured = False
        return send_array