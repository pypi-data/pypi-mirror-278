...
def configure_model_from_mps_file(model: Model, file_path: Path) -> None:
    ...
    try:
        ...
        nbr_of_varaiables = len(variables)
        ...
        model.set_objective_matrices(np.zeros((nbr_of_varaiables, nbr_of_varaiables), dtype=np.float32), coefficients.astype(np.float32))
