...
from jlx_helper.config import url_map
from jlx_helper.utils import timestamp_to_datetime_str
...
class OrderService(object):
    ...
    def get_token(self) -> str:
        ...
        if response_data.get("code") == 0 and response_data.get("success") is True:
            ...
            logger.warning("获取就旅行api鉴权token失败，接口返回内容：{}".format(response_data))
        return token
    ...
    def get_flight_quotation(self, ticket_channel: str, channel_user: str, channel_password: str, departure_date: str,
                             ...
                             trip_type: str = 'single_trip') -> list:
        if not jlx_token:
        ...
        if response_data.get("code") == 0 and response_data.get("success") is True:
            return response_data.get("data") if isinstance(response_data.get("data"), list) else list()
        else:
            logger.warning("获取就旅行航班报价信息失败，接口返回内容：{}".format(response_data))
        return list()
    ...
    def gen_jlx_order(self, ticket_channel: str, channel_user: str, order_id: str, departure_date: str,
                      ...
                      internal_phone: str, cabin: str = None, jlx_token: str = None, price_context: str = None,
                      conditions: list = None, trip_type: str = 'single_trip') -> dict:
        if not jlx_token:
            jlx_token = self.get_token()
            if not jlx_token:
                return dict()
        ...
        if response_data.get("code") == 0 and response_data.get("success") is True:
            ...
            logger.warning("创建就旅行出票订单失败，接口返回内容：{}".format(response_data))
        return dict()
    ...
    def payment_jlx_order(self, ticket_channel: str, channel_user: str, jlx_order_id: str, pay_type: str,
        ...
        if response_data.get("code") == 0 and response_data.get("success") is True:
            ...
            logger.warning("支付就旅行订单<{}>失败，接口返回内容：{}".format(jlx_order_id, response_data))
        return dict()
    ...
    def get_jlx_order_itinerary_info(self, ticket_channel: str, channel_user: str, jlx_order_id: str,
        ...
        if response_data.get("code") == 0 and response_data.get("success") is True:
            ...
            logger.warning("获取就旅行订单<{}>的票号失败，接口返回内容：{}".format(jlx_order_id, response_data))
        return dict()
