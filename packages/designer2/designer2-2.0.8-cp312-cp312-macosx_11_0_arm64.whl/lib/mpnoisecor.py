...
class MP(object):
    def __init__(self, dwi, kernel, step, shrinkage, algorithm, crop, n_cores):
        ...
        self.sigma = self.get_sigma_prior(self.dwi)
    ...
    def denoise(self, X, sigma_prior):
        """
        ...
        t = np.where(sigmasq_2 < np.mean(sigma_prior, where=sigma_prior > 0))
        ...
        #gc.collect()
        return s, sigma, npars
    
    def get_sigma_prior(self, X):
        
        print('getting sigma prior')
        u,s,v = svd(X, full_matrices=False)
        vals = (s**2).astype(float)

        ydata = np.log(vals)
        xdata = np.arange(1,len(vals)+1)

        weights = abs(np.diff(ydata, 2))
        # deweight the first and last points
        weights = np.hstack((1/weights, np.finfo(float).eps, np.finfo(float).eps))
        weights[0] = np.finfo(float).eps
        weights[~np.isfinite(weights)] = np.finfo(float).eps
        #weights = weights / np.sum(weights)

        design = np.vstack([xdata**(3-i) for i in range(4)]).T
        design_weighted = design * np.sqrt(weights[:, np.newaxis])
        ydata_weighted = ydata * np.sqrt(weights)
        lsqres = np.linalg.lstsq(design_weighted, ydata_weighted, rcond=None)
        coeffs = lsqres[0]
        yfit = design @ coeffs

        t = np.where(abs(ydata-yfit) < 0.2)
        if np.any(t):
            t = t[0][0]
        else:
            t = len(xdata)-1
        ...
        datadn = u @ np.diag(s) @ v

        return np.std(X - datadn, axis=-1)**2
    ...
    def process(self):
        ...
        results = Parallel(n_jobs=self.n_cores, prefer='processes')\
            (delayed(self.denoise)(
                self.dwi[self.temp[i,:],:], self.sigma[self.temp[i,:]]
