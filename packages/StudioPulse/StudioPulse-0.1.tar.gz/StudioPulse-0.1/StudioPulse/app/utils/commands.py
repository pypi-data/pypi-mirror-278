...
from StudioPulse.app.utils import thumbnail as thumbnail_utils, auth
from StudioPulse.app.stores import auth_tokens_store, file_store
...
def sync_with_ldap_server():
    ...
    EMAIL_DOMAIN = os.getenv("EMAIL_DOMAIN", "StudioPulse.local")
...
def dump_database(store=False):
    ...
    filename = f"StudioPulse-db-backup-{now}.sql.gz"
    if store:
