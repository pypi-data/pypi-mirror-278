...
from StudioPulse.app import config, db
...
class Person(db.Model, BaseMixin, SerializerMixin):
    """
    Describe a member of the StudioPulse (and an API user).
    """
    ...
    StudioPulse_id = db.Column(
        UUIDType(binary=False), db.ForeignKey("StudioPulse.id"), index=True
    )
    ...
    def present_minimal(self, relations=False, milliseconds=False):
        ...
        return {
            ...
            "StudioPulse_id": data["StudioPulse_id"],
            "role": data["role"],
