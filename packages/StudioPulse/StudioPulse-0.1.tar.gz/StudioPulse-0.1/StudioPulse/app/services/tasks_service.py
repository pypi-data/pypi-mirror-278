...
from StudioPulse.app import app, db
from StudioPulse.app.utils import events

from StudioPulse.app.models.attachment_file import AttachmentFile
from StudioPulse.app.models.comment import (
    Comment,
...
from StudioPulse.app.utils import (
    cache,
    fields,
...
from StudioPulse.app.services.exception import (
    ...
    StudioPulseNotFoundException,
    WrongDateFormatException,
    TimeSpentNotFoundException,
)

from StudioPulse.app.services import (
    assets_service,
    edits_service,
    base_service,
    files_service,
...
def clear_StudioPulse_cache(StudioPulse_id):
    cache.cache.delete_memoized(get_StudioPulse, StudioPulse_id)
    cache.cache.delete_memoized(get_StudioPulses)
...
@cache.memoize_function(120)
def get_StudioPulses():
    return fields.serialize_models(StudioPulse.get_all())
...
@cache.memoize_function(120)
def get_StudioPulse(StudioPulse_id):
    """
    Get StudioPulse matching given id as a dictionary.
    """
    try:
        StudioPulse = StudioPulse.get(StudioPulse_id)
    except StatementError:
        raise StudioPulseNotFoundException

    if StudioPulse is None:
        raise StudioPulseNotFoundException

    return StudioPulse.serialize()
