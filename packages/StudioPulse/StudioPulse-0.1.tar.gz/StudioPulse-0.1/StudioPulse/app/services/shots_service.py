...
from StudioPulse.app.models.task import assignees_table
...
def get_weighted_quotas(project_id, task_type_id, StudioPulse_id=None):
    """
    ...
    if StudioPulse_id is not None:
        persons_from_StudioPulse = Person.query.filter(
            Person.StudioPulse_id == StudioPulse_id
        ).all()
        query = query.filter(
            or_(*[Task.assignees.contains(p) for p in persons_from_StudioPulse])
        )
    result = query.all()
...
def get_raw_quotas(project_id, task_type_id, StudioPulse_id=None):
    """
