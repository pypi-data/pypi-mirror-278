...
from StudioPulse.app.utils import dbhelpers, auth, commands
from StudioPulse.app.services import persons_service, auth_service
...
from StudioPulse import __file__ as root_path
...
def version():
    ...
    """
    from StudioPulse import __version__

    print(f"StudioPulse version: {__version__}")
...
def upgrade_db(no_telemetry=False):
    ...
    with app.app_context():
        ...
        if not no_telemetry and config.IS_SELF_HOSTED:
            from StudioPulse.app.services import telemetry_services
...
def clear_memory_cache():
    ...
    from StudioPulse.app import cache
...
def dump_database(store=False):
    """
    Dump database described in StudioPulse environment variables and save it to
