...
class iSession(iContext, iSchema, iAuthenticator):
    ...
    async def _inspect_schema(self, uri, kind):
...
class iAgent:
    ...
    async def bootstrap(self):
        ...
        for func, args, kwargs in self._bootstrap():
            ...
            wave0 = await self._get_initial_wave(target)
            if not wave0:
                log.warning(
...
class iSQLSession(iSession):
    ...
    async def get_samples(self, _uri: DURI, N=10):
        ...
        sql = f"SELECT * FROM {_kind['_path']} LIMIT {N}"
    ...
    async def _inspect_schema(self, _uri: DURI):
        ...
        return schema
        # TODO: delete function from this point
        table = kind
        ...
        async with self as session:
            log.debug("trying to get table scheme for [%s]", kind)
            method = getattr(session, self.DEFAULT_METHOD)
            response = await method(
                url, headers=self.bot.headers, json=payload
            )
            response = await method(
                url, headers=self.bot.headers, json=payload
            )
            async with response:
                if 200 <= response.status < 300:
                    ...
                    info = self.RESULT_STRUCTURE = (
                        self._map_result_structure(data, rows=N)
                    )
                    # info = self.guess_schema(**kw)
                elif 400 <= response.status < 500:
                    # need authentication
                    await self.authenticate(url)
                else:
                    raise RuntimeError(f"auth for {url} failed")
        return info
...
class iAsyncCrawler(iCrawler):
    ...
    async def _get_initial_wave(self, uri):
        ...
        if not storage:
            ...
            raise RuntimeError(
                f"You need to use a WaveStorage in any SyncModel to use waves"
            )
