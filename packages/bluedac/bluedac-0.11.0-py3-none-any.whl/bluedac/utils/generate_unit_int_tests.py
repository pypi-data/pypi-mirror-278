...
def generate_unit_int_tests(project_root: str, stack: str):
    ...
    for res in used_resources:
        ...
        shutil.copytree(f"{os.environ["DAC_PATH"]}/tests/unit/{res}", f"{project_root}/tests/unit/{res}",
                        dirs_exist_ok=True)

    # ------------------- Copying integration tests. ------------------- #
    shutil.copytree(f"{os.environ["DAC_PATH"]}/tests/integration", f"{project_root}/tests/integration",
                    dirs_exist_ok=True)
